...
Logger.info('KEYSTONEJS', 'Building KeystoneJS docs...');
...
content.routes.forEach(function(route) {
	...
	Logger.info('KEYSTONEJS', 'Writing ' + filename + '.html');
	fs.outputFileSync(filepath, html);
	
});

Logger.info('KEYSTONEJS', 'KeystoneJS docs built to ' + path.resolve(args.dest));
process.exit();
