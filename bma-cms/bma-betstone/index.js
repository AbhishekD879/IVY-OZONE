...
// Coral start
  this.set('cors allow origin', process.env.ALLOWED_CORS_ORIGINS);
// Coral end
  ...
  if (process.env.AKAMAI_KEY_NAME && process.env.AKAMAI_KEY && process.env.AKAMAI_HOST && process.env.AKAMAI_PATH) {
    var akamaiConf = {
      akamaiKeyName: process.env.AKAMAI_KEY_NAME,
      akamaiKey: process.env.AKAMAI_KEY,
      akamaiHost: process.env.AKAMAI_HOST,
      akamaiPath:process.env.AKAMAI_PATH,
      akamaiCredUser:process.env.AKAMAI_CRED_USER,
      akamaiCredPass:process.env.AKAMAI_CRED_PASS,
      akamaiUrl:process.env.AKAMAI_URL,
      akamaiTimeout: Number(process.env.AKAMAI_TIMEOUT)
    };
    this.set('akamai config', akamaiConf);
    akamai.setConfig({
      keyName: akamaiConf.akamaiKeyName,
      key: akamaiConf.akamaiKey,
      host: akamaiConf.akamaiHost,
      path: akamaiConf.akamaiPath,
      cred_user: akamaiConf.akamaiCredUser,
      cred_pass: akamaiConf.akamaiCredPass,
      url: akamaiConf.akamaiUrl,
      request: { 
	    timeout: akamaiConf.akamaiTimeout
	  }
    });
  }

  if (process.env.IMAGEMIN_ENABLED) {
    var imageminConfig = {
      enabled: process.env.IMAGEMIN_ENABLED
    };

    if (process.env.IMAGEMIN_JPEG_QUALITY) {
      imageminConfig.jpeg = {
        quality: process.env.IMAGEMIN_JPEG_QUALITY
      };
    }

    if (process.env.IMAGEMIN_PNG_OPTIMIZATION_LEVEL) {
      imageminConfig.png = {
        optimizationLevel: process.env.IMAGEMIN_PNG_OPTIMIZATION_LEVEL
      };
    }

    imagemin.setConfig(imageminConfig);
  }
