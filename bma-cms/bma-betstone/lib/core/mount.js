...
function mount(mountPath, parentApp, events) {
  ...
  if (!this.app) {
    Logger.error('MOUNT', 'KeystoneJS Initialisaton Error: app must be initialised. Call keystone.init() or keystone.connect(new Express()) first.');
    process.exit(1);
  }
  ...
  if (!this.get('cookie secret')) {
    Logger.error('MOUNT', 'KeystoneJS Configuration Error: Please provide a `cookie secret` value for session encryption.');
    process.exit(1);
  }
    ...
    switch (sessionStore) {
      ...
      case 'connect-mongostore':
        ...
        if (!sessionStoreOptions.db) {
          Logger.error('MONGO', 'ERROR: connect-mongostore requires `session store options` to be set.',
    ...
    try {
      ...
      sessionStorePromise = new P(function(resolve) {
        ...
        sessionOptions.store.on('disconnect', function() {
          Logger.error('MOUNT', `There was an error connecting to the ${sessionStore} session store.`);
          process.exit(1);
        });
      });
    } catch(e) {
      if (e.code === 'MODULE_NOT_FOUND') {
        Logger.error('MOUNT', e.toString(),
        ...
        try {
          if ('function' === typeof err404) {
            ...
            if (keystone.get('logger')) {
              Logger.error('MOUNT', 'Error handling 404 (not found): Invalid type (' + (typeof err404) + ') for 404 setting.');
            }
            default404Handler(req, res, next);
          }
        } catch(e) {
          if (keystone.get('logger')) {
            Logger.error('MOUNT', 'Error handling 404 (not found): ', e);
          }
          default404Handler(req, res, next);
        }
      } else {
    ...
    var default500Handler = function(err, req, res, next) {
      ...
      if (keystone.get('logger')) {
        if (err instanceof Error) {
          Logger.error('MOUNT', (err.type ? err.type + ' ' : '') + 'Error thrown for request: ' + req.url);
        } else {
          Logger.error('MOUNT', 'Error thrown for request: ' + req.url);
        }
        Logger.error('MOUNT', err.stack || err);
      }
      ...
      if (err500) {
        try {
          if ('function' === typeof err500) {
            ...
            if (keystone.get('logger')) {
              Logger.error('MOUNT', 'Error handling 500 (error): Invalid type (' + (typeof err500) + ') for 500 setting.');
            }
            default500Handler(err, req, res, next);
          }
        } catch(e) {
          if (keystone.get('logger')) {
            Logger.error('MOUNT', 'Error handling 500 (error): ', e);
          }
          default500Handler(err, req, res, next);
        }
      } else {
