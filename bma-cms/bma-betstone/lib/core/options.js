...
function options() {
	...
	exports.set = function(key, value) {
		...
		if (remappedOptions[key]) {
			if (this.get('logger')) {
				Logger.warn('OPTIONS', `Warning: the ${key} option has been deprecated. Please use ${remappedOptions[key]} instead.`,
			...
			case 'mongo':
				if ('string' !== typeof value) {
					if (Array.isArray(value) && (value.length === 2 || value.length === 3)) {
						Logger.warn('OPTIONS', 'Warning: using an array for the `mongo` option has been deprecated.',
              'Please use a mongodb connection string, e.g. mongodb://localhost/db_name instead.',
						...
						Logger.error('OPTIONS', 'Invalid Configuration: The `mongo` option must be a mongodb connection string, e.g. mongodb://localhost/db_name');
						process.exit(1);
					}
				}
			break;
