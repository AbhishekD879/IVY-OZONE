	...
	events.onMount = function() {
		...
		var serverStarted = function() {
			...
			if (keystone.get('logger')) {
				Logger.info('KEYSTONE_START', startupMessages);
			}
		...
		if (ssl && !unixSocket) {
			...
			if (!sslOpts.key || !sslOpts.cert) {
				...
				if (ssl === 'only') {
					Logger.error('KEYSTONE_START', keystone.get('name'), ' failed to start: invalid ssl configuration');
					process.exit();
				} else {
		...
		process.on('uncaughtException', function(e) {
			if (e.code === 'EADDRINUSE') {
				...
				Logger.error('KEYSTONE_START', 'Connection reset by peer');
				Logger.error('KEYSTONE_START', e);
			} */else {
				Logger.error('KEYSTONE_START', e.stack || e);
				process.exit(1);
			}
		});
