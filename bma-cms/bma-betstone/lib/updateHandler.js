...
UpdateHandler.prototype.process = function(data, options, callback) {
	...
	var progress = function(err) {
		if (err) {
			if (options.logErrors) {
				Logger.error('UPDATE_HANDLER', 'Error saving changes to ' + this.item.list.singular + ' ' + this.item.id + ':');
				Logger.error('UPDATE_HANDLER', err);
			}
	...
	options.fields.forEach(function(path) {
		...
		if (usingDefaultFields && field.noedit && !options.ignoreNoedit) {
		  Logger.warn('UPDATE_HANDLER', `Skipping field ${path} (noedit: true)`);
			return;
		}
