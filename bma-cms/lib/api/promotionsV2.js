...
exports = module.exports = function(options) {
  ...
  function formQuery(categories) {
    return categories && categories.length
      ? { $or: [ { categoryId: { $in: categories.map(item => { return item.id; }) } }, { categoryId: { $size: 0 } } ] }
      : {};
  }
  ...
  function getPromotions(categories) {
    ...
    return getSystemConfig().then(config => {
      ...
      keystone.list('promotions')
        .model.find(formQuery(categories))
        .where(subQuery)
  ...
  return getCategories(options.categories).then(getPromotions);
};
