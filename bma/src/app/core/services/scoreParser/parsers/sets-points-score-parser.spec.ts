...
describe('SetsPointsScoreParser', () => {
  let parser: SetsPointsScoreParser;
  let eventNamePipe: EventNamePipe;

  beforeEach(() => {
    eventNamePipe = new EventNamePipe();
    parser = new SetsPointsScoreParser(eventNamePipe);
  });
  ...
  describe('should not match strings and return null', () => {
    ...
    it('if there is no space between (U) X-Y (V) group and serving mark', () => {
      expect(parser.parse('TeamA *(3) 1-2 (4) TeamB')).toEqual(null);
      expect(parser.parse('TeamA (3) 1-2 (4)* TeamB')).toEqual(null);
      expect(parser.parse('Team A (0) 2-3 (0) *Team B').away.isServing).toBeTruthy();
      expect(parser.parse('Team A (0) 2-3 (0) Team B*').away.isServing).toBeTruthy();
      expect(parser.parse('Team A* (0) 2-3 (0) Team B').home.isServing).toBeTruthy();
      expect(parser.parse('*Team A (0) 2-3 (0) Team B').home.isServing).toBeTruthy();
    });
