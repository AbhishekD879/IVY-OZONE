import {
  ...
  export class MultiMarketTemplateComponent implements OnInit, OnChanges, OnDestroy {
    ...
    multiMarketList: string[];
    eventMarketList: string[];
    diffMarketList: string[];
    readonly sportName = 'football';
    showThreeOdds: boolean = false;
    ...
    constructor(
      ...
      private outcomeTemplateHelper: OutcomeTemplateHelperService
  ) {
    ...
    init(): void {
      this.showThreeOdds = false;
      this.multiMarketList = this.selectedMarket ? this.selectedMarket.split(',') : [];
      const frameMrkts = [];
      this.eventMarketList = this.marketTypeService.extractMarketNameFromEvents([this.event], true);
      this.event.markets.forEach((market) => {
        ...
        if (market.outcomes.length >= 3 && this.multiMarketList.indexOf(market.templateMarketName) !== -1) {
          this.showThreeOdds = true;
        }
      });
      ...
      this.eventMarketList = this.marketTypeService.extractMarketNameFromEvents([this.event], true);
      this.diffMarketList = _.difference(this.multiMarketList, this.eventMarketList);
      this.marketsCountFilter = this.getMarketsCount(this.event);
      ...
      this.watchVariables.push(this.marketsCountFilter);
    ...
    reorder(_event: ISportEvent) {
      this.outcomeTemplateHelper.setOutcomeMeaningMinorCode(_event.markets, _event);
      this._event.markets.forEach((market) => {
        if (market.outcomes.length >= 3) {
          const newOutcomes = [];
          newOutcomes.push(market.outcomes[0]);
          newOutcomes.push(market.outcomes[2]);
          newOutcomes.push(market.outcomes[1]);
          market.outcomes = newOutcomes;
        }
      });
    }
    isFanzonePage() {
    ...
    isSelectedMarket(market: IMarket): boolean {
      if (this.selectedMarket) {
        const marketFilterList = this.selectedMarket.split(',');
        return marketFilterList.indexOf(market.templateMarketName) !== -1
          || marketFilterList.indexOf(market.name) !== -1
      }
      return true;
    }
    /**
    ...
    private addRecalculationEventListeners() {
      ...
      this.pubSubService.subscribe(`oddCardSport_${this.uniqueId}_${this.event.id}`, this.pubSubService.API.OUTCOME_UPDATED, (market: IMarket) => {
        if (this.selectedMarketIds.length && this.selectedMarketIds.includes(market.id)) {
      ...
      this.pubSubService.subscribe(`oddCardSport_${this.uniqueId}_${this.event.id}`, this.pubSubService.API.WS_EVENT_UPDATED, (event: ISportEvent) => {
          if (this.event && event && this.event.id === event.id) {​​​​​​​
            if(event.comments){
              this.event.comments = JSON.parse(JSON.stringify(event.comments));
              this.watchGroupHandler();
            }
            this.changeDetectorRef.detectChanges();
          }
    ...
    isShowMarketsCount(): boolean {
      return !this.sportsViewTypes.outrights && this.marketsCountFilter > 1 && this.isOddsSports;
    }
    /**
    ...
    private getCorrectedOutcomes() {
        ...
        const marketOutcomes = this.event.markets
          .filter((market: IMarket) => {
            const hasMarket = this.multiMarketList.indexOf(market.templateMarketName) !== -1;
            if(hasMarket){
              this.selectedMarketIds.push(market.id)
            }
            return hasMarket;
          }).map((market: IMarket) => market.outcomes)
          .filter((outcome: IOutcome[]) => outcome);
        this.subscribeOutcomeChanges(marketOutcomes.flat());
    }
    ...
    /**
     *  event.marketsCount 
     * 'Total marketsCount' minus ('displayed markets in slp' minus ())
     *
     * @param event
     */
    getMarketsCount(event: ISportEvent): number {
      ...
      if (_.isNumber(event.marketsCount)) {
        marketsCount = event.marketsCount - ((this.multiMarketList.length - (_.difference(this.multiMarketList,this.eventMarketList)).length) -1);
      }
      return marketsCount;
    }
    /**
     * To append O (over), U (under) for handicap value
     * @param templateMarketName  string
     * @param name string
     * @returns string
     */
    setOverUnderTag(templateMarketName, name) : string {
      const overUnder = ['O','U'];
      if(templateMarketName.toLowerCase().includes('points') && overUnder.includes(name.charAt(0))) {
