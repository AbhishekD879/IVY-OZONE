...
describe('RacingOutcomeResultedCardComponent', () => {
  ...
  it('#ngOnInit with full data', () => {
    ...
    component.unPlaced = {} as any;
    component.nonRunners = {} as any;
    ...
    expect(raceOutcomeDetailsService.getSilkStyle).toHaveBeenCalledWith(component.nonRunners, component.outcomeEntity, '0');
  ...
  it('#ngOnInit with unPlaced full data', () => {
    component.outcomeEntity = outcome;
    component.unPlaced = outcome;

    component.ngOnInit();
    expect(component.nonRunners).toBeFalsy();
    expect(component.unPlaced).toBeTruthy();
    expect(raceOutcomeDetailsService.getSilkStyle).toHaveBeenCalledWith(component.unPlaced, component.outcomeEntity, '0');
  });
  ...
  it('#getPositionWithSuffix should return empty suffix', () => {
    component.outcomeEntity = { results: {
        position: '0',
        priceNum: 1,
        priceDen: 7,
        resultCode: 'L'
    } } as any;
    expect(component.outcomeEntity.results.position).toEqual('0');
    expect(component.outcomeEntity.results.resultCode).toEqual('L');
    expect(component.outcomeEntity.results.resultCode).not.toBe('-');
  ...
  it('#getPositionWithSuffix should return resultcode suffix', () => {
    component.outcomeEntity = { results: {
        position: '0',
        priceNum: 1,
        priceDen: 7,
    ...
    const result = component['getPositionWithSuffix']();
    expect(component.outcomeEntity.results.position).toEqual('0');
    expect(result).toEqual('PU');
  });
