...
describe('RaceTimerComponent', () => {
  ...
  let timeSyncService, timeService, windowRef, changeDetectorRef;
  beforeEach(() => {
    timeSyncService = {
      getTimeDelta: jasmine.createSpy('getTimeDelta').and.returnValue(123)
    };
    timeService = {
      formatByPattern: jasmine.createSpy('getTimeDelta').and.returnValue('formattedTime')
    };
    windowRef = {
      nativeWindow: {
        _setTimeoutId: 5,
    ...
    component = new RaceTimerComponent(timeSyncService, timeService, windowRef, changeDetectorRef);
  });
  it('should create', () => {
  ...
  it('should set properties', () => {
    expect((component as any).DURATION).toEqual(2700000);
  ...
  describe('ngOnChanges', () => {
    ...
    it('should call init method', () => {
      component.ngOnChanges({ event: true } as any);
      expect((component as any).init).toHaveBeenCalled();
    });
    it('should not call init method', () => {
      component.ngOnChanges({ event: null } as any);
  ...
  describe('ngOnDestroy', () => {
    it('should clear timeouts', () => {
      ...
      expect(windowRef.nativeWindow.clearTimeout.calls.allArgs()).toEqual([[1], [2]]);
    });
  ...
  describe('init', () => {
    ...
    describe('should do nothing', () => {
      it('if event is unavailable', () => component.event = null);
      it('if event has no startTime', () => component.event = {} as any);
      afterEach(() => {
        component.init();
    ...
    describe('if event.startTime is available', () => {
      beforeEach(() => {
        component.event = { startTime: '13:52' } as any;
      });
      ...
      it('should set startTime to event.startTime parsed time', () => {
        component.init();
      ...
      it('should set startTimeStr to formatted timestring', () => {
        ...
        expect((window as any).Date).toHaveBeenCalledWith('13:52');
        expect(timeService.formatByPattern).toHaveBeenCalledWith(date, 'HH:mm', 'UTC');
      });
      ...
      it('should set postpone timeout if getDiff is greater than 45min', () => {
        ...
        expect(windowRef.nativeWindow.setTimeout).toHaveBeenCalledWith(jasmine.any(Function), 1);
        expect((component as any).tick).not.toHaveBeenCalled();
  ...
  it('status getter should return event.raceStage value', () => {
    (component as any).event = { raceStage: 'isOff' };
  ...
  describe('isCountdown', () => {
    ...
    describe('should return false', () => {
      it('if displayCountdown=false', () => component.displayCountdown = false);
      describe('if displayCountdown=true but', () => {
        let value;
        beforeEach(() => component.displayCountdown = true);
        it('start time is equal 0', () => value = 0);
        it('start time is equal 45min', () => value = (component as any).DURATION);
        afterEach(() => (component as any).getDiff.and.returnValue(value));
      });
