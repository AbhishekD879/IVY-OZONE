...
describe('BreadcrumbsService', () => {
  ...
  describe('#buildEDPBreadcrumb', () => {
    ...
    it('should build breadcrumb path for Greyhounds', () => {
      ...
      expect(result).toEqual([
        { name: 'Home', targetUri: '/' },
        { name: 'greyhound racing', targetUri: '/greyhound-racing/today' },
        { name: 'Shepp dogs 25 oct', targetUri: '/greyhound-racing/shepp-dogs-25-oct' }
      ]);
    });

    it('should build breadcrumb path for Olympics football', () => {
      service['config'].isOlympicsPage = true;
      service['config'].eventName = 'USA vs China';
      service['breadcrumbsTitles'] = { 'olympics': 'olympics', 'football': 'football' };
      const pathArray = ['olympics', 'football'];
      const result = service['initializeBreadcrumbsList'](pathArray);
      expect(result).toEqual([
        { name: 'Home', targetUri: '/' },
        { name: 'olympics', targetUri: '/olympics' },
        { name: 'football', targetUri: '/olympics/football' }
      ]);
    });
