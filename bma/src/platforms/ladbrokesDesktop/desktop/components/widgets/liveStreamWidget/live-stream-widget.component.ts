...
export class LiveStreamWidgetComponent extends AbstractOutletComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.pubSubService.subscribe(this.title, this.pubSubService.API.DELETE_EVENT_FROM_CACHE,
      (eventId: number) => {
  ...
  get market(): IMarket | null {
    return (this.event && this.event.markets && this.event.markets[0]) ? this.event.markets[0] : null;
  }

  private set market(value: IMarket | null){}
  ...
  private handleSelectionDeletion(updateData: { selectionId: string; marketId: string; eventId: string; }) {
