    ...
    this.routeListener = this.route.params.pipe(
      concatMap((params: Params) => {
        ...
        const sportDataObservable = this.inplayMainService.getSportId(this.sportUri).pipe(concatMap((sportId: number) => {
          ...
          const isHR = this.sportId.toString() === this.HORSE_RACING_CATEGORY_ID;
            /**
            ...
            }, true, true, true, isHR, this.topMarkets);
          }));

        const ribbonDataObservable$ = this.inplayMainService.getRibbonData().pipe(takeUntil(this.unsubscribe));
        return observableForkJoin([systemConfigObservable, sportDataObservable, ribbonDataObservable$]);
      }))
      .subscribe((routeData) => {
        const systemConfig: ISystemConfig = routeData[0];
        const sportData = routeData[1];
        const ribbonCache: IRibbonCache = routeData[2];
         ...
         * attr: true - subscribe for events
         */
        this.getSwitchers(ribbonCache.data);
