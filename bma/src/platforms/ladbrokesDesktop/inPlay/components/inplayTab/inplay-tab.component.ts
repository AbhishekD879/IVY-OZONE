...
export class InplayTabComponent implements OnInit, OnDestroy {
  ...
  isAwaitingForConnect: boolean = false;
  filter: string;
  ...
  ngOnInit(): void {
     ...
     */
    this.inPlayConnectionService.connectComponent().subscribe(() => {
      this.appReady = true;
      /**
      ...
      this.cmsService.getSystemConfig(null).subscribe(data => {
        this.expandedLeaguesCount = Number(data.InPlayCompetitionsExpanded.competitionsCount);
        /**
         * Get data on init - General approach
         */
  ...
  addEventListeners(): void {
    ...
    if (this.singleSport && this.id) {
      ...
      this.pubsubService.subscribe('inplay', this.pubsubService.API.EVENT_COUNT_UPDATE, data => {
  ...
  goToFilter(filter: string): void {
    if (filter !== this.filter) {
      this.resetDropdown = (filter === this.liveNowTab);
