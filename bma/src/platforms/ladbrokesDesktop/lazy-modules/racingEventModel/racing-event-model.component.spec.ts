...
describe('Ladbrokes LadbrokesDesktopRacingEventModelComponent', () => {
  let component: LadbrokesDesktopRacingEventModelComponent;
  let windowRef;
  let timeService;
  let pubSubService;
  ...
  beforeEach(() => {
    ...
    horseracingService = {
      ...
      isRacingSpecials: jasmine.createSpy(),
    ...
    sbFiltersService = {
      orderOutcomeEntities: jasmine.createSpy('orderOutcomeEntities').and.returnValue(outcomes)
    };
    routerService = {
    ...
    component = new LadbrokesDesktopRacingEventModelComponent(
      windowRef,
      timeService,
      pubSubService,
      nativeBridgeService,
  ...
  it('formatEventTerms', () => {
    const result = component.formatEventTerms('Each Way: ODDS- places 12312312');

    expect(result).toEqual('E/W Places 12312312');
  });

  describe('#ngOnInit', () => {
    it('test cmsService.getSystemConfig', () => {
      component.ngOnInit();

      expect(cmsService.getSystemConfig).toHaveBeenCalled();
      expect(component.sortOptionsEnabled).toEqual(true);
      expect(component.applySortBy).toHaveBeenCalled();
      expect(pubSubService.subscribe).toHaveBeenCalledWith('RacingEventComponent', 'SORT_BY_OPTION1', jasmine.any(Function));
    });
    ...
    it('should expandedSummary equals [[false]]', () => {
      component.ngOnInit();
  ...
  describe('#toggleShowOptions', () => {
    it('should toggleShowOptions by index', () => {
      const expandedSummary = [[false, false, false], [false, false, false], [false, false, false]];
      component.toggleShowOptions(expandedSummary, 1, true);

      expect(expandedSummary).toEqual([[false, false, false], [true, true, true], [false, false, false]]);
    });
  });

  describe('#onExpandSection', () => {
    it('should call onExpandSection', () => {
      const expandedSummary = [[false, false, false], [false, false, false], [false, false, false]];
      component.onExpandSection(expandedSummary, 2, 2);

      expect(expandedSummary).toEqual([[false, false, false], [false, false, false], [false, false, true]]);
      expect(component.isInfoHidden.info).toEqual(true);
    });

    it('should call onExpandSection isInfoHidden false', () => {
      const expandedSummary = [[false, false, false], [false, false, false], [true, true, true]];
      component.onExpandSection(expandedSummary, 2, 0);
      component.onExpandSection(expandedSummary, 2, 1);
      component.onExpandSection(expandedSummary, 2, 2);

      expect(expandedSummary).toEqual([[false, false, false], [false, false, false], [false, false, false]]);
