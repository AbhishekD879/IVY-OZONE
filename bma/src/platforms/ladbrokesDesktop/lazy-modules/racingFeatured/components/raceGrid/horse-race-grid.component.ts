...
export class DesktopHorseRaceGridComponent extends HorseRaceGridComponent implements OnInit, OnChanges, OnDestroy {
  ...
  generateEventList(): IConstant {
    ...
    const regexpSubRegion = /\(.*\)/g;
    _.each(this.groupedRaces, (racesGroup: IRacingGroup) => {
      if (racesGroup.groupName.match(regexpSubRegion)) {
        racesGroup.subRegion = racesGroup.groupName.match(regexpSubRegion).toString();
        racesGroup.groupName = racesGroup.groupName.replace(racesGroup.subRegion, '').trim();
      }
  ...
  trackSwitchDayEvent(day) {
    ...
    this.racingGaServicePublic.trackEvent({
      ...
      eventAction,
      eventLabel: `select day - ${this.localeServicePublic.getString(day)}`
    });
  }
}
