...
export class BreadcrumbsComponent implements OnInit, OnDestroy {
  ...
  private resetBreadcrumbsList(): void {
    ...
    this.adaptBreadcrumbsForNextRaces();
  }

  private adaptBreadcrumbsForNextRaces(): void {
    this.breadcrumbsItems.forEach((item, index) => {
      if (item.name === 'races') {
        item.name = 'next races';
        item.targetUri = '/horse-racing/races/next';
      }

      if (item.name === 'greyhound racing') {
        item.targetUri = this.defaultTab === 'today' ? '/greyhound-racing/today' : '/greyhound-racing/races/next';
      }
  ...
  private buildConfig(): IBreadcrumbConfig {
    ...
    return {
      ...
      display: this.routingState.getRouteParam('display', this.route.snapshot) ||
                  (this.route.snapshot.firstChild && this.route.snapshot.firstChild.routeConfig.path) || '',
