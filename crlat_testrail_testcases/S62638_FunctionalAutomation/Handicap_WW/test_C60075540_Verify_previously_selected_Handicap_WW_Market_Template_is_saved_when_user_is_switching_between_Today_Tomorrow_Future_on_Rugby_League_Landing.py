...
@pytest.mark.sports
@vtest
class Test_C60075540_Verify_previously_selected_Handicap_WW_Market_Template_is_saved_when_user_is_switching_between_Today_Tomorrow_Future_on_Rugby_League_Landing_Page(Common):
    """
    TR_ID: C60075540
    NAME: Verify previously selected ‘Handicap WW Market’ Template is saved when user is switching between Today/Tomorrow/Future on Rugby League Landing Page
    DESCRIPTION: This test case verifies that previously selected ‘Handicap WW Market’ Template is saved when user is switching between Today/Tomorrow/Future on Rugby League Landing Page
    ...
    def test_005_verify_ga_tracking_for_the_handicap_2_way(self):
        """
    ...
    def test_006_switch_to_the_tomorrow_tab(self):
        ...
        EXPECTED: • Previously selected option is displayed in the 'Market Selector' dropdown (Ex: Handicap 2 way)
        ...
        EXPECTED: Note:
        EXPECTED: If events are not present for Handicap(coral)/Handicap 2-way(Lads) market and if events are present for Total Points market then Total Points will be displayed in the switcher (Same logic applies to Today/Tomorrow/Future Tabs)
