...
@pytest.mark.betslip
@vtest
class Test_C29100_Verify_Adding_Different_Selections_via_Direct_Link_Several_times(Common):
    """
    TR_ID: C29100
    ...
    def test_002_enter_direct_url_valid_outcome_ids_in_address_bar_including_active_and_suspendedstarted_ids___press_enter_key(self):
        ...
        EXPECTED: 4.  Error message on red background is shown on the Bet Slip for each suspended/started selection ( **From OX99** Error message on red background was removed. 'SUSPENDED' label is shown for selection)
        EXPECTED: 5.  Corresponding 'Multiples'/('Forecasts/Tricasts'  actual before OX 98) selections formed from active outcomes are present and shown correctly (if available)
        """
        pass
    ...
    def test_004_repeat_step__2(self):
        ...
        EXPECTED: 4.  Corresponding 'Multiples'/ (/'Forecasts/Tricasts' before OX 98) selections formed from active outcomes are present and shown correctly (if available)
        """
        pass
