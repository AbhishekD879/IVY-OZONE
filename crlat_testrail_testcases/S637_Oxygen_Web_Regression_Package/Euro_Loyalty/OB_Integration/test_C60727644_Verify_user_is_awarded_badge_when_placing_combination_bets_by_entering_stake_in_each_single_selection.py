...
@pytest.mark.other
@vtest
class Test_C60727644_Verify_user_is_awarded_badge_when_placing_combination_bets_by_entering_stake_in_each_single_selection(Common):
    """
    TR_ID: C60727644
    NAME: Verify user is awarded badge when placing combination bets by entering stake in each single selection
    DESCRIPTION: This test case is to verify user is awarded badge when placing combination bets by entering stake in each single selection
    ...
    def test_002_select_2_or_3_selections_from_multiple_events_in_which_at_least_one_should_qualify__to_get_euro_badgeenter_stake_in_each_single_selection_and_make_sure_enter_qualifying_stake_for_euro_qualified_event_and_place_bet(self):
        """
        DESCRIPTION: Select 2 or 3 selections from multiple events in which at least one should qualify  to get euro badge
        DESCRIPTION: Enter stake in each single selection and make sure enter qualifying stake for euro qualified event and place bet
        EXPECTED: User should awarded badge since TBD
        """
        pass
