...
@pytest.mark.other
@vtest
class Test_C61042788_Euro_Loyalty_Customer_receives_a_badge_when_two_qualifying_singles_same_time_on_same_bet_slip(Common):
    """
    TR_ID: C61042788
    NAME: Euro Loyalty-Customer receives a badge when 
two qualifying singles same time on same bet slip
    DESCRIPTION: This test case verifies whether a customer receives a badge  when a bet is placed on two qualifying singles same time on same bet slip
    ...
    def test_002_verify_if_the_customer_places_two_qualifying_singles_same_time_on_same_bet_slip(self):
        """
        DESCRIPTION: Verify if the Customer places two qualifying singles same time on same bet slip
