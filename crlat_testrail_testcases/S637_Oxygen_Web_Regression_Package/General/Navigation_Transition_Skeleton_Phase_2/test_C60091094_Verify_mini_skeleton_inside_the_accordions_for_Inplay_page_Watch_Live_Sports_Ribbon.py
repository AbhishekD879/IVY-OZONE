...
@pytest.mark.in_play
@vtest
class Test_C60091094_Verify_mini_skeleton_inside_the_accordions_for_Inplay_page_Watch_Live_Sports_Ribbon(Common):
    """
    TR_ID: C60091094
    NAME: Verify mini skeleton inside the accordions for Inplay  page_ Watch Live  (Sports Ribbon)
    DESCRIPTION: This test case verifies mini loading skeleton inside accordions for Inplay  page_ Watch Live (Sports Ribbon) while content loads so that user gets feedback that something is happening when expanding accordions.
    ...
    def test_001_tap_on_any_collapsed_accordion_on_the_inplay_page__watch_live_sports_ribbon(self):
        """
        DESCRIPTION: Tap on any collapsed accordion on the Inplay page_ watch Live (Sports Ribbon)
    ...
    def test_005_repeat_all_above_steps_for_all_available_sports_from_inplay_page_from_sports_ribbon(self):
        """
        DESCRIPTION: Repeat all above steps for all available sports from Inplay page from Sports Ribbon
        EXPECTED: Skeleton should not display
        """
        pass
