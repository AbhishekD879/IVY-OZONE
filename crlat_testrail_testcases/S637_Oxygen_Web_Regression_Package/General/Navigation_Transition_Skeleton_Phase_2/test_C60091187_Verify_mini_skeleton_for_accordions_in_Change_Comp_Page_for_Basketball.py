...
@pytest.mark.sports
@vtest
class Test_C60091187_Verify_mini_skeleton_for_accordions_in_Change_Comp_Page_for_Basketball(Common):
    """
    TR_ID: C60091187
    NAME: Verify mini skeleton for accordions in Change Comp Page for Basketball
    DESCRIPTION: This test case verifies mini loading skeleton in Change Competition Page for Basketball inside accordions while content loads so that user gets feedback that something is happening when expanding accordions.
    ...
    PRECONDITIONS: - User is on Basketball -> Competitions Tab -> specific Competition -> Click on 'Change Competition' where accordions can be expanded
    ...
    def test_001_tap_on_any_collapsed_accordion_on_the_change_competition_page(self):
        ...
        EXPECTED: ![](index.php?/attachments/get/122292755)
