...
@pytest.mark.other
@vtest
class Test_C11114607_Featured_module_by_Market_ID_Price_Change(Common):
    """
    TR_ID: C11114607
    NAME: Featured module by Market ID: Price Change
    DESCRIPTION: This test case verifies situation when a price is changed for outcomes of the market on the EventHub tab (mobile/tablet) of a module by MarketID
    PRECONDITIONS: 1. Event Hub is created in CMS > Sport Pages > Event Hub.
    PRECONDITIONS: 2. Featured module by Market ID (Only primary markets, outright markets, Win or Each Way markets supported) is created under Event Hub in CMS and contains event.
    PRECONDITIONS: 3. Module Ribbon Tab is created in CMS (CMS > Module Ribbon Tabs) and Event Hub is assigned to it.
    PRECONDITIONS: 4. User is on Homepage > Featured tab
    PRECONDITIONS: 5. CMS, TI: https://confluence.egalacoral.com/display/SPI/Environments+-+to+be+reviewed
    """
