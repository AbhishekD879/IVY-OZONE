...
@pytest.mark.races
@vtest
class Test_C2986447_Verify_currency_behavior_on_International_tote_race_card(Common):
    """
    TR_ID: C2986447
    NAME: Verify currency behavior on International tote race card
    DESCRIPTION: This test case verifies user and pool currency display on International HR race card.
    PRECONDITIONS: * User should have an International HR EDP open ("Tote" tab)
    ...
    def test_002_navigate_through_the_pool_type_tabs_and_verify_the_pool_value_currency_display(self):
        """
        DESCRIPTION: Navigate through the pool type tabs and verify the pool value currency display
