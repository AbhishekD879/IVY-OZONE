...
@vtest
class Test_C62681927_Verify_whether_the_customer_is_able_to_select_only_one_contest_entry_button_at_a_time(Common):
    """
    TR_ID: C62681927
    NAME: Verify whether the customer is able to select only one contest entry button at a time
    DESCRIPTION: 
    ...
    def test_005_verify_if_the_customer_is_able_to_select_only_one_contest_entry_button_at_a_time(self):
        """
        DESCRIPTION: Verify if the customer is able to select only one contest entry button at a time
        EXPECTED: Customer should be able to select one button at a time
        """
        pass
