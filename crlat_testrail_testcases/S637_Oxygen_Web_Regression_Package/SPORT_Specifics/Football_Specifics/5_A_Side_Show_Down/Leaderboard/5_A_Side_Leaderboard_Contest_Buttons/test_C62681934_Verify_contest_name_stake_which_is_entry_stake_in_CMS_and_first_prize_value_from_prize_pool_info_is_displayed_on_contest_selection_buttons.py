...
@vtest
class Test_C62681934_Verify_contest_name_stake_which_is_entry_stake_in_CMS_and_first_prize_value_from_prize_pool_info_is_displayed_on_contest_selection_buttons(Common):
    """
    TR_ID: C62681934
    NAME: Verify contest name, stake which is entry stake in CMS and first prize value from prize pool info is displayed on contest selection buttons
    DESCRIPTION: 
    ...
    def test_005_check_if_the_contest_name_entry_stake_and_first_prize_value_are_shown_as_per_cms_configuration_on_contest_entry_button(self):
        """
        DESCRIPTION: Check if the contest name, entry stake and first prize value are shown as per CMS configuration on contest entry button
        EXPECTED: Contest name, entry stake and first prize value should be shown as per CMS configuration
        """
        pass
