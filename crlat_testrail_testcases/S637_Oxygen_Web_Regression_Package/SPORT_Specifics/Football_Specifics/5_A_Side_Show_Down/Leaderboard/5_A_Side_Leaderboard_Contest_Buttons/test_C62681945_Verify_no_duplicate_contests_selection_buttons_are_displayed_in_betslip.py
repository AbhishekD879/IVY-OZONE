...
@vtest
class Test_C62681945_Verify_no_duplicate_contests_selection_buttons_are_displayed_in_betslip(Common):
    """
    TR_ID: C62681945
    NAME: Verify no duplicate contests selection buttons are displayed in betslip
    DESCRIPTION: 
    ...
    def test_005_check_if_any_contest_entry_buttons_are_duplicated_comparing_to_cms_contests_created(self):
        """
        DESCRIPTION: check if any contest entry buttons are duplicated comparing to CMS contests created
        EXPECTED: No contest entry button should be duplicated, there should be only one button for one contest created.
        """
        pass
