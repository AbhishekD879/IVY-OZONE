...
@vtest
class Test_C62755630_Verify_if_stats_are_consumed_during_grace_period_before_Extra_time_and_Penalties(Common):
    """
    TR_ID: C62755630
    NAME: Verify if stats are consumed during grace period,  before Extra time and Penalties.
    DESCRIPTION: Verify if stats are consumed by FE, during Extra time and Penalties.
    ...
    def test_007_validate_stats_are_consumed_by_fe_during_extra_time_and_penalties(self):
        """
        DESCRIPTION: Validate stats are consumed by FE, during Extra time and Penalties.
        EXPECTED: stats should be consumed by FE during grace period and before Extra time and Penalties
        EXPECTED: Note:- Any Updates should not consider after header is received.
        """
        pass
