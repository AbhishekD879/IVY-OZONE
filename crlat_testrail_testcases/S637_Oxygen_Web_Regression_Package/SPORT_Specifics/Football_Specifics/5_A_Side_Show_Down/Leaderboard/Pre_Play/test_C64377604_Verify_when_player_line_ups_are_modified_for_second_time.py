...
@vtest
class Test_C64377604_Verify_when_player_line_ups_are_modified_for_second_time(Common):
    """
    TR_ID: C64377604
    NAME: Verify when player line-ups are modified for second time
    DESCRIPTION: This test case verifies the unvoiding of bets, when we have line-ups for second time
    ...
    def test_002_modify_line_upsmodifying_line_ups_is_only_possible_in_test0_add_players_whose_bets_are_voided_previouslynote__line_ups_should_be_received_where_there_are_changes_with_player_dataexample_1st_line_upsabcdefghiuvxyz_player_bets_are_voided_in_the_above_case2nd_line_upsabcdefghiuvxyz(self):
        ...
        EXPECTED: Unvoiding process should be triggered for every line-ups which has player ID change.
        EXPECTED: Check the below strings in Kibana
        EXPECTED: "5AShowdown Un-Voided entrei EventId"
        EXPECTED: Will return the count of unvoided bets.
        EXPECTED: Note : Unvoiding process will only be triggered only when there are line-ups change.
        """
        pass

    def test_003_verify_the_current_entries_field_in_cms(self):
        """
        DESCRIPTION: Verify the current entries field in CMS
        EXPECTED: Current entries should match the entries in Pre-leaderboard.
        """
        pass
