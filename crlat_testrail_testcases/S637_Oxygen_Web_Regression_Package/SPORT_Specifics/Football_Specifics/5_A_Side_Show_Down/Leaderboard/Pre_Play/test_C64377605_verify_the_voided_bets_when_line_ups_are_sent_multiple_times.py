...
@vtest
class Test_C64377605_verify_the_voided_bets_when_line_ups_are_sent_multiple_times(Common):
    """
    TR_ID: C64377605
    NAME: verify the voided bets when line-ups are sent multiple times
    DESCRIPTION: This test case verifies the unvoiding of bets, when we have mutliple line-ups
    ...
    def test_002_modify_line_upsmodifying_line_ups_is_only_possible_in_test0_add_players_whose_bets_are_voided_previouslynote__line_ups_should_be_received_where_there_are_changes_with_player_dataexample_1st_line_upsabcdefghiuvxyz_player_bets_are_voided_in_the_above_case2nd_line_upsabcdefghiuvxyz(self):
        ...
        EXPECTED: When there is no change in line-ups, un-voiding process is not triggered
        EXPECTED: Check the below strings in Kibana
        EXPECTED: "5AShowdown Un-Voided entrei EventId"
        EXPECTED: Will return the count of un-voided bets.
        EXPECTED: Note : Un-voiding process will only be triggered only when there are line-ups change.
        """
        pass
