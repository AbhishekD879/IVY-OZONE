...
@vtest
class Test_C62700693_Verify_the_behaviour_of_invitation_contest_when_parallel_contests_with_different_events_are_running(Common):
    """
    TR_ID: C62700693
    NAME: Verify the behaviour of invitation contest when parallel contests with different events are running.
    DESCRIPTION: This test case verifies the behaviour of the contest when multiple events are running as the same time
    ...
    PRECONDITIONS: Set up parallel contests for 2 or more events including invitational contests as mentioned above.
    PRECONDITIONS: User should place bets on all the contests created for the same event ID & Different event ID.
    ...
    def test_002_2_click_on_5_a_side_leaderboard_button_on_my_bets_for_the_above_mentioned_event_bets_placing_mentioned_in_pre_conditions(self):
        """
        DESCRIPTION: 2. Click on '5-A Side leaderboard' button on My bets for the above mentioned event. (Bets placing mentioned in Pre-conditions)
        EXPECTED: 2. User will be navigated to Pre-leaderboard of that particular event.
        """
        pass
    ...
    def test_003_3_click_on_return_to_lobby_cta(self):
        ...
        EXPECTED: 3. User will be navigated back to lobby, with all the elligible showdown cards(contests) in lobby displayed according to the CMS priority.
        """
        pass

    def test_004_4_verify_the_contests_created_for_the_different_event_as_mention_in_pre_conditions(self):
        """
        DESCRIPTION: 4. Verify the contests created for the different event as mention in Pre-conditions
        EXPECTED: 4. As user placed bets on all the contests for the all events, all the contests are displayed in Lobby.
        """
        pass

    def test_005_5_navigate_to_each_contest_and_verify_all_the_leaderboards_in_prelivepost_states(self):
        """
        DESCRIPTION: 5. Navigate to each contest and verify all the leaderboards in Pre/Live/Post states.
