...
@pytest.mark.
@vtest
class Test_C60618878_Verify_Service_for_Getting_Player_Statistics(Common):
    """
    TR_ID: C60618878
    NAME: Verify Service for Getting Player Statistics
    DESCRIPTION: This test case verifies displaying of player statistics
    ...
    def test_003_send_via_postman_the_xml_where_servicename__playerstatisticsnote_put_5_digits_before_the_xml_text_and_only_after_that_tap_send_button(self):
        """
        DESCRIPTION: Send via Postman the XML where servicename = 'playerstatistics'
    ...
    def test_004_open_web_browser_and_tap_the_following_endpoint_for_checking_the_servicedomainapiplayeridstatisticsparam_playerid___id_of_playerall_params_are_taken_from_the_xml(self):
        """
        DESCRIPTION: Open web browser and tap the following endpoint for checking the service:
        DESCRIPTION: {domain}/api/player/:id/statistics
        DESCRIPTION: param :playerId - id of player
        DESCRIPTION: All params are taken from the XML
        EXPECTED: The endpoint returns player statistics
        """
        pass
