...
@pytest.mark.sports
@vtest
class Test_C2552940_Banach_Markets_with_Player_Lists_and_switcher_single_choice(Common):
    """
    TR_ID: C2552940
    NAME: Banach. Markets with Player Lists and switcher (single choice)
    DESCRIPTION: This test case verifies the display and selection rule of markets with player lists and switcher where single selection can be added to dashboard .
    ...
    PRECONDITIONS: **Scope of markets with Player lists with switcher where single selection can be added to dashboard. Select available for testing:**
    PRECONDITIONS: First Goalscorer, Player to Get First Booking
    ...
    def test_005_tap_on_second_selection_from_the_same_tab(self):
        """
        DESCRIPTION: Tap on second selection from the same tab
        EXPECTED: - Fist selection is deselected inside accordion and removed from the dashboard
        EXPECTED: - Second selection is selected inside accordion and is in the dashboard
        """
        pass

    def test_006_switch_to_away_team_tab_and_tap_on_a_selection(self):
        """
        DESCRIPTION: Switch to [Away team] tab and tap on a selection
        EXPECTED: - Selection from [Home team] tab is deselected inside accordion and removed from the dashboard
        EXPECTED: - Selection from [Away team] tab is selected inside accordion and is in the dashboard
        """
        pass
