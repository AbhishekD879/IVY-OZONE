...
@pytest.mark.other
@vtest
class Test_C64293347_Watching_a_IMG_Stream_with_incorrect_configuration(Common):
    """
    TR_ID: C64293347
    NAME: Watching a IMG Stream with incorrect configuration
    DESCRIPTION: User is  trying to watch a Stream when CMS configuration for that specific stream provider is incorrect.
    PRECONDITIONS: SiteServer event should be configured to support Perform streaming (**'typeFlagCodes'**='IVA , ... ' AND 'drilldownTagNames'='EVFLAG_IVM' flags should be set) and should be mapped to IMG stream event
    ...
    PRECONDITIONS: Change CMS configurations for IMG stream to incorrect values here: CMS -> System Configuration -> Structure -> 'IMGStreaming' ( **After OX103:*
    PRECONDITIONS: CMS -> Secrets -> 'IMGStreaming')
    PRECONDITIONS: Postconditions:
    PRECONDITIONS: All configurations should be set to DEFAULT again:
    PRECONDITIONS: **operatorId **= 26
    PRECONDITIONS: **IMGSecret **= UYhUc4Wep2
    """
