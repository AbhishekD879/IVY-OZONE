...
@pytest.mark.sports
@vtest
class Test_C59551272_Verify_display_of_betradar_scoreboard_for_logged_in_user(Common):
    """
    TR_ID: C59551272
    NAME: Verify display of betradar scoreboard for logged in user
    DESCRIPTION: Verify that when Betradar component is available for the Ice Hockey and when the event is In Play User is able to view the Scoreboard & Visualization.
    PRECONDITIONS: 1: Ice Hockey Event should be In-Play.
    PRECONDITIONS: 2: Inplay Ice Hockey should be subscribed to betradar scoreboard
    ...
    def test_003_click_on_ice_hockey_sport_from_a_z_menu(self):
        """
        DESCRIPTION: Click on Ice Hockey sport from A-Z menu
        EXPECTED: User should be able to view the Ice Hockey Event Landing Page.
        """
        pass
