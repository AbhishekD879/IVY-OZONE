...
@vtest
class Test_C58597206_Verify_restrictions_and_their_lifting_for_inactive_users_which_try_to_view_a_stream_end_to_end(Common):
    """
    TR_ID: C58597206
    ...
    PRECONDITIONS: SiteServer event should be configured to support ATR streaming (**'typeFlagCodes'**='AVA' AND **'drilldownTagNames'**='EVFLAG_AVA' flags should be set) and should be mapped to ATR stream events #1 and #2
    ...
    def test_002_mobile_or_tablet_responsivedesktoptapclick_on_watchlive_stream_button(self):
        ...
        EXPECTED: Warning message is shown (popup for mobile) with a following text : "This stream has not yet started. Please try again soon".
        EXPECTED: **Ladbrokes:**
        EXPECTED: [Mobile or Tablet Responsive]
    ...
    def test_003_wait_till_2_or_less_minutes_left_before_event_start_time_and_repeat_step_2(self):
        ...
        EXPECTED: [Mobile/Tablet Responsive & Desktop view]
        EXPECTED: **Coral:**
        EXPECTED: Warning message is shown (popup for mobile) with a following text : "In order to watch the race you must have a funded account or have placed a bet in the last 24 hours"
        EXPECTED: **Ladbrokes:**
        EXPECTED: [Mobile or Tablet Responsive]
