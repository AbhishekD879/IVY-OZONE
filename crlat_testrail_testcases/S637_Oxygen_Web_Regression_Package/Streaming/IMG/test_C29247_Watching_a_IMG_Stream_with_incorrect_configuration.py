...
@vtest
class Test_C29247_Watching_a_IMG_Stream_with_incorrect_configuration(Common):
    """
    TR_ID: C29247
    ...
    PRECONDITIONS: 1. SiteServer event should be configured to support Perform streaming (**'typeFlagCodes'**='IVA , ... ' AND **'drilldownTagNames'**='EVFLAG_IVM' flags should be set) and should be mapped to IMG stream event
    ...
    PRECONDITIONS: 4. Change CMS configurations for IMG stream to incorrect values here: CMS -> System Configuration -> Structure -> 'IMGStreaming' ( **After OX103:** CMS -> Secrets -> 'IMGStreaming')
