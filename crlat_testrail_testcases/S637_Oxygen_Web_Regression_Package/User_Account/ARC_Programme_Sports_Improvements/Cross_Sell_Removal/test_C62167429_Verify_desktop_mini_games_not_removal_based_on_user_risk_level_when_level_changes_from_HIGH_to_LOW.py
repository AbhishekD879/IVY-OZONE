...
@vtest
class Test_C62167429_Verify_desktop_mini_games_not_removal_based_on_user_risk_level_when_level_changes_from_HIGH_to_LOW(Common):
    """
    TR_ID: C62167429
    NAME: Verify desktop mini games not removal based on user risk level when level changes from HIGH to LOW
    ...
    def test_003_verify_desktop_minigames_are_not_appear(self):
        """
        DESCRIPTION: Verify desktop minigames are not appear
        EXPECTED: Mini games removed for all sports
        """
        pass

    def test_004_go_to_cms_and_login_as_content_user_and_configure_model_risk_moh_fields_as_problem_gamblerrisk_level_changes_from_high_to_low_tbd_respectivley(self):
        """
        DESCRIPTION: Go to CMS and login as Content User and configure Model &Risk &MOH fields as Problem Gambler,Risk Level changes from HIGH to LOW ,TBD respectivley
        EXPECTED: CMS configuration done with all changes.
        """
        pass
    ...
    def test_006_login_again_into_oxygen_application_as_low_risk_user__gtgt_check_minigames_are_removed_for_all_sports_on_new_session(self):
        """
        DESCRIPTION: Login again into Oxygen application as Low risk user -&gt;&gt; check minigames are removed for all Sports on new session
        EXPECTED: Minigames NOT removed for all sports
        """
        pass
