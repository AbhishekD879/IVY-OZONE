...
@vtest
class Test_C62843081_Verify_Quickbet_removed_based_on_input_Model_as_Problem_Gamblers_and_Risk_Level_as_VERY_HIGH_and_MoH_as_TBC_after_login(Common):
    """
    TR_ID: C62843081
    NAME: Verify Quickbet removed based on input Model as Problem Gamblers and Risk Level as VERY HIGH and MoH as TBC after login
    DESCRIPTION: The test case verifies the quickbet feature get removed for risk level users
    PRECONDITIONS: Pre-requisite
    PRECONDITIONS: ---------------------------------
    PRECONDITIONS: Configure in CMS like input Model as Problem Gamblers and Risk Level as V.High AND MoH as TBC
    """
    keep_browser_open = True

    def test_001_login_into_oxygen_application_as_vhigh_risk_user__add_one_selection(self):
        """
        DESCRIPTION: Login into Oxygen Application as V.HIGH risk User-> add one selection
