...
@vtest
class Test_C63770682_Verify_User_redirects_to_BetSlip_after_login_from_QuickBet_screen_based_on_user_risk_levelMoH_AT_RISK(Common):
    """
    TR_ID: C63770682
    NAME: Verify User redirects to BetSlip after login from QuickBet screen based on user risk level,MoH_AT RISK
    ...
    PRECONDITIONS: Medium :
    PRECONDITIONS: Configure in CMS like input Model as AT RISK and Risk Level as MEDIUM & MoH as TBC
    PRECONDITIONS: High :
    PRECONDITIONS: Configure in CMS like input Model as AT RISK  and Risk Level as HIGH &  MoH as TBC
    """
    ...
    def test_004_now_try_to_login_from_quickbet_screen_as_medium_risk_user_and_check_that_user_redirected_to_betslipwith_pre_filled_stake_entered(self):
        """
        DESCRIPTION: Now, Try to Login from Quickbet screen as Medium Risk User and check that user redirected to Betslip
        DESCRIPTION: with pre filled stake entered
    ...
    def test_008_check_for_high_risk_levels(self):
        """
        DESCRIPTION: Check for HIGH risk levels
        EXPECTED: Should be Same behaviour for High  risk level users
        """
        pass
