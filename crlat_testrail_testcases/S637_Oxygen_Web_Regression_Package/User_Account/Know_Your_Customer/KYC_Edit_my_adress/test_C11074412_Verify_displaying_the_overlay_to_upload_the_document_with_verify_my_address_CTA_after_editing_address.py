...
@vtest
class Test_C11074412_Verify_displaying_the_overlay_to_upload_the_document_with_verify_my_address_CTA_after_editing_address(Common):
    """
    TR_ID: C11074412
    NAME: Verify displaying the overlay to upload the document with verify my address CTA after editing address
    DESCRIPTION: This test case verifies displaying the overlay to upload the document with verify my address CTA after editing address
    ...
    PRECONDITIONS: - User with **IMS Age verification status** = *Active grace period* AND **Player tag** = *'AGP_Success_Upload < 5'* has edited his address inside **'Review my details'** pop up
    PRECONDITIONS: - The number of edits for the address is less than 3('Review my details' link is available)
    """
    keep_browser_open = True

    def test_001_before_a_user_clicks_update_address_button_on_review_details_pop_upin_ims__find_a_just_registered_user__add_player_tags_poa_required__tap_update_info(self):
        """
        DESCRIPTION: Before a user clicks "Update address" button on Review details pop up:
        DESCRIPTION: In IMS:
        DESCRIPTION: - Find a just registered user
        DESCRIPTION: - Add Player tags “POA required”
        DESCRIPTION: - Tap 'Update Info'
        EXPECTED: - Changes are saved in IMS
        EXPECTED: - Player tags “POA required” is added
        """
        pass
    ...
    def test_003_after_a_user_clicks_update_address_button_on_review_details_pop_upin_app_devtools__verify_response_in_openapi_websocket(self):
        ...
        EXPECTED: - 'ageVerificationStatus' = **"inprocess"** (in response with "ID":31083)  AND "Player tag"= "POA_required" is received (in response with "ID":35548)
        """
        pass

    def test_004_verify_displaying_the_overlay_to_verify_the_address_and_success_screen_with_verify_my_address_cta(self):
        """
        DESCRIPTION: Verify displaying the overlay to verify the address and success screen **with** verify my Address CTA
        ...
        EXPECTED: - Utility bill/Bank statement ->
        ...
        EXPECTED: -Green button **"Verify my address"** OR "Review my Details" link are present
