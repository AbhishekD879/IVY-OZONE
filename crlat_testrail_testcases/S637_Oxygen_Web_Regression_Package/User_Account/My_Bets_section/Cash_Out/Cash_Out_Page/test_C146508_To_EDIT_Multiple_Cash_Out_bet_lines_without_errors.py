...
@vtest
class Test_C146508_To_EDIT_Multiple_Cash_Out_bet_lines_without_errors(Common):
    """
    TR_ID: C146508
    NAME: [To EDIT] Multiple Cash Out bet lines without errors
    DESCRIPTION: This test case verifies multiple bet lines without errors on 'Cash Out' tab
    ...
    PRECONDITIONS: * User has placed multiple bets on Pre Match or In-Play events, special events and on events with eventSortCode not a match (e.g.  outright) where Cash Out offer is available
    ...
    def test_002_verify_cash_out_tab(self):
        ...
        EXPECTED: *   Accumulator bets are loaded
        EXPECTED: *   Bet section(s) - as many as bets are available
        """
        pass
    ...
    def test_005_verify_bet_section_content__for_multiple_bet(self):
        ...
        EXPECTED: *  **FOR** **99** **release**: Odds with the relevant price of the selection is displayed on the same line with
        EXPECTED: the Selection Name next to it (using such format @1/2).
    ...
    def test_006_verify_start_timestart_date_for_pre_match_events(self):
        ...
        EXPECTED: * If the event start time is today (based on user's time zone) then the time is displayed in format:
        EXPECTED: **Today - HH:MM, Today (e.g. "14:00 or 05:00, Today")
        EXPECTED: * If the event start time is tomorrow or further in the future (based on user's time zone) then the date and time are displayed in a 24 hours format:
    ...
    def test_014_swipe_left_to_see_next_leg(self):
        """
        DESCRIPTION: Swipe left to see next leg
        EXPECTED: * First leg of multiple bet is not displayed
        EXPECTED: * Second leg of multiple bet is fully displayed with corresponding bet details
        EXPECTED: * Third leg of multiple bet is slightly displayed from the right side of second bet
        EXPECTED: * Second navigation dot is highlighted with different color
        """
        pass

    def test_015_swipe_right_to_see_previous_leg(self):
        """
        DESCRIPTION: Swipe right to see previous leg
        EXPECTED: * First leg of multiple bet is fully displayed with corresponding bet details
        EXPECTED: * Second leg of multiple bet is slightly displayed from the right side of first bet
        EXPECTED: * First navigation dot is highlighted with different color
        """
        pass

    def test_016_swipe_right_again_to_see_last_leg_in_bet(self):
        """
        DESCRIPTION: Swipe right again to see last leg in bet
        EXPECTED: * Last leg of multiple bet is fully displayed with corresponding bet details
        EXPECTED: * First leg of multiple bet is slightly displayed from the right side of first bet
        EXPECTED: * Last navigation dot is highlighted with different color
        """
        pass

    def test_017_swipe_left_to_see_first_leg(self):
        """
        DESCRIPTION: Swipe left to see first leg
        EXPECTED: * Last leg of multiple bet is not displayed
        EXPECTED: * First leg of multiple bet is fully displayed with corresponding bet details
        EXPECTED: * Second leg of multiple bet is slightly displayed from the right side of first bet
        EXPECTED: * First navigation dot is highlighted with different color
        """
    ...
    def test_018_click_on_any_not_selected_dot(self):
        """
        DESCRIPTION: Click on any not selected dot
        EXPECTED: * Corresponding leg of multiple bet is fully displayed with corresponding bet details
        EXPECTED: * Clicked dot is shown as selected
        """
        pass
