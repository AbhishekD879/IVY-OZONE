...
@vtest
class Test_C62736549_Verify_ZMC_is_applicable_for_Multiple_bets_on_Racing_Preplay_events__SPLP(Common):
    """
    TR_ID: C62736549
    NAME: Verify ZMC is applicable for Multiple bets on Racing  Preplay events - SP,LP
    DESCRIPTION: This test case verifies Zero margin cash Out is applicable for full cash out with Multiple bets on Racing(Horse Racing and Grey Hounds) with prices LP and SP of Preplay events when there are no price changes in Cash Out tab
    ...
    def test_001_add_several_selections_from_different_hr_events_to_the_betslip_with_prices_as_sp_and_lp_for_which_cash_out_is_available(self):
        """
        DESCRIPTION: Add several selections from different HR events to the betslip with prices as SP and LP for which cash out is available
        EXPECTED: 
        """
        pass
    ...
    def test_012_repeat_steps_1_11_for_greyhounds_only_with_lp_prices(self):
        """
        DESCRIPTION: Repeat steps 1-11 for Greyhounds (Only with LP prices)
        EXPECTED: 
        """
        pass
