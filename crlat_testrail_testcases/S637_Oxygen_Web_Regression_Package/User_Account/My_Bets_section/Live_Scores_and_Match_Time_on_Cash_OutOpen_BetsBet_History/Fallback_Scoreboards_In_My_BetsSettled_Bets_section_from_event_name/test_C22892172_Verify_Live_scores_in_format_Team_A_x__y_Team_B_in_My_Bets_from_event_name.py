...
@vtest
class Test_C22892172_Verify_Live_scores_in_format_Team_A_x__y_Team_B_in_My_Bets_from_event_name(Common):
    """
    TR_ID: C22892172
    NAME: Verify Live scores in format 'Team A <x>  - <y> Team B'  in 'My Bets'  (from event name)
    ...
    DESCRIPTION: Sports related to this template: Football , Rugby Union, Rugby League, Basketball, American Football, Baseball, Ice Hockey, Esports, Snooker
    ...
    PRECONDITIONS: NOTE: in order to update event live scores, go to TI (backoffice) and update score in event name, that is in format: Team A <x> vs <y> Team B (eg. Team/Player A	2-1	Team/Player B)
    PRECONDITIONS: Testing Fallback: https://confluence.egalacoral.com/pages/viewpage.action?spaceKey=SPI&title=Testing+Fallback
    ...
    def test_002_in_ti_update_the_score_in_event_name_for_team_ateam_b_save_changesin_app_check_the_scores_are_updated(self):
        ...
        EXPECTED: - Scores are in format: Team A <x> vs <y> Team B (eg. Team/Player A 2-1 Team/Player B)
        """
        pass
