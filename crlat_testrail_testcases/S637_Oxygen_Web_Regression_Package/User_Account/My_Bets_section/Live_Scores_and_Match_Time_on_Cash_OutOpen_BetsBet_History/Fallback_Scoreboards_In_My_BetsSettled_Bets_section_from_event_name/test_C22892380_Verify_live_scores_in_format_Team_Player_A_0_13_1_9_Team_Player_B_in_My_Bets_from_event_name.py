...
@vtest
class Test_C22892380_Verify_live_scores_in_format_Team_Player_A_0_13_1_9_Team_Player_B_in_My_Bets_from_event_name(Common):
    """
    TR_ID: C22892380
    NAME: Verify live scores in format Team/Player A| 0-13-1-9 |Team/Player B in My Bets (from event name)
    DESCRIPTION: This test case verifies live scores displaying for Gaelic Football in My Bets section (scores updates coming from event name in TI)
    ...
    PRECONDITIONS: - User has placed single/multiple bets on live garlic football event (with cashout available)
    ...
    PRECONDITIONS: Doc with all templates for scores: https://confluence.egalacoral.com/pages/viewpage.action?spaceKey=SPI&title=Testing+Fallback
    """
    ...
    def test_002_in_ti_update_the_score_in_event_name_for_team_ateam_b_save_changesin_app_check_the_scores_are_updated(self):
        ...
        EXPECTED: - Scores are in format: |Team/Player A| 0-13-1-9 |Team/Player B|
        """
        pass
