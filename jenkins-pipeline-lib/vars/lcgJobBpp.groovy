...
def call(Map arguments = [:]) {
    ...
    // Service to build
    def service = lcgCommonFunctions.getConstantsParameters("services.serviceBetPlacement")
            ...
            if (branchName == "develop") {
                jobs.put("Deploy to Coral dev0", {
                    timeout(time: deploymentTimeout, unit: 'SECONDS') {
                        build job: "BPP-DEV0-FRAMEWORK-Deployment",
                                parameters: [
                ...
                jobs.put("Deploy to Ladbrokes dev0", {
                    timeout(time: deploymentTimeout, unit: 'SECONDS') {
                        build job: "BPP-DEV0-LADBROKES-Deployment",
                                parameters: [
                                        string(name: "Commit_oxygen_playbook", value: "master"),
