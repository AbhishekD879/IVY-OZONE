...
@pytest.mark.tst2
@pytest.mark.stg2
# @pytest.mark.prod #cannot suspend an event in prod
@pytest.mark.medium
@pytest.mark.cash_out
@pytest.mark.desktop
@vtest
class Test_C58212454_Verify_Cashout_MS_update_before_active_connection_with_price_1_100(BaseBetSlipTest):
    """
    TR_ID: C58212454
    ...
    def test_000_preconditions(self):
        ...
        event1 = self.ob_config.add_UK_racing_event(number_of_runners=1, lp_prices=self.prices, cashout=True)
        self.__class__.event_name1 = event1[6]['event']['name']
        self.__class__.selection_name1, selection_id1 = list(event1.selection_ids.items())[0]
        self.__class__.eventID1 = event1.event_id
        event2 = self.ob_config.add_UK_racing_event(number_of_runners=1, lp_prices=self.prices, cashout=True)
        self.__class__.event_name2 = event2[6]['event']['name']
        ...
        event3 = self.ob_config.add_UK_racing_event(number_of_runners=1, lp_prices=self.prices, cashout=True)
    ...
    def test_004_repeat_steps_above_for_the_multiple_bet(self):
        ...
        bet_id = result.split("\"bets\":[{")[1].split(",")[0].replace("\"", "").split(":")[1]
        self.ob_config.change_event_state(event_id=self.eventID2, displayed=True, active=True)
        sleep(3)
        self.cashout_bet_update_status(bet_id=bet_id, bet_type='DOUBLE', eventname=self.event_name2)
        for bet_leg_name, bet_leg in self.bet.items_as_ordered_dict.items():
