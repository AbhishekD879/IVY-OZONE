...
@pytest.mark.login
@vtest
class Test_C237127_Lost_Status_of_selection(BaseCashOutTest):
    """
    TR_ID: C237127
    NAME: Lost status of selection
    DESCRIPTION: This test case verifies displaying 'Lost' status on the Cash Out page
    """
    ...
    def test_001_navigate_to_cash_out_tab(self):
        ...
        status = self.get_bet_status_from_cashout(bet_type=vec.bet_history.MY_BETS_SINGLE_STAKE_TITLE,
    ...
    def test_002_in_ob_backoffice_set_lost_result_for_selection_of_event_with_placed_single_bet(self):
        """
        DESCRIPTION: In OB Backoffice set **Lost** result settle for selection of event
        """
    ...
    def test_003_check_badge_for_single_bet(self):
        ...
        EXPECTED: Bet is shown with status badge of "Lost" for a few seconds and after it bet disappears from the page
        """
    ...
    def test_004_check_badge_for_multiple_bet(self):
        ...
        EXPECTED: Bet is shown with status badge of "Lost" for a few seconds and after it bet disappears from the page
        """
        # we cannot verify badge status for multiple bet because it disappears too fast
        _, bet = self.site.cashout.tab_content.accordions_list.get_bet(event_names=self.event1_name,
                                                                       bet_type='DOUBLE',
                                                                       raise_exceptions=False,
                                                                       number_of_bets=3)
