...
@pytest.mark.stg2
@pytest.mark.desktop
# @pytest.mark.prod - invloves events creation
@pytest.mark.high
@pytest.mark.betslip
@pytest.mark.acca
@vtest
class Test_C9346553_Verify_the_flow_when_the_game_result_WIN(BaseBetSlipTest):
    """
    TR_ID: C9346553
    ...
    def test_001_go_to_ti_and_set_win_result_for_any_event_in_the_placed_betverify_that_selection_removal_button_is_not_shown_for_resulted_event(self, open_bets=True):
        """
        ...
        self.assertTrue(selection.icon.is_displayed(),
                        msg=f'Win label "{vec.betslip.CANCELLED_STAKE.title()}" is not displayed')
    ...
    def test_002_tap_selection_removal_button_for_any_other_selection_in_the_betverify_that_confirm_button_is_clickable_and_undo_button_is_shown_for_removed_selection(self):
        ...
        if self.brand == 'bma':
            self.bet.edit_my_acca_button.click()
            self.test_001_go_to_ti_and_set_win_result_for_any_event_in_the_placed_betverify_that_selection_removal_button_is_not_shown_for_resulted_event(
                open_bets=False)
