...
@pytest.mark.login
@vtest
class Test_C119663_Verify_Order_of_bet_lines_for_Regular_bets_Different_Start_Time(BaseBetSlipTest):
    """
    TR_ID: C119663
    VOL_ID: C9697643
    NAME: Verify Order of bet lines for Regular bets with different start time
    ...
    def test_001_login_create_event_and_place_bets(self):
        ...
        for i in range(0, self.num_of_events):
            start_time = self.get_date_time_formatted_string(hours=i)
            event_params = self.ob_config.add_autotest_premier_league_football_event(start_time=start_time)
            self.__class__.selections[event_params.team1] = event_params.selection_ids[event_params.team1]
        self.site.login(username=tests.settings.betplacement_user)
        selection_ids = list(self.selections.values())
        random.shuffle(selection_ids)
        self.open_betslip_with_selections(selection_ids=selection_ids)
        self.place_and_validate_single_bet(number_of_stakes=self.num_of_events)
    ...
    def test_005_verify_oder_of_bets_with_the_same_bet_placement_time(self):
        ...
        # Bet lines are ordered by Event Start Time (with the earliest start time first)
        self.assertListEqual(actual_selection_names, list(self.selections.keys()))
