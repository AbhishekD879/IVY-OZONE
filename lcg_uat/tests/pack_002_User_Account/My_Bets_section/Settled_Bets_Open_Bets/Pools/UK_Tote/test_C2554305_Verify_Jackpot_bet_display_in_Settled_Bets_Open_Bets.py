...
@pytest.mark.login
@vtest
class Test_C2554305_Verify_Jackpot_bet_is_displayed_on_Open_Bets_tab(BaseUKTote, BaseBetSlipTest):
    """
    TR_ID: C2554305
    NAME: Verify Jackpot bet is displayed on Open Bets tab
    DESCRIPTION: This test case verifies the displayed contents of a Jackpot tote bet in the Settled Bets/Open Bets
    ...
    expected_market_name = 'Jackpot'
    bet_history = vec.bet_history.SETTLED_BETS_TAB_NAME
    open_bets = vec.bet_history.OPEN_BETS_TAB_NAME

    def verify_jackpot_tote_bet_information(self, tab_name):
        """
        This method verifies whether Jackpot tote bet information corresponds to placed bet:
        :param tab_name - Settled Bets / Open Bets tab names
        * Jackpot title
        ...
        * Estimated returns value is N/A
        ...
        if tab_name == self.open_bets:
            ...
            bet_name, self.__class__.tote_pool = self.site.bet_history.tab_content.accordions_list.get_bet(
                bet_type='TOTEPOOL', event_names=self.updated_event_names, number_of_bets=1)
    ...
    def test_000_preconditions(self):
        ...
        jackpot_bet_results = self.place_multiple_legs_bet(event_id=self.eventID,
                                                           tab_name=vec.uk_tote.UK_TOTE_TABS.jackpot,
                                                           unit_stake=self.bet_amount)
        self.__class__.selected_outcomes = jackpot_bet_results.get('selected_outcomes')
        self.__class__.races_titles = jackpot_bet_results.get('races_titles')
    ...
    def test_002_tap_on_the_pools_sub_section(self):
        ...
        new_selection_outcomes = list(map(lambda x: re.sub(r"(\d)+(:) ", lambda str: str.group(1) + " ", x), self.selection_outcomes))
        ...
        self.verify_jackpot_tote_bet_information(tab_name=self.open_bets)
