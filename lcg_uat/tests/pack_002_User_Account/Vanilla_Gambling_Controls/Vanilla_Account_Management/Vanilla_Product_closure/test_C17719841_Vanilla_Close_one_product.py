...
@vtest
class Test_C17719841_Vanilla_Close_one_product(BaseCashOutTest, BaseUserAccountTest):
    """
    TR_ID: C17719841
    ...
    ACCOUNT_CLOSURE_SECTIONS = ['Bingo', 'Casino', 'Poker']
    ...
    def test_001_click_the_close_button_of_one_of_the_open_products_eg_sports(self):
        ...
        self.assertEqual(selected_product, vec.SB.LOBBY, msg=f'User isn\'t taken to the page with the detailed information, selected product is "{selected_product}"')
    ...
    def test_005_click_the_continue_button(self):
        ...
        self.assertEquals(expected_btn_text.upper(), actual_close_btn_text, msg=f'Actual button name "{actual_close_btn_text}" is not matching with exepected button name "{expected_btn_text}"')
    ...
    def test_006_click_the_close_productname_button(self):
        ...
        self.assertTrue(self.site.service_closure.close_all_button.is_displayed(), msg=f'There is no a "CLOSE ALL" button')
        self.assertListEqual(sorted(list(filter(None, self.site.service_closure.items_as_ordered_dict.keys()))), sorted(self.ACCOUNT_CLOSURE_SECTIONS),
                             msg=f'list of open product in UI "{list(filter(None, self.site.service_closure.items_as_ordered_dict.keys()))}" is not matching with expected product "{self.ACCOUNT_CLOSURE_SECTIONS}"')
