...
@pytest.mark.sports
@vtest
class Test_C15392945_Verify_Forecast_Bet_receipt_for_Horse_racing_Greyhounds(BaseRacing, BaseBetSlipTest):
    """
    TR_ID: C15392945
    ...
    def placing_forecast_bet(self, event_ids, combination_bet=False):

        self.navigate_to_edp(event_id=event_ids, sport_name='horse-racing')
        ...
        if combination_bet:
            self.assertIn(vec.betslip.COMBINATION_FORECAST, self.stake.market_name,
                          msg=f'Market name "{vec.betslip.COMBINATION_FORECAST}" '
    ...
    def test_000_preconditions(self):
        ...
        if tests.settings.backend_env == 'prod':
            ...
            start_time_local = self.convert_time_to_local(date_time_str=event2['event']['startTime'],
                                                          ob_format_pattern=self.ob_format_pattern,
                                                          future_datetime_format=self.my_bets_event_future_time_format_pattern,
                                                          ss_data=True)
            self.__class__.event_name2 = f'{event2["event"]["name"]} {start_time_local}'
            self.__class__.event_id2 = event2['event']['id']
            self._logger.info(
                f'*** Found Horse Racing Forecast/Tricast event "{self.event_name2}" with id "{self.event_id2}"')
            market_name, outcomes = next(
                ((market['market']['name'], market['market']['children']) for market in event2['event']['children']
                 if market['market'].get('children')), None)
            if outcomes is None:
                raise SiteServeException('There are no available outcomes')
            self.__class__.selection_ids = {i['outcome']['name']: i['outcome']['id'] for i in outcomes}
            self.__class__.selection_name, self.__class__.selection_id = list(self.selection_ids.items())[1]
            self.__class__.selection_id2 = list(self.selection_ids.values())[1]

        else:
            event1 = self.ob_config.add_UK_racing_event(number_of_runners=5,
                                                        forecast_available=True,
                                                        tricast_available=True)
            event_start_time = event1.event_date_time
            ...
            self.__class__.event_id = event1.event_id

            event2 = self.ob_config.add_UK_racing_event(number_of_runners=5,
                                                        forecast_available=True,
                                                        tricast_available=True)
            event_start_time = event2.event_date_time
            start_time_local = self.convert_time_to_local(date_time_str=event_start_time)
            self.__class__.event_name = f'{self.horseracing_autotest_uk_name_pattern} {start_time_local}'
            self.__class__.event_id2 = event2.event_id

        self.site.login()
        self.placing_forecast_bet(event_ids=self.event_id)
    ...
    def test_004_place_a_combination_forecast_betverify_bet_receipt_card_in_the_betslip(self):
        ...
        self.placing_forecast_bet(event_ids=self.event_id, combination_bet=True)
        self.test_003_verify_forecast_bet_receiptindexphpattachmentsget31337()
    ...
    def test_005_place_multiple_single_bets_and_check_the_bet_receipt(self):
        ...
        self.placing_forecast_bet(event_ids=self.event_id2)
        self.test_003_verify_forecast_bet_receiptindexphpattachmentsget31337()
    ...
    def test_010_click_on_reuse_selection_button(self):
        ...
        self.assertEqual(stake.market_name, vec.betslip.FORECAST,
                         msg=f'Actual Market name "{stake.market_name}" is not the same as expected "{vec.betslip.FORECAST}"')
