...
@pytest.mark.sports
@vtest
class Test_C15785354_Verify_Forecast_and_Tricast_Bet_receipt_using_free_bets_for_Horse_Racing(BaseRacing, BaseBetSlipTest):
    """
    TR_ID: C15785354
    ...
    def place_forecast_tricast_bet(self, multiple=False, forecast=True, tricast=False):
        ...
        expected_selection_name = self.place_forecast_tricast_bet_from_event_details_page(forecast=forecast, tricast=tricast)
        if multiple:
            self.ob_config.grant_freebet(username=self.username)
            self.navigate_to_edp(event_id=self.event1.event_id, sport_name='horse-racing')
            expected_selection_name = self.place_forecast_tricast_bet_from_event_details_page(forecast=forecast, tricast=tricast)
    ...
    def test_000_preconditions(self):
        self.__class__.event = self.ob_config.add_UK_racing_event(forecast_available=True,
                                                                  tricast_available=True)
        self.__class__.event1 = self.ob_config.add_UK_racing_event(forecast_available=True,
                                                                   tricast_available=True)
