...
@pytest.mark.desktop
@vtest
class Test_C66100490_Verify_the_GA_Tracking_when_user_Clicks_scrolls_of_Events_matches_on_right_side(BaseDataLayerTest):
    """
    TR_ID: C66100490
    NAME: Verify the GA Tracking when user Clicks/scrolls of Events/matches on right side
    DESCRIPTION: This test case is to verify the GA Tracking when user Clicks/scrolls of Events/matches on right side
    ...
    def test_003_verify_the_display_of_byb_widget_on_the_byb_homepage(self):
        ...
        for byb_widget_container_name, byb_widget_container in byb_widget_containers.items():
            ...
            for i, (byb_widget_name, byb_widget) in enumerate(list(byb_widgets.items())[::-1]):
                if self.device_type == 'desktop':
                    if i % 2 == 0:
                        previous_arrow_status = self.site.home.byb_widget.has_arrow_prev()
                        self.assertTrue(previous_arrow_status, msg='previous Arrow is not display')
                        self.site.home.byb_widget.arrow_prev()
                        # need to add small sleep to make test run more stable after click with java script
                    self.device.driver.implicitly_wait(1)
                else:
                    byb_widget.scroll_to()
                    self.assertTrue(byb_widget.is_displayed(),
                                    msg=f'Next race: "{byb_widget_name}" not displayed after scrolling to')
                    if 1 < i < (len(byb_widgets.items()) - 2):
                        next_byb_widget_name, next_byb_widget = list(byb_widgets.items())[::-1][i - 3]
                        self.assertFalse(next_byb_widget.is_displayed(expected_result=False, scroll_to=False),
                                         msg=f'Next race: "{next_byb_widget_name}" still displayed after scrolling to previous')

            self.device.driver.implicitly_wait(1)

    def test_004_for_desktop_click_on_the_eventsmatches_on_the_right_side(self):
        """
        DESCRIPTION: For Desktop: Click on the Events/matches on the right side
        EXPECTED: User can able to click on the Event/Match on the right side
        """
        # covered in above step

    def test_005_for_mobile_scroll_the_eventsmatches_on_the_right_side(self):
        """
        DESCRIPTION: For Mobile: Scroll the Events/matches on the right side
        EXPECTED: User can able sroll the Events/matches on the right side
        """
        # covred in above step
