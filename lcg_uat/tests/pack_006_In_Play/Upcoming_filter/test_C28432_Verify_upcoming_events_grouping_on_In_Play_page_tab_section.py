...
class Test_C28432_Verify_upcoming_events_grouping_on_In_Play_page_tab_section(Common):
    ...
    def verify_sport_existence(self):
        ...
        sports = get_inplay_ls_structure()['upcomingLiveStream']['eventsBySports']
        no_of_sports = 2 if len(sports) > 2 else len(sports)
        for i in range(no_of_sports):
            self.total_sports.append(sports[i]['categoryCode'].replace('_', ' '))

    def verify_event_filtering(self, watchlive=True):
        if watchlive:
            inplay_module = get_inplay_sports_by_section(type='UPCOMING_STREAM_EVENT')
        else:
            inplay_module = get_inplay_sports_by_section(type='UPCOMING_EVENT')
        if not inplay_module:
            raise PreconditionNotMetException("No inplay events available in any sport")
        ...
        self.inplay_sports = {k: v for k, v in sorted(self.inplay_sports.items(), key=lambda item: item[1])}
        ...
        if self.device_type in ['mobile', 'tablet']:
            ...
            for i in range(no_of_sports):
                ...
                for item in list(expected_sport_type.keys()):
                    self.assertIn(item, actual_sport_type, msg=f'Expected sport type: "{item}" is not in the '
            ...
            self.assertListEqual(actual_sport_group, list(self.inplay_sports.keys()), msg=f'Actual sports list:"{actual_sport_group}" is not equal to'
            ...
            for item_name, item in expected_sport_type.items():
                ...
                index = index + 1
            expected_events_under_type.clear()
            self.assertListEqual(list(inplay_list_upcoming.keys()), self.total_sports, msg='Not equal')
    ...
    def sport_and_inplay(self):
        ...
        expected_sport_type = self.verify_event_filtering(watchlive=False)
        for item in expected_sport_type:
            self.assertIn(item, actual_sport_type,
                          msg=f'Expected sport type: "{item}" is not in the actual sport type:"{actual_sport_type}"'
                              'Events are not grouped by typeID within <Type> expanded accordions/odds headers')
    ...
    def test_003_repeat_steps_1_2_onhome_page__in_play_tab_mobiletablet(self):
        """
    ...
    def test_004_repeat_step_2_onsports_landing_page__in_play_tabin_play_page__sport_tab(self):
        """
