...
@vtest
class Test_C2855551_Verify_Smart_Boost_market_on_sports_EDP(BaseSportTest):
    """
    TR_ID: C2855551
    ...
    def test_001_verify_the_market_presence_on_edp(self):
        ...
        if vec.siteserve.EXPECTED_MARKET_TABS.all_markets in markets_tabs.keys():
            ...
            selected_filter = self.site.sport_event_details.tab_content.timeline_filters.selected_filter.name
            if selected_filter!=vec.siteserve.EXPECTED_MARKET_TABS.all_markets.title():
                self.site.sport_event_details.tab_content.timeline_filters.items_as_ordered_dict.get(vec.siteserve.EXPECTED_MARKET_TABS.all_markets.title()).click()
        markets_sections = wait_for_result(
            lambda: self.site.sport_event_details.tab_content.accordions_list.items_as_ordered_dict, timeout=3)
    ...
    def test_002_verify_the_view_of_selection_with_changed_price(self):
        ...
        if not self.market.is_expanded():
            self.market.expand()
