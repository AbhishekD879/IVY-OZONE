...
@pytest.mark.login
@vtest
class Test_C147946_Multiple_Non_Cash_Out_bet_lines_without_errors(BaseCashOutTest, BaseRacing, BaseSportTest):
    """
    TR_ID: C147946
    VOL_ID: C9698354
    NAME: Multiple Non Cash Out bet lines without errors
    """
    keep_browser_open = True
    currency = '£'
    my_bets_tab_name = 'My Bets'
    lp_price = None
    ...
    def test_000_preconditions(self):
        ...
        """
        self.__class__.sport_event_info = self.ob_config.add_baseball_event_to_autotest_league(cashout=False)
        ...
        self.__class__.lp_price = self.ob_config.event.prices['odds_draw']
        self.__class__.racing_event_info = self.ob_config.add_UK_racing_event(cashout=False, number_of_runners=1,
                                                                              ew_terms=self.ew_terms,
                                                                              lp_prices={0: self.lp_price})
    ...
    def test_002_place_bet_for_events(self):
        ...
        self.add_selection_from_event_details_to_quick_bet(selection_name=self.sport_event_info.team1,
                                                           market_name=self.expected_market_sections.money_line)
    ...
    def test_004_verify_my_bets_tab(self):
        ...
        EXPECTED: Separate section for each bet is present and includes following:
                 - Bet Section header
                 - Bet Section content with bet lines details
        """
        ...
        for bet_section_name, bet_section in self.bet_sections.items():
            self.assertTrue(bet_section.has_header(), msg='Bet: "%s" section header not found' % bet_section_name)
    ...
    def test_005_verify_sport_bet_section_content(self):
        ...
        self.assertTrue(self.bet_legs, msg='No one bet leg found for bet: "%s"' % self.sport_event_name)
        self.__class__.sport_bet_leg_name = '%s - %s' % (self.sport_event_info.team1, self.sport_event_name)
        ...
        expected_market_name = self.ob_config.backend.ti.baseball.baseball_autotest.autotest_league.market_name.replace('|', '')
        ...
        self.assertEqual(current_tab_name, 'My Bets',
    ...
    def test_006_verify_racing_bet_section_content(self):
        ...
        result = wait_for_result(lambda: self.racing_bet_leg.odds_value == self.lp_price,
                                 name='Odds value to appear and become equal to expected value',
                                 timeout=3)
        self.assertTrue(result, msg='Actual odds value: "%s" does not match with expected: "%s"'
                                    % (self.racing_bet_leg.odds_value, self.lp_price))
    ...
    def test_007_verify_bet_receipt_section_and_buttons_panel(self):
        ...
        EXPECTED: Button with label "CASH OUT: <currency symbol><amount>" not displayed under line with 'Stake' and 'Est. Returns'
        EXPECTED: Button with label "PARTIAL CASHOUT" not displayed under line with 'Stake' and 'Est. Returns'
        """
        ...
        expected_est_returns = '%s%s' % (self.currency, self.bet_info['Double']['estimate_returns'])
        ...
        self.assertFalse(self.bet.buttons_panel.has_full_cashout_button(expected_result=False),
                         msg='"CASHOUT" button was found on bet: "%s" section' % self.bet_name)
        self.assertFalse(self.bet.buttons_panel.has_partial_cashout_button(expected_result=False),
                         msg='"PARTIAL CASHOUT" button was found on bet: "%s" section' % self.bet_name)
