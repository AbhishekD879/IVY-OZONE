...
@pytest.mark.desktop
@pytest.mark.in_play
@pytest.mark.badminton
@pytest.mark.live_scores
@vtest
class Test_C23201001_Verify_Badminton_Live_Scores_by_Template_in_OB(BaseFallbackScoreboardTest):
    """
    TR_ID: C23201001
    NAME: Verify Badminton Live Scores by Template in OB
    DESCRIPTION: This test case verifies that Live Scores for Badminton are shown and updated through OB name template
    ...
    badminton_sport_number, sport_event_count, in_play_event_count = None, None, None
    ...
    def custom_tearDown(cls, **kwargs):
        if cls.is_mobile and tests.settings.cms_env != 'prd0':
            ...
            if cls.badminton_sport_number is not None and cls.sport_event_count is not None:
                cms_config.update_inplay_sport_event_count(sport_number=cls.badminton_sport_number,
    ...
    def test_000_preconditions(self):
        ...
        """
        self.check_fallback_scoreboard_is_configured_for_sport(category_id=self.ob_config.badminton_config.category_id)
        self.check_bip_score_is_configured_for_sport(category_id=self.ob_config.badminton_config.category_id)
        self.check_sport_presence_on_inplay(sport_name='badminton')

        event_params = self.ob_config.add_badminton_event_to_autotest_league(score=self.score, is_live=True,
                                                                             perform_stream=True)
        ...
        self.__class__.league_name_in_play_live_stream_homepage = self.get_accordion_name_for_event_from_ss(event=event[0],
    ...
    def test_002_verify_scores_on_watch_live_and_badminton_pages_from_inplay_page(self):
        """
        DESCRIPTION: **For Mobile/Tablet:**
        DESCRIPTION: **'In-Play'** page from the Sports Menu Ribbon
        DESCRIPTION: Check on **'Watch live'** and **Badminton** pages
        ...
        wait_for_category_in_inplay_ls_structure(category_id=self.ob_config.backend.ti.badminton.category_id)
        ...
        expected_scores = self.get_scores_from_initial_data(
            category_id=str(self.ob_config.backend.ti.badminton.category_id),
            event_id=self.event_id)
        self.verify_event_score_template(events=events,
                                         expected_player1_game_score=expected_scores.get('player_1', ''),
                                         expected_player2_game_score=expected_scores.get('player_2', ''),
                                         expected_player1_points=expected_scores['running_set_score']['1'],
                                         expected_player2_points=expected_scores['running_set_score']['2'])
        self.ob_config.change_score(event_id=self.event_id, team1_name=self.team1, team2_name=self.team2,
                                    home_score=self.new_home_score, away_score=self.new_away_score, points_score=True)
    ...
    def test_003_verify_scores_on_home_page_featured_tab_inplay_module(self):
        ...
        if self.is_mobile:
            ...
            self.__class__.badminton_sport_number = sport_number + 1
            ...
            self.__class__.sport_event_count = self.cms_config.get_sport_event_count(
                sport_number=self.badminton_sport_number)
            initial_number_of_events = self.cms_config.get_max_number_of_inplay_event(
                sport_category=self.ob_config.backend.ti.badminton.category_id)
            ...
            self.cms_config.update_inplay_sport_event_count(sport_number=self.badminton_sport_number,
            ...
            self.assertTrue(in_play_module, msg='Inplay module is not found for Homepage Featured tab')
    ...
    def test_004_verify_scores_on_in_play_and_live_stream_tabs(self):
        ...
        if self.is_mobile:
            ...
            wait_for_category_in_inplay_structure(category_id=self.ob_config.backend.ti.badminton.category_id)
            ...
            self.navigate_to_page(name='/')
            self.site.wait_content_state(state_name='Homepage')
            wait_for_category_in_inplay_sports_ribbon_home_page(category_id=self.ob_config.backend.ti.badminton.category_id)

            inplay_sports = self.site.home.desktop_modules.inplay_live_stream_module.menu_carousel.items_as_ordered_dict
            badminton = inplay_sports.get(self.sport_name)
            self.assertTrue(badminton, msg=f'"{self.sport_name}" not found among "{inplay_sports.keys()}"')
            badminton.click()
            ...
            if self.league_name_in_play_live_stream_homepage in leagues.keys():
                ...
                self._logger.info(f'"{self.league_name_in_play_live_stream_homepage}" is not displayed, '
                                  f'due to hard coded limitation of 4 events')
            ...
            if self.league_name_in_play_live_stream_homepage in leagues.keys():
                ...
                if self.event_name in list(events.keys()):
                    self.ob_config.change_score(event_id=self.event_id, team1_name=self.team1, team2_name=self.team2,
                                                home_score=self.new_home_score,
                                                away_score=self.new_away_score,
                                                points_score=True)
                    ...
                    self.verify_event_score_template(events=events, expected_home_score=self.new_home_score,
                                                     expected_away_score=self.new_away_score, points_score=True,
                                                     serving_ball=True)
                else:
                    self._logger.info(
                        f'"{self.event_name}" is not displayed, due to hard coded limitation of 4 events')
    ...
    def test_005_go_to_badminton_landing_page_in_play_tab_module(self):
        """
        DESCRIPTION: Go to **Badminton Landing page > 'In-Play'** tab/module
        EXPECTED: - Scores are NOT shown in the event name
        EXPECTED: - Each score for a particular team is shown in the same row as the player's name near the Price/Odds button
        EXPECTED: - Score corresponds to the ** 'value'** attribute from WS
        EXPECTED: Score for the home player is shown opposite the home player name (roleCode="HOME"/"TEAM_1")
        ...
        EXPECTED: - Serving is shown as a dot near the player with '*' in the name
        """
        self.navigate_to_page(name='sport/badminton')
        self.site.wait_content_state(state_name='Badminton')
        delimiter = '42/51,' if self.is_mobile else '42'
        if self.is_mobile:
            resp = get_in_play_module_from_ws(delimiter=delimiter)
            self.softAssert(self.assertTrue, resp,
                            msg='In Play module is not configured for Badminton Landing Page')
            self.softAssert(self.assertTrue, self.site.sports_page.tab_content.has_inplay_module(),
                            msg='In Play module is not shown on Badminton landing page')
            ...
            self.softAssert(self.assertTrue, self.site.sports_page.tab_content,
                            msg='In Play module is not enabled for Badminton landing page')
            in_play_tab_name = self.expected_sport_tabs.in_play
            result = self.site.badminton.tabs_menu.click_button(button_name=in_play_tab_name)
            self.assertTrue(result,
                            msg=f'"{in_play_tab_name}" tab is not selected')
            inplay_section = self.site.baseball.tab_content.accordions_list.items_as_ordered_dict
    ...
    def test_006_navigate_to_edp_of_the_appropriate_event(self):
        ...
        self.assertNotIn(self.final_home_score, home_team, msg=f'Home Score is showed in the event name "{home_team}"')
        self.assertNotIn(self.new_away_score, away_team, msg=f'Away Score is showed in the event name "{away_team}"')
        actual_event_name = title.event_name
        expected_event_name = f'{self.team1} {self.team2} S P {self.home_set_scores} {self.new_home_score} {self.away_set_scores} {self.new_away_score}'
