...
@pytest.mark.tst2
@pytest.mark.stg2
# @pytest.mark.prod   # Don't have access to prod OB
# @pytest.mark.hl
@pytest.mark.medium
@pytest.mark.sports
@pytest.mark.in_play
...
@vtest
class Test_C23201019_Verify_Baseball_Live_Scores_by_Template_in_OB(BaseFallbackScoreboardTest):
    """
    TR_ID: C23201019
    NAME: Verify Baseball Live Scores by Template in OB
    DESCRIPTION: This test case verifies that Live Score for Baseball are shown and updated through OB name template
    ...
    PRECONDITIONS: + CMS configs https://confluence.egalacoral.com/display/SPI/Fallback+CMS+Configs
    """
    keep_browser_open = True
    baseball_sport_number, sport_event_count, in_play_event_count = None, None, None
    ...
    def custom_tearDown(cls, **kwargs):
        if cls.is_mobile and tests.settings.cms_env != 'prd0':
            ...
            if cls.baseball_sport_number is not None and cls.sport_event_count is not None:
                cms_config.update_inplay_sport_event_count(sport_number=cls.baseball_sport_number,
    ...
    def test_000_preconditions(self):
        ...
        """
        self.check_fallback_scoreboard_is_configured_for_sport(category_id=self.ob_config.baseball_config.category_id)
        self.check_bip_score_is_configured_for_sport(category_id=self.ob_config.baseball_config.category_id)
        self.check_sport_presence_on_inplay(sport_name='snooker')
        event_params = self.ob_config.add_baseball_event_to_autotest_league(score=self.score, is_live=True, perform_stream=True)
    ...
    def test_002_verify_scores_on_inplay_watch_live_baseball_pages(self):
        """
        ...
        EXPECTED: ![](index.php?/attachments/get/36810)
        """
        ...
        wait_for_category_in_inplay_ls_structure(category_id=self.ob_config.backend.ti.baseball.category_id)
        ...
        expected_scores = self.get_scores_from_initial_data(category_id=str(self.ob_config.backend.ti.baseball.category_id),
    ...
    def test_003_verify_scores_for_the_event_on_home_page_featured_tab_inplay_module(self):
        """
        DESCRIPTION: **For Mobile/Tablet:**
        ...
        if self.is_mobile:
            ...
            self.__class__.baseball_sport_number = sport_number + 1
            ...
            self.__class__.sport_event_count = self.cms_config.get_sport_event_count(
                sport_number=self.baseball_sport_number)
            initial_number_of_events = self.cms_config.get_max_number_of_inplay_event(
                sport_category=self.ob_config.backend.ti.baseball.category_id)
            sport_event_count = initial_number_of_events + 12
            inplay_event_count = initial_number_of_events + 30
            self.cms_config.update_inplay_event_count(event_count=inplay_event_count)
            self.cms_config.update_inplay_sport_event_count(sport_number=self.baseball_sport_number,
                                                            event_count=sport_event_count)
            time.sleep(10)  # to avoid delays in CMS
    ...
    def test_004_verify_scores_on_in_play_tab_and_live_stream_tabs(self):
        """
        DESCRIPTION: **For Mobile/Tablet:**
        ...
        if self.is_mobile:
            ...
            wait_for_category_in_inplay_structure(category_id=self.ob_config.backend.ti.baseball.category_id)
            ...
            self.assertTrue(sections, msg=f'No sports sections found on {live_stream_tab} page')
            section = sections.get(self.sport_name.upper())
            self.assertTrue(section, msg=f'"{self.sport_name.upper()}" section not found among "{sections.keys()}"')
            section.expand()
            ...
            wait_for_category_in_inplay_sports_ribbon_home_page(category_id=self.ob_config.backend.ti.baseball.category_id)

            inplay_sports = self.site.home.desktop_modules.inplay_live_stream_module.menu_carousel.items_as_ordered_dict
            baseball = inplay_sports.get(self.sport_name)
            self.assertTrue(baseball, msg=f'"{self.sport_name}" not found among "{inplay_sports.keys()}"')
            baseball.click()
            ...
            leagues = self.site.home.get_module_content(module_name=module_name).accordions_list.items_as_ordered_dict
            self.assertTrue(leagues, msg=f'No leagues found for "{vec.sb.BASEBALL}"')
            if self.league_name_in_play_live_stream_homepage in leagues.keys():
                league = leagues.get(self.league_name_in_play_live_stream_homepage)
                events = league.items_as_ordered_dict
                self.assertTrue(events, msg=f'No events were found for "{self.league_name_in_play_live_stream_homepage}" league')
    ...
    def test_005_go_to_baseball_landing_page_in_play_tab_module(self):
        """
        DESCRIPTION: Go to **Baseball Landing page > 'In Play'** tab/module
        EXPECTED: - Scores are NOT shown in the event name
        EXPECTED: - Each score for particular team is shown in the same row as team's name near the Price/Odds button
        EXPECTED: - Score corresponds to the ** 'value'** attribute from WS
        EXPECTED: Score for the home team is shown opposite the home team name (roleCode="HOME"/"TEAM_1")
        ...
        delimiter = '42/5,' if self.is_mobile else '42'
        if self.is_mobile:
            resp = get_in_play_module_from_ws(delimiter=delimiter)
            self.softAssert(self.assertTrue, resp, msg='In Play module is not configured for Baseball Landing Page')
            self.softAssert(self.assertTrue, self.site.sports_page.tab_content.has_inplay_module(),
                            msg='In Play module is not enabled for Baseball landing page')
            inplay_section = self.site.baseball.tab_content.in_play_module.items_as_ordered_dict
            ...
            result = self.site.baseball.tabs_menu.click_button(button_name=in_play_tab_name)
            self.assertTrue(result,
        ...
        self.assertTrue(league, msg=f'"{league_name}" league not found among "{inplay_section.keys()}"')
        league.expand()
        events = league.items_as_ordered_dict
        ...
        self.ob_config.change_score(event_id=self.event_id, team1_name=self.team1, team2_name=self.team2,
                                    home_score=self.new_home_score, away_score=self.new_away_score)
        self.wait_for_score_update_from_inplay_ms(score=self.new_home_score, team='home', event_id=self.event_id, delimiter=delimiter)
        self.verify_event_score_template(events=events,
                                         expected_home_score=self.new_home_score,
                                         expected_away_score=self.new_away_score)
    ...
    def test_006_navigate_to_edp_of_the_appropriate_event(self):
        ...
        EXPECTED: ![](index.php?/attachments/get/36811)
        """
