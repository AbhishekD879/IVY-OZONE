...
@pytest.mark.five_a_side_combination_reg_tests
...
@pytest.mark.banach
@vtest
class Test_C65053102_Verify_successful_Bet_Placement_and_Receipt_for_Bal_To_Concede_1Passes_35_GoalOutsideBox1_Tackles1_Tackles1(BaseFiveASide):
    """
    TR_ID: C65053102
    NAME: Verify successful Bet Placement and Receipt for Balanced - To Concede-1 Passes-35 Goal outside the box-1 Tackles-1 tackles
    ...
    def test_000_preconditions(self):
        ...
        self.__class__.event_start_time_local = self.convert_time_to_local(
            ...
            ss_data=True)
        self.__class__.full_event_name = f'{self.event_name} {self.__class__.event_start_time_local}'
        self._logger.info(
            f'***Found Football event "{event_id}" "{self.full_event_name}" with market name "{market_name}", '
            f'event id "{self.event_name}", event start time "{self.__class__.event_start_time_local}"')
    ...
    def test_007_navigate_to_my_bets_open__settled(self):
        ...
        bet_name, single_byb_section = self.site.open_bets.tab_content.accordions_list.get_bet(
            bet_type=vec.bet_history.FIVE_A_SIDE_BET_TYPE_NAME.upper(), event_names=self.full_event_name, number_of_bets=1)
        ...
        self.assertEqual(betleg.event_name, self.event_name, msg=f'Event name "{betleg.event_name}" is not '
        ...
        self.assertEqual(betleg.odds_value, self.betslip_odds,
                         msg=f'Odds value "{betleg.odds_value}" is not the same as expected "{self.betslip_odds}"')
