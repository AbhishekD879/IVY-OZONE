...
@pytest.mark.banach
@vtest
class Test_C65221770_Verify_successful_BPReceiptforBal_ToConcede1_Passes35_GoalsOutsideTheBox1_ShotsOutsideTheBox1_ShotsOutsideTheBox1(BaseFiveASide):
    """
    TR_ID: C65221770
    NAME: 	Verify successful Bet Placement and Receipt for Balanced - To Concede-1 Passes-35 Goals Outside The Box-1 Shots Outside The Box-1 Shots Outside The Box-1
    ...
    def test_005_verify_bet_receipt_on_ui(self):
        ...
        # self.__class__.expected_players_list = [item.lower() for item in self.expected_players_list]
    ...
    def test_007_navigate_to_my_bets_open__settled(self):
        ...
        self.assertEquals(len(outcome_names), len(self.expected_players_list),
                          msg=f'Outcome name "{len(outcome_names)}" is not '
                              f'the same as expected "{len(self.expected_players_list)}"')
