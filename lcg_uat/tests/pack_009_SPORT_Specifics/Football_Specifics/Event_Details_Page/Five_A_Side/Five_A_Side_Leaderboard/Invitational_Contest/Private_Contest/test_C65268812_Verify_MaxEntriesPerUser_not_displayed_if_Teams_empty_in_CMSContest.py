...
@pytest.mark.banach
@vtest
class Test_C65268812_Verify_MaxEntriesPerUser_text_not_displayed_if_Teams_empty_in_CMSPrivateContest(BaseFiveASide,
                                                                                                     BaseFeaturedTest):
    """
     TR_ID: C65268812
     NAME: 	Verify MAXIMUM ENTRIES PER USER text on Leaderboard is not displayed if Teams entry is empty in CMS-Private Contest
     DESCRIPTION: This testcase verifies MAXIMUM ENTRIES PER USER text on Leaderboard is not displayed if Teams entry is empty in CMS-Private Contest
     ...
     PRECONDITIONS: In Fronend: Navigate to private contest via URL(https://beta-sports.ladbrokes.com/5-a-side/leaderboard/{contest-id}) by using contest-id or take the URL from Contest URL field in CMS
    ...
    def test_000_preconditions(self):
        ...
        self.device.navigate_to(url=tests.HOSTNAME + '/5-a-side/leaderboard/' + contest_id)
        sleep(6)
    ...
    def test_005_Verify_Leaderboard_UI_elements(self):
        ...
        self.assertEqual(actual_message, vec.five_a_side.TOTAL_ENTRIES.format(s=1, n=self.size_value),
                         msg=f'Actual message: "{actual_message}" is not same as Expected message: "{vec.five_a_side.MAX_ENTRIES_REACHED.format(s=1, n=self.size_value)}"')
