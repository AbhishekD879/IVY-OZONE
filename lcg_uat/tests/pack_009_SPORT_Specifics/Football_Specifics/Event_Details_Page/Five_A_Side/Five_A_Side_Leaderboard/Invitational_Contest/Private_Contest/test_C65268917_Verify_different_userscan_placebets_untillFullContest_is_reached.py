...
@pytest.mark.banach
@vtest
class Test_C65268917_Verify_different_users_can_place_bets_untill_full_contest_is_reached(BaseFiveASide):
    """
    TR_ID: C65268917
    NAME: Verify different users can place bets untill full contest is reached-private contest
    DESCRIPTION: Verify different users can place bets untill full contest is reached- private contest
    ...
    def test_000_preconditions(self):
        ...
        self.cms_config.enable_disable_show_down_overlay(enabled=False)
        self.__class__.contest_name = f"Auto test " + event_name + "_" + str(self.event_id) + "_" + str(round(time()))
        self.__class__.response_id = self.cms_config.create_five_a_side_show_down(contest_name=self.contest_name,
                                                                                  entryStake="1", size="1", teams="1",
    ...
    def test_003_enter_stake_1_and_place_bet(self):
        ...
        self.byb_betslip_panel.selection.content.amount_form.enter_amount(value=1)
