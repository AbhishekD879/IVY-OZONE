...
@pytest.mark.banach
@vtest
class Test_C65268918_Verify_MAX_ENTRIES_REACHED_on_Leaderboard_for_single_contest_and_Teams_entry_as_1_in_CMS_Private_Contest(BaseFiveASide, BaseFeaturedTest):
    """
    TR_ID: C65268918
    NAME: Verify MAX ENTRIES REACHED on Leaderboard for a single contest and Teams entry as 1 in CMS-Private Contest
    DESCRIPTION: Verify MAX ENTRIES REACHED on Leaderboard for a single contest and Teams entry as 1 in CMS-Private Contest
    ...
    def test_000_preconditions(self):
        ...
        contest_response = self.cms_config.create_five_a_side_show_down(contest_name=self.contest_name, isInvitationalContest=True,
                                                                        isPrivateContest=True, entryStake="1", size="2", teams="1",
    ...
    def test_005_verify_leaderboard_ui_elements(self):
        ...
        self.assertEqual(actual_message, "MAX ENTRIES REACHED (1/1)",
                         msg=f'Actual message: "{actual_message}" is not same as Expected message: "MAX ENTRIES REACHED (1/1)"')
