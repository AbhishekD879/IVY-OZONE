...
@pytest.mark.banach
@vtest
class Test_C65271576_Verify_MAXIMUM_TOTAL_ENTRIES_text_on_Leaderboard_is_not_displayed_if_Size_entry_is_empty_in_CMS_Public_contest_via_lobby(BaseFiveASide, BaseFeaturedTest):
    """
    TR_ID: C65271576
    NAME: Verify MAXIMUM TOTAL ENTRIES text on Leaderboard is not displayed if Size entry is empty in CMS - Public contest via lobby
    DESCRIPTION: Verify MAXIMUM TOTAL ENTRIES text on Leaderboard is not displayed if Size entry is empty in CMS - Public contest via lobby
    ...
    PRECONDITIONS: Destination: https://beta2-sports.ladbrokes.com/5-a-side/lobby    PRECONDITIONS: Contest name: {Team1}vs{Team2}(event id)
    ...
    def test_000_preconditions(self):
        """
        DESCRIPTION: "In CMS, quick link should be configured for Lobby
        DESCRIPTION: Homepage> Quick Links > Create Sports Quick Link.
        DESCRIPTION: Title: {title} e.g.: Beta2 Lobby
        DESCRIPTION: Destination: https://beta2-sports.ladbrokes.com/5-a-side/lobby    DESCRIPTION: Contest name: {Team1}vs{Team2}(event id)
        DESCRIPTION: Stake: 1
        DESCRIPTION: Event: {event id}
        DESCRIPTION: Size: {blank}
        DESCRIPTION: Teams: 2
        ...
        quick_link_title = 'Autotest Beta2 Lobby' + '_C65271576'
    ...
    def test_002_enter_stake_1_place_bet(self):
        ...
        wait_for_result(lambda: self.byb_betslip_panel.select_your_leaderboard.contest.items_as_ordered_dict is not None,
                        timeout=10)
        contest = self.byb_betslip_panel.select_your_leaderboard.contest.items_as_ordered_dict.get(self.contest_name)
    ...
    def test_004_verify_button_build_your_team_and_is_enabled(self):
        """
    ...
    def test_005_verify_Maximum_Size_total_entries_1_Size_should_not_be_available(self):
        """
        DESCRIPTION: Verify <Maximum {Teams} entries per user (1/{Teams})> should not be available
