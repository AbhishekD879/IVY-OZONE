...
@pytest.mark.banach
@vtest
class Test_C65271635_Verify_MAX_ENTRIES_PER_USER_text_on_Leaderboard_is_not_displayed_if_Teams_entry_empty_in_CMS_Public_contest_via_lobby(BaseFiveASide, BaseFeaturedTest):
    """
    TR_ID: C65271635
    NAME: Verify MAXIMUM ENTRIES PER USER text on Leaderboard is not displayed if Teams entry is empty in CMS - Public contest via lobby
    DESCRIPTION: Verify MAXIMUM ENTRIES PER USER text on Leaderboard is not displayed if Teams entry is empty in CMS - Public contest via lobby
    ...
    def test_000_preconditions(self):
        ...
        self.__class__.contest_name = f"Auto test " + event_name + "_" + str(self.event_id) + "_" + "1635"
    ...
    def test_004_Verify_button_BUILD_ANOTHER_TEAM_and_is_enabled(self):
        """
         ...
         """
        self.assertTrue(self.site.five_A_side_leaderboard.build_btn.is_enabled(), msg='')
        actual_button_text = self.site.five_A_side_leaderboard.build_btn.text
        self.assertEqual(actual_button_text, vec.five_a_side.BUILD_ANOTHER_TEAM_TEXT,
                         msg=f'Actual button text: "{actual_button_text}" is not same as Expected button text: "{vec.five_a_side.BUILD_ANOTHER_TEAM_TEXT}"')
    ...
    def test_005_Verify_Maximum_Teams_entries_per_user_1_Teams_should_not_be_available(self):
        ...
        self.assertIsNone(self.site.five_A_side_leaderboard.entries_per_user,
                          msg='"Maximun entries per user" is still displayed which is not Expected')
