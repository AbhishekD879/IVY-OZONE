...
@pytest.mark.banach
@vtest
class Test_C65272220_Verify_MAX_ENTRIES_REACHED_on_Leaderboard_for_a_single_contest_and_Teams_entry_as_1_in_CMS_Public_contest_via_lobby(BaseFiveASide, BaseFeaturedTest):
    """
    TR_ID: C65272220
    NAME: Verify MAX ENTRIES REACHED on Leaderboard for a single contest and Teams entry as 1 in CMS - Public contest via lobby
    DESCRIPTION: Verify MAX ENTRIES REACHED on Leaderboard for a single contest and Teams entry as 1 in CMS - Public contest via lobby
    ...
    def test_000_preconditions(self):
        ...
        contest_name = f"Auto test " + event_name + "_" + str(event_id) + "_" + str(round(time()))
        self.__class__.response_id = self.cms_config.create_five_a_side_show_down(contest_name=contest_name,
                                                                                  entryStake=self.stake_value, size=self.size_value, teams="1",
    ...
    def test_003_click_on_button_VIEW_ENTRY(self):
        ...
        """
        wait_for_result(lambda: self.site.byb_bet_receipt_panel.view_entry.view_entry_button.is_displayed(),
                        timeout=5,
