...
@pytest.mark.banach
@vtest
class Test_C65272243_Verify_BP_without_selecting_contest_on_Betslip_for_a_single_contest_Public_contest_via_lobby(BaseFiveASide,
                                                                                                      BaseFeaturedTest):
    """
    TR_ID: C65272243
    NAME: Verify bet placement without selecting contest on Betslip for a single contest - Public contest via lobby
    ...
    def test_000_preconditions(self):
        ...
        """
        self.__class__.quick_link_name = "Autotest_" + "C65272243"
        ...
        self.cms_config.create_five_a_side_show_down(contest_name=self.contest_name,
                                                     ...
                                                     event_id=self.event_id, isInvitationalContest=True, isPrivateContest=False)
        self.site.login(username=tests.settings.betplacement_user)
        if self.device_type == 'mobile':
            self.navigate_to_page('/5-a-side/lobby')
        else:
