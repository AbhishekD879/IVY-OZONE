...
@pytest.mark.banach
@vtest
class Test_C65269105_Verify_MAXIMUM_ENTRIES_PER_USER_text_on_Leaderboard_is_not_displayed_if_Teams_entry_is_empty_in_CMS_Publi_Contest(BaseFiveASide):
    """
    TR_ID: C65269105
    NAME: Verify MAXIMUM ENTRIES PER USER text on Leaderboard is not displayed if Teams entry is empty in CMS - Public Contest
    DESCRIPTION: Verify MAXIMUM ENTRIES PER USER text on Leaderboard is not displayed if Teams entry is empty in CMS - Public Contest
    ...
    def test_005_Verify_Maximum_Teams_entries_per_user_1_Teams_should_not_be_available(self):
        ...
        self.assertFalse(self.site.five_A_side_leaderboard.entries_per_user,
                         msg='Maximum entries per user" is not displayed')
