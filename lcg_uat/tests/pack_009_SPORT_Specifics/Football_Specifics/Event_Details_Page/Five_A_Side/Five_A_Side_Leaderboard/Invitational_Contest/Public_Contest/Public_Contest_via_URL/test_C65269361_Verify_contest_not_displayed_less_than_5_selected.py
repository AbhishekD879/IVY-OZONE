...
@pytest.mark.banach
@vtest
class test_C65269361_Verify_the_contest_is_not_displayed_less_than_5_legs_are_selected_for_bet_placement(BaseFiveASide):
    """
    TR_ID: C65269361
    NAME: Verify the contest is not displayed if less than 5 legs (players) are selected for bet placement - public contest
    DESCRIPTION: Verify the contest is not displayed if less than 5 legs (players) are selected for bet placement - public contest
    ...
    def test_000_preconditions(self):
        ...
        response_id = self.cms_config.create_five_a_side_show_down(contest_name=contest_name,
                                                                   ...
                                                                   isPrivateContest=False, display=True)
        self.site.login()
        self.device.navigate_to(url=tests.HOSTNAME + '/5-a-side/leaderboard/' + response_id['id'])
    ...
    def test_001_Select_less_than_5_players_for_5_A_Side_formation_Click_on_Place_bet(self):
        """
        DESCRIPTION: Select less than 5 valid players on Pitch view
        ...
        leader_board_section = byb_betslip_panel.has_select_your_leaderboard()
        self.assertFalse(leader_board_section, msg="Contest is displaying on betslip")
