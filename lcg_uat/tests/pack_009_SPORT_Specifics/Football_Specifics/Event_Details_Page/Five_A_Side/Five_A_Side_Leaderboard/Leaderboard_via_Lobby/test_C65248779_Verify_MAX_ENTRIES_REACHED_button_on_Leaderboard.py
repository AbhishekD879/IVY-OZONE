...
@pytest.mark.banach
@vtest
class Test_C65248779_Verify_MAX_ENTRIES_REACHED_button_on_Leaderboard_for_a_single_contest_and_Teams_entry_as_1_in_CMS_navigation_via_lobby(BaseFiveASide, BaseFeaturedTest):
    """
    TR_ID: C65248779
    NAME: Verify MAX ENTRIES REACHED button on Leaderboard for a single contest and Teams entry as 1 in CMS - navigation via lobby
    DESCRIPTION: Verify MAX ENTRIES REACHED button on Leaderboard for a single contest and Teams entry as 1 in CMS - navigation via lobby
    ...
    def test_000_preconditions(self):
        ...
        quick_link_title = 'Autotest Beta2 Lobby' + '_C65248779'
        ...
        contest_name = f"Auto test " + event_name + "_" + str(event_id) + "_" + 'C65248779'
    ...
    def test_004_Verify_button_MAX_ENTRIES_REACHED_and_it_is_disabled(self):
        ...
        EXPECTED: Verify <Maximum {1} entries per user (1/{1})>
        EXPECTED: Verify <Maximum {5} total entries (1/{5})>
        EXPECTED: Verify button MAX ENTRIES REACHED". Button should be disabled.
        ...
        self.assertTrue(self.site.five_A_side_leaderboard.total_entries.is_displayed(),
                        msg='Maximum {1} total entries (1/{1})" Link Displayed')
        max_entries_reached = self.site.five_A_side_leaderboard.build_btn
        self.assertTrue(max_entries_reached.is_displayed(), msg='MAX ENTRIES REACHED is not displayed')
        self.assertEqual(max_entries_reached.text, 'MAX ENTRIES REACHED (1/1)',
                         msg=f'Actual text {max_entries_reached.text} is not same as '
                             f'Expected text "MAX ENTRIES REACHED (1/1)"')
