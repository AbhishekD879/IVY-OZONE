...
@pytest.mark.banach
@vtest
class Test_C65248783_Verify_UI_elements_on_Leaderboard_for_a_single_contest_navigation_via_lobby(BaseFiveASide, BaseFeaturedTest):
    """
    TR_ID: C65248783
    NAME: Verify UI elements on Leaderboard for a single contest - navigation via lobby
    DESCRIPTION: Verify UI elements on Leaderboard for a single contest - navigation via lobby
    """
    ...
    def test_000_preconditions(self):
        ...
        quick_link_title = 'Autotest Beta2 Lobby' + '_C65248783'
        ...
        contest_name = f"Auto test " + event_name + "_" + str(event_id) + "_" + 'C65248783'
        self.__class__.response_id = self.cms_config.create_five_a_side_show_down(contest_name=contest_name,
                                                                                  ...
                                                                                  teams="1",
                                                                                  event_id=event_id, display=True)['id']
        self.site.login()
        if self.device_type == 'mobile':
            sleep(10)   # Takes time for the event to reflect
