...
@pytest.mark.banach
@vtest
class Test_C65268668_Verify_User_Can_See_N_Number_Of_Contests_On_Betslip_As_Created_In_CMS_For_An_Event_via_Lobby(BaseFiveASide, BaseFeaturedTest):
    """
    TR_ID: C65268668
    NAME:Verify user can see n number of contests on betslip as created in CMS for an event - navigation via lobby
    DESCRIPTION:Verify user can see n number of contests on betslip as created in CMS for an event - navigation via lobby
    ...
    def test_000_preconditions(self):
        ...
        for i in range(1, 3):
            ...
            self.cms_config.create_five_a_side_show_down(contest_name=contest_name, entryStake=self.stake_value, size="5",
                                                         teams="2",
                                                         event_id=event_id)
