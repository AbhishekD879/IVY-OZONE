...
@pytest.mark.other
@vtest
class Test_C65304880_Deleted_removed_surface_bets_should_not_be_displayed_in_Fanzone_FE(Common):
    """
    TR_ID: C65304880
    NAME: Deleted/removed surface bets should not be displayed in Fanzone FE
    DESCRIPTION: To verify if user removes/deletes the surface bet from CMS then those surface bets should not be displayed in Fanzone page
    ...
    def test_000_preconditions(self):
        ...
        if not fanzone_status.get('enabled'):
            self.cms_config.update_system_configuration_structure(config_item='Fanzone',
                                                                  field_name='enabled',
                                                                  field_value=True)
        self.cms_config.get_fanzone(vec.fanzone.TEAMS_LIST.aston_villa.title())
    ...
    def test_002_verify_if_the_surface_bets_are_shown_in_now_and_next_tab(self):
        ...
        self.assertIn(surface_bet_name, self.surface_bets,
                      msg=f'Created surface bet "{surface_bet_name}" is not present in "{self.surface_bets}"')

    def test_003_now_login_to_cms_and_removedelete_some_of_the_surface_bets_created_for_fanzone_page_for_the_particular_team(self):
        """
        DESCRIPTION: Now login to CMS and remove/delete some of the surface bets created for Fanzone page for the particular team
        EXPECTED: User should be able to delete/remove surface bets from CMS
        """
        self.cms_config.delete_surface_bet(surface_bet_id=self.surface_bet.get('id'))
        # Updating the surface bet list, since one bet-id is removed above
        self.cms_config._created_surface_bets.remove(self.surface_bet.get('id'))
    ...
    def test_004_navigate_to_fanzone_fe_and_verify_the_surface_bets(self):
        ...
        EXPECTED: Deleted/removed surface bets should not be shown in Fanzone fe under Now and Next tab
        """
        sleep(10)  # deleted surface bet took time to update in ui
