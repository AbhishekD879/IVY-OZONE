...
@pytest.mark.desktop
@vtest
class Test_C65304896_Verify_Now_and_Next_tab_displays_all_the_upcoming_matches_in_Chronological_order_only(Common):
    """
    TR_ID: C65304896
    NAME: Verify Now and Next tab displays all the upcoming matches in Chronological order only
    DESCRIPTION: Verify Now and Next tab displays all the upcoming matches in Chronological order only
    ...
    def test_004_verify_that_the_events_are_displayed_in_chronological_order(self):
        """
        DESCRIPTION: Verify that the events are displayed in chronological order
        EXPECTED: User should be able to see the events are shown in chronological order with the soonest event will be shown first
        """
        expected_date_list = []
