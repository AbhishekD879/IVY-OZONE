...
@pytest.mark.other
@vtest
class Test_C65305124_Top_4_markets_are_shown_in_Now_and_Next_tab(Common):
    """
    TR_ID: C65305124
    NAME: Top 4 markets are shown in Now and Next tab
    DESCRIPTION: To verify created Top 4 market is shown in Now and Next tab under (Teams)Bet in Fanzone page
    PRECONDITIONS: 1)User has access to CMS
    PRECONDITIONS: 2)Fanzone option is enabled in Structure and Fanzone entry points in Fanzone section in CMS
    PRECONDITIONS: 3) Top 4 market is created in OB for that type or competition
    PRECONDITIONS: 4)User has FE url and Valid credentials to Login Lads FE
    """
    keep_browser_open = True
    teamname = vec.fanzone.TEAMS_LIST.aston_villa
    new_market_name = "Top 4"
    ...
    def test_004_verify_is_user_is_able_to_see_top_4_market_in_now_and_next_tab(self):
        """
        DESCRIPTION: Verify is user is able to see Top 4 Market in Now and next Tab
        EXPECTED: User should be able to see Top 4 market under (Team)bet label in Now and Next tab
        """
