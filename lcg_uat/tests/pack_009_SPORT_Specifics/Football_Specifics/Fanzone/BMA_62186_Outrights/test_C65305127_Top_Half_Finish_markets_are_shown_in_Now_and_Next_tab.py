...
@pytest.mark.other
@vtest
class Test_C65305127_Top_Half_Finish_markets_are_shown_in_Now_and_Next_tab(Common):
    """
    TR_ID: C65305127
    NAME: Top Half Finish markets are shown in Now and Next tab
    DESCRIPTION: To verify created Top Half Finish market is shown in Now and Next tab under (Teams)Bet in Fanzone page
    PRECONDITIONS: 1)User has access to CMS
    PRECONDITIONS: 2)Fanzone option is enabled in Structure and Fanzone entry points in Fanzone section in CMS
    PRECONDITIONS: 3) Top Half Finish market is created in OB for that type or competition
    PRECONDITIONS: 4)User has FE url and Valid credentials to Login Lads FE
    """
    keep_browser_open = True
    teamname = vec.fanzone.TEAMS_LIST.chelsea
    new_market_name = "Top Half Finish"
    ...
    def test_000_preconditions(self):
        ...
        market_template_id = outright_event.ss_response['event']['children'][0]['market'][
            'templateMarketId']
        ...
        self.ob_config.map_fanzone_event_selection_id(selection_id=selection_id,
                                                      ...
                                                      team_external_id=self.ob_config.football_config.fanzone_external_id.chelsea)
    ...
    def test_004_verify_is_user_is_able_to_see_top_half_finish_market_in_now_and_next_tab(self):
        """
        DESCRIPTION: Verify is user is able to see Top Half Finish Market in Now and next Tab
        EXPECTED: User should be able to see Top Half Finish market under (Team)bet label in Now and Next tab
        """
        self.device.refresh_page()
        self.site.fanzone.tabs_menu.items_as_ordered_dict.get(vec.fanzone.NOW_AND_NEXT).click()
