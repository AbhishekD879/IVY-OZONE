...
@pytest.mark.fanzone
@vtest
class Test_C65305070_Verify_User_will_be_able_to_navigate_to_Fanzone_page_from_other_entry_points_when_Show_in_Football_Home_launch_Banner_toggle_is_OFF(Common):
    """
    TR_ID: C65305070
    NAME: Verify User will be able to navigate to Fanzone page from other entry points when Show in Football Home launch Banner toggle is OFF
    DESCRIPTION: Verify User will be able to navigate to Fanzone page from other entry points when Show in Football Home launch Banner toggle is OFF
    """
    ...
    def test_000_preconditions(self):
        ...
        PRECONDITIONS: 3) Show In Football Home Launch Banner toggle should be OFF and all other Toggle's should be ON in Fanzone Configuration(CMS)
        ...
        if not fanzone_status.get('enabled'):
            self.cms_config.update_system_configuration_structure(config_item='Fanzone',
                                                                  field_name='enabled',
                                                                  field_value=True)
        newcastle_united_fanzone = self.cms_config.get_fanzone(vec.fanzone.TEAMS_LIST.leeds_united.title())
        if newcastle_united_fanzone:
            self.cms_config.update_fanzone(vec.fanzone.TEAMS_LIST.leeds_united.title(),
                                           homePage=True,
                                           sportsRibbon=True,
                                           footballHome=False,
                                           atozMenu=True)
    ...
    def test_001_logon_to_lads_application(self):
        ...
        teams[vec.fanzone.TEAMS_LIST.leeds_united.title()].scroll_to_we()
        teams[vec.fanzone.TEAMS_LIST.leeds_united.title()].click()
    ...
    def test_003_verify_launch_banner_is_not_displayed(self):
        """
    ...
    def test_004_verify_user_is_able_to_navigate_to_fanzone_page_using_any_of_the_below_listed_entry_pointsa_home_page_launch_bannerb_fanzone_in_a_z_menuc_sports_ribbonmobile(self):
        """
        DESCRIPTION: Verify user is able to navigate to Fanzone page using any of the below listed entry points
        DESCRIPTION: a. Home Page Launch Banner
