...
@pytest.mark.desktop
@vtest
class Test_C65304914_To_verify_user_is_navigated_to_Fanzone_page_when_user_clicks_on_EXIT_CTA_on_TEAM_ALERTS_popup(Common):
    """
    TR_ID: C65304914
    NAME: To verify user is navigated to Fanzone page when user clicks on EXIT CTA on TEAM ALERTS popup
    DESCRIPTION: To verify user is navigated to Fanzone page when user clicks on EXIT CTA on TEAM ALERTS popup
    ...
    def test_001_click_on_settings_button(self):
        ...
        self.__class__.dialog_teamalert = self.site.wait_for_dialog(dialog_name=vec.dialogs.DIALOG_MANAGER_TEAM_ALERTS, timeout=30)
    ...
    def test_004_click_on_exit_cta(self):
        ...
        """
        self.dialog_teamalert.exit_button.click()
        wait_for_result(lambda: self.site.fanzone.tabs_menu,
                        name='"Fanzone tab menus" to be displayed.')
        current_tab = self.site.fanzone.tabs_menu.current
        self.assertEqual(current_tab, vec.fanzone.NOW_AND_NEXT,
