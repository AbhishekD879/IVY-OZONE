...
@vtest
class Test_C65972770_Verify_GA_Tracking_When_user_adds_the_bet_from_the_carousel_in_the_betslip(BaseDataLayerTest, BaseBetSlipTest):
    """
    TR_ID: C65972770
    NAME: Verify GA Tracking When user adds the bet from the carousel in the betslip
    DESCRIPTION: This test case is to verify GA Tracking When user adds the bet from the carousel in the betslip
    ...
    def test_000_preconditions(self):
        ...
        if tests.settings.backend_env == 'prod':
            ...
            self.assertTrue(event, msg=f'no events available for category football')
    ...
    def test_005_add_any_selection_from_the_trending_bets_carousel(self):
        """
        DESCRIPTION: Add any selection from the trending bets carousel
        EXPECTED: Able to add the selection from the trending bets carousel
        """
        trending_bet_section = self.site.betslip.trending_bets_section
        self.assertTrue(trending_bet_section, msg= f'trending_bet_section is not available')
        ...
        self.__class__.market_name = bet_card_info.market_name
