...
@pytest.mark.desktop
@pytest.mark.in_play
@pytest.mark.gaelic_football
@pytest.mark.live_scores
@vtest
class Test_C22966214_Verify_Gaelic_Football_Live_Scores_by_Template_in_OB(BaseFallbackScoreboardTest):
    """
    TR_ID: C22966214
    NAME: Verify Gaelic Football Live Scores by Template in OB
    ...
    gaelic_football_sport_number, sport_event_count, in_play_event_count = None, None, None
    ...
    def custom_tearDown(cls, **kwargs):
        if cls.is_mobile and tests.settings.cms_env != 'prd0':
            ...
            if cls.gaelic_football_sport_number is not None and cls.sport_event_count is not None:
                cms_config.update_inplay_sport_event_count(sport_number=cls.gaelic_football_sport_number,
    ...
    def test_000_preconditions(self):
        ...
        """
        self.check_fallback_scoreboard_is_configured_for_sport(category_id=self.ob_config.gaelic_football_config.category_id)
        self.check_bip_score_is_configured_for_sport(category_id=self.ob_config.gaelic_football_config.category_id)
        self.check_sport_presence_on_inplay(sport_name='gaelicfootballgaa')
        event_params = self.ob_config.add_gaelic_football_event_to_gaelic_football_all_gaelic_football(score=self.score,
                                                                                                       is_live=True,
                                                                                                       perform_stream=True)
    ...
    def test_002_verify_scores_on_watch_live_and_gaelic_football_pages_from_inplay_page(self):
        """
        ...
        wait_for_category_in_inplay_ls_structure(category_id=self.ob_config.backend.ti.gaelic_football.category_id)
        ...
        expected_scores = self.get_scores_from_initial_data(category_id=str(self.ob_config.backend.ti.gaelic_football.category_id),
    ...
    def test_003_verify_scores_on_home_page_featured_tab_inplay_module(self):
        ...
        if self.is_mobile:
            ...
            self.__class__.gaelic_football_sport_number = sport_number + 1
            self.__class__.in_play_event_count = self.cms_config.get_inplay_event_count()
            self.__class__.sport_event_count = self.cms_config.get_sport_event_count(sport_number=self.gaelic_football_sport_number)
            initial_number_of_events = self.cms_config.get_max_number_of_inplay_event(
                sport_category=self.ob_config.backend.ti.gaelic_football.category_id)
            ...
            self.cms_config.update_inplay_sport_event_count(sport_number=self.gaelic_football_sport_number,
    ...
    def test_004_verify_scores_on_inplay_and_live_stream_tab_module(self):
        ...
        if self.is_mobile:
            ...
            wait_for_category_in_inplay_structure(category_id=self.ob_config.backend.ti.gaelic_football.category_id)
            ...
            self.assertTrue(events, msg=f'No events were found for "{self.league_name_in_play_live_stream_homepage}" league')
            self.wait_for_score_update_from_inplay_ms(score=self.new_home_score, team='home', event_id=self.event_id)
            self.verify_event_score_template(events=events, expected_home_score='7',
                                             expected_away_score=self.new_away_score)
        else:
            self.navigate_to_page(name='/')
            self.site.wait_content_state(state_name='Homepage')

            wait_for_category_in_inplay_sports_ribbon_home_page(category_id=self.ob_config.backend.ti.gaelic_football.category_id)

            inplay_sports = self.site.home.desktop_modules.inplay_live_stream_module.menu_carousel.items_as_ordered_dict
            gaelic_football = inplay_sports.get(self.sport_name)
            self.assertTrue(gaelic_football, msg=f'"{self.sport_name}" not found among "{inplay_sports.keys()}"')
            gaelic_football.click()
    ...
    def test_005_go_to_gaelic_football_landing_page_in_play_tab_module(self):
        """
        ...
        EXPECTED: - Score for the away team is shown opposite the away team name (roleCode="AWAY"/"TEAM_2")
        """
        self.navigate_to_page(name='sport/gaelic-football')
        self.site.wait_content_state(state_name='GaelicFootball')
        delimiter = '42/53,' if self.is_mobile else '42'
        if self.is_mobile:
            resp = get_in_play_module_from_ws(delimiter=delimiter)
            self.softAssert(self.assertTrue, resp, msg='In Play module is not configured for Gaelic Football Landing Page')
            self.softAssert(self.assertTrue, self.site.sports_page.tab_content.has_inplay_module(),
                            msg='In Play module is not enabled for Gaelic Football landing page')
