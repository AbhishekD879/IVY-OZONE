...
@pytest.mark.in_play
@vtest
class Test_C23061808_Verify_Handball_Live_Scores_by_Template_in_OB(BaseFallbackScoreboardTest):
    """
    TR_ID: C23061808
    NAME: Verify Handball Live Scores by Template in OB
    DESCRIPTION: This test case verifies that Live Score for Handball are shown and updated through OB name template
    ...
    handball_sport_number, sport_event_count, in_play_event_count = None, None, None
    ...
    def custom_tearDown(cls, **kwargs):
        if cls.is_mobile and tests.settings.cms_env != 'prd0':
            ...
            if cls.handball_sport_number is not None:
                cms_config.update_inplay_sport_event_count(sport_number=cls.handball_sport_number,
    ...
    def test_002_verify_scores_on_watch_live_and_handball_pages_from_inplay_page(self):
        """
        DESCRIPTION: **For Mobile/Tablet:**
        DESCRIPTION: **'In-Play'** page from the Sports Menu Ribbon
        DESCRIPTION: Check on **'Watch live'** and **Handball** pages
        ...
        EXPECTED: ![](index.php?/attachments/get/36790)
        """
        ...
        expected_scores = self.get_scores_from_initial_data(category_id=str(self.ob_config.backend.ti.handball.category_id),
    ...
    def test_003_verify_scores_on_home_page_featured_tab_inplay_module(self):
        ...
        if self.is_mobile:
            ...
            self.__class__.handball_sport_number = sport_number + 1
            initial_number_of_events = self.cms_config.get_max_number_of_inplay_event(
                sport_category=self.ob_config.backend.ti.handball.category_id)
            ...
            self.cms_config.update_inplay_sport_event_count(sport_number=self.handball_sport_number,
    ...
    def test_004_verify_scores_on_inplay_and_live_stream_tab_module(self):
        ...
        if self.is_mobile:
            ...
            self.navigate_to_page(name='/')
            self.site.wait_content_state(state_name='Homepage')
            wait_for_category_in_inplay_sports_ribbon_home_page(category_id=self.ob_config.backend.ti.handball.category_id)

            inplay_sports = self.site.home.desktop_modules.inplay_live_stream_module.menu_carousel.items_as_ordered_dict
            handball = inplay_sports.get(self.sport_name)
            self.assertTrue(handball, msg=f'"{self.sport_name}" not found among "{inplay_sports.keys()}"')
            handball.click()
            inplay_sports.get(self.sport_name).click()
            ...
            league = leagues.get(self.league_name_in_play_live_stream_homepage)
            self.assertTrue(league, msg=f'"{self.league_name_in_play_live_stream_homepage}" league not found among "{leagues.keys()}"')
            events = league.items_as_ordered_dict
            self.assertTrue(events, msg=f'No events were found for "{self.league_name_in_play_live_stream_homepage}" league')
            ...
            leagues = self.site.home.get_module_content(module_name=module_name).accordions_list.items_as_ordered_dict
            self.assertTrue(leagues, msg=f'No leagues found for "{vec.siteserve.HANDBALL_TAB}"')
            league = leagues.get(self.league_name_in_play_live_stream_homepage)
            self.assertTrue(league, msg=f'"{self.league_name_in_play_live_stream_homepage}" league not found')
            events = league.items_as_ordered_dict
            self.assertTrue(events, msg=f'No events were found for "{self.league_name_in_play_live_stream_homepage}" league')
    ...
    def test_005_go_to_handball_landing_page_in_play_tab_module(self):
        """
        DESCRIPTION: Go to **Handball Landing page > 'In Play'** tab/module
        EXPECTED: - Scores are NOT shown in the event name
        EXPECTED: - Each score for particular team is shown in the same row as team's name near the Price/Odds button
        EXPECTED: - Score corresponds to the ** 'value'** attribute from WS
        EXPECTED: Score for the home team is shown opposite the home team name (roleCode="HOME"/"TEAM_1")
    ...
    def test_007_navigate_to_edp_of_the_appropriate_event(self):
        ...
        EXPECTED: ![](index.php?/attachments/get/36791)
        """
