...
@pytest.mark.desktop
@vtest
class Test_C59888775_Verify_MS_on_Matches_Tab_for_Rugby_Union_SLP(BaseBetSlipTest, BaseSportTest):
    """
    TR_ID: C59888775
    ...
    def test_000_preconditions(self):
        ...
        # getting matches tab data from cms
        ...
        for market in self.__class__.cms_markets:
            self.expected_display_names.append(market.get('title').strip().upper())
            self.expected_aggregate_market_names.append(market.get('title').strip().upper()) if ',' in market.get(
                'templateMarketName') else self.expected_none_aggregated_market_names.append(
                market.get('title').strip().upper())
        ...
        if tests.settings.backend_env != 'prod':
            ...
            self.cms_config.verify_and_update_sport_config(
                sport_category_id=self.ob_config.rugby_union_config.category_id,
    ...
    def test_001_verify_displaying_of_the_market_selector_dropdown_list(self):
        ...
        self.assertIn(selected_value.upper(), self.expected_display_names,
                         msg=f'Actual selected value: "{selected_value.upper()}" is not in'
                             f'Expected display names: "{self.expected_display_names}"')
        ...
        if tests.settings.backend_env != 'prod':
            for item in actual_list:
                self.assertIn(item, self.expected_display_names,
                              msg=f'actual List item: "{item}" is not present in expected List: "{self.expected_display_names}"')
    ...
    def test_003_select_match_result_in_the_market_selector_dropdown_list(self):
        ...
        for market_name in actual_markets:
            ...
            if market_name.upper() in self.expected_aggregate_market_names:
    ...
    def test_006_repeat_steps_3_5_for_the_following_markets_handicap_2_wayhandicap_total_points(self):
        ...
        if self.device_type == 'mobile':
            ...
            self.site.wait_content_state_changed()
        dropdown_names = list(self.site.sports_page.tab_content.dropdown_market_selector.items_as_ordered_dict.keys())[1:]
        for market_name in dropdown_names:
            dropdown = self.site.sports_page.tab_content.dropdown_market_selector.items_as_ordered_dict
            dropdown.get(market_name).click()
            # selecting markets
    ...
    def test_007_verify_bet_placement_for_single_multiple_and_quick_bet_for_the_below_markets_match_result_handicap_2_way_handicap_in_coral_and_handicap_2_way_in_lads_total_points(self):
        ...
        """
        # Bet placement for "Match Betting" and "Handicap" is covered in above steps
        # Total Points
