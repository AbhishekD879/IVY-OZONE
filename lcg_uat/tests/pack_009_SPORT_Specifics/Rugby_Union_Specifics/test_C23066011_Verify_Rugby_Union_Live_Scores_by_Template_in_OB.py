...
@pytest.mark.desktop
@pytest.mark.in_play
@pytest.mark.rugby_union
@pytest.mark.live_scores
@vtest
class Test_C23066011_Verify_Rugby_Union_Live_Scores_by_Template_in_OB(BaseFallbackScoreboardTest):
    """
    TR_ID: C23066011
    NAME: Verify Rugby Union Live Scores by Template in OB
    DESCRIPTION: This test case verifies that Live Score for Rugby Union are shown and updated through OB name template
    ...
    rugby_union_sport_number, sport_event_count, in_play_event_count = None, None, None
    ...
    def custom_tearDown(cls, **kwargs):
        if cls.is_mobile and tests.settings.cms_env != 'prd0':
            ...
            if cls.rugby_union_sport_number is not None and cls.sport_event_count is not None:
                cms_config.update_inplay_sport_event_count(sport_number=cls.rugby_union_sport_number,
    ...
    def test_000_preconditions(self):
        ...
        """
        self.check_fallback_scoreboard_is_configured_for_sport(category_id=self.ob_config.rugby_union_config.category_id)
        self.check_bip_score_is_configured_for_sport(category_id=self.ob_config.rugby_union_config.category_id)
        self.check_sport_presence_on_inplay(sport_name='rugby-union')
        event_params = self.ob_config.add_rugby_union_event_to_rugby_union_all_rugby_union(score=self.score, is_live=True,
                                                                                           perform_stream=True)
    ...
    def test_002_verify_scores_on_watch_live_and_rugby_union_pages_from_inplay_page(self):
        """
        DESCRIPTION: **For Mobile/Tablet:**
        DESCRIPTION: **'In-Play'** page from the Sports Menu Ribbon
        DESCRIPTION: Check on **'Watch live'** and **Rugby Union** pages
        ...
        EXPECTED: ![](index.php?/attachments/get/36793)
        """
        ...
        wait_for_category_in_inplay_ls_structure(category_id=self.ob_config.backend.ti.rugby_union.category_id)
    ...
    def test_003_verify_scores_on_home_page_featured_tab_inplay_module(self):
        ...
        if self.is_mobile:
            ...
            self.__class__.rugby_union_sport_number = sport_number + 1
            ...
            self.__class__.sport_event_count = self.cms_config.get_sport_event_count(
                sport_number=self.rugby_union_sport_number)
            initial_number_of_events = self.cms_config.get_max_number_of_inplay_event(
                sport_category=self.ob_config.backend.ti.rugby_union.category_id)
            ...
            self.cms_config.update_inplay_sport_event_count(sport_number=self.rugby_union_sport_number,
            ...
            self.assertTrue(events, msg=f'No events were found under Rugby Union section')
    ...
    def test_004_verify_scores_on_in_play_and_live_stream_tab_module(self):
        ...
        if self.is_mobile:
            ...
            wait_for_category_in_inplay_structure(category_id=self.ob_config.backend.ti.rugby_union.category_id)
            ...
            self.navigate_to_page(name='/')
            self.site.wait_content_state(state_name='Homepage')
            wait_for_category_in_inplay_sports_ribbon_home_page(category_id=self.ob_config.backend.ti.rugby_union.category_id)

            inplay_sports = self.site.home.desktop_modules.inplay_live_stream_module.menu_carousel.items_as_ordered_dict
            rugby_union = inplay_sports.get(self.sport_name)
            self.assertTrue(rugby_union, msg=f'"{self.sport_name}" not found among "{inplay_sports.keys()}"')
            rugby_union.click()
    ...
    def test_005_go_to_rugby_union_landing_page_in_play_tab_module(self):
        """
        DESCRIPTION: Go to **Rugby Union Landing page > 'In Play'** tab/module
        EXPECTED: - Scores are NOT shown in the event name
        EXPECTED: - Each score for particular team is shown in the same row as team's name near the Price/Odds button
        EXPECTED: - Score corresponds to the ** 'value'** attribute from WS
        EXPECTED: Score for the home team is shown opposite the home team name (roleCode="HOME"/"TEAM_1")
        EXPECTED: - Score for the away team is shown opposite the away team name (roleCode="AWAY"/"TEAM_2")
        """
        self.navigate_to_page(name='sport/rugby-union')
        self.site.wait_content_state(state_name='RugbyUnion')
        delimiter = '42/31,' if self.is_mobile else '42'
        if self.is_mobile:
            resp = get_in_play_module_from_ws(delimiter=delimiter)
            self.softAssert(self.assertTrue, resp,
                            msg='In Play module is not configured for Rugby Union Landing Page')
            self.softAssert(self.assertTrue, self.site.sports_page.tab_content.has_inplay_module(),
                            msg='In Play module is not shown for Rugby Union landing page')
            ...
            self.softAssert(self.assertTrue, self.site.sports_page.tab_content,
                            msg='In Play module is not enabled for Rugby Union landing page')
    ...
    def test_007_navigate_to_edp_of_the_appropriate_event(self):
        ...
        EXPECTED: ![](index.php?/attachments/get/36792)
        """
        ...
        self.assertNotIn(self.new_away_score, away_team, msg=f'Away Score is showed in the away team name "{away_team}"')
        actual_event_name = title.event_name.replace('\n', ' ')
        expected_event_name = f'{self.team1} {self.new_home_score} {self.new_away_score} {self.team2}'
        self.assertEqual(actual_event_name, expected_event_name,
                         msg=f'Actual event name "{actual_event_name}" with team names and scores are '
                             f'not same as expected "{expected_event_name}"')
