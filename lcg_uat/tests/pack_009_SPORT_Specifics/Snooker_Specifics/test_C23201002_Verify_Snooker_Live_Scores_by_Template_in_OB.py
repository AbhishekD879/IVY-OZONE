...
@vtest
class Test_C23201002_Verify_Snooker_Live_Scores_by_Template_in_OB(BaseFallbackScoreboardTest):
    """
    TR_ID: C23201002
    ...
    snooker_sport_number, sport_event_count, in_play_event_count = None, None, None
    ...
    def custom_tearDown(cls, **kwargs):
        if cls.is_mobile and tests.settings.cms_env != 'prd0':
            ...
            if cls.snooker_sport_number is not None and cls.sport_event_count is not None:
                cms_config.update_inplay_sport_event_count(sport_number=cls.snooker_sport_number,
    ...
    def test_000_preconditions(self):
        ...
        """
        self.check_fallback_scoreboard_is_configured_for_sport(category_id=self.ob_config.snooker_config.category_id)
        self.check_bip_score_is_configured_for_sport(category_id=self.ob_config.snooker_config.category_id)
        self.check_sport_presence_on_inplay(sport_name='snooker')
        event_params = self.ob_config.add_snooker_event_to_snooker_all_snooker(score=self.score, is_live=True, perform_stream=True)
    ...
    def test_002_verify_scores_on_inplay_watch_live_snooker_pages(self):
        """
        DESCRIPTION: **For Mobile/Tablet:**
        DESCRIPTION: **'In-Play'** page from the Sports Menu Ribbon
        DESCRIPTION: Check on **'Watch live'** and **Snooker** pages
        ...
        wait_for_category_in_inplay_ls_structure(category_id=self.ob_config.backend.ti.snooker.category_id)
    ...
    def test_003_verify_scores_for_the_event_on_home_page_featured_tab_inplay_module(self):
        ...
        if self.is_mobile:
            ...
            self.softAssert(self.assertTrue, resp, msg='In Play module is not configured for Homepage Featured')
            ...
            self.__class__.snooker_sport_number = sport_number + 1
            ...
            self.__class__.sport_event_count = self.cms_config.get_sport_event_count(
                sport_number=self.snooker_sport_number)
            initial_number_of_events = self.cms_config.get_max_number_of_inplay_event(
                sport_category=self.ob_config.backend.ti.snooker.category_id)
            sport_event_count = initial_number_of_events + 30
            inplay_event_count = initial_number_of_events + 110
            self.cms_config.update_inplay_event_count(event_count=inplay_event_count)
            self.cms_config.update_inplay_sport_event_count(sport_number=self.snooker_sport_number,
            ...
            self.assertTrue(sport_section, msg=f'"{sport_name}" sport section not found among "{in_play_module_items.keys()}"')
    ...
    def test_004_verify_scores_on_in_play_tab_and_live_stream_tabs(self):
        ...
        if self.is_mobile:
            ...
            wait_for_category_in_inplay_structure(category_id=self.ob_config.backend.ti.snooker.category_id)

            inplay_list_live = self.site.home.tab_content.live_now.items_as_ordered_dict
            sport_section = inplay_list_live.get(vec.siteserve.SNOOKER_TAB)
            self.assertTrue(sport_section, msg=f'"{vec.siteserve.SNOOKER_TAB}" sport section not found')
            sport_section.expand()
            self.assertTrue(sport_section.is_expanded(),
                            msg=f'"{vec.siteserve.SNOOKER_TAB}" section is not expanded')
            ...
            section = sections.get(vec.siteserve.SNOOKER_TAB)
            self.assertTrue(section, msg=f'"{vec.siteserve.SNOOKER_TAB}" section not found')
            section.expand()
            if section.has_show_more_leagues_button():
                section.show_more_leagues_button.click()
            leagues = section.items_as_ordered_dict
            self.assertTrue(leagues, msg=f'No leagues found for "{vec.siteserve.SNOOKER_TAB}"')
            league = leagues.get(self.league_name_in_play_live_stream_homepage)
            self.assertTrue(league, msg=f'"{self.league_name_in_play_live_stream_homepage}" league not found')
            events = league.items_as_ordered_dict
            self.assertTrue(events, msg=f'No events were found for "{self.league_name_in_play_live_stream_homepage}" league')
            ...
            wait_for_category_in_inplay_sports_ribbon_home_page(category_id=self.ob_config.backend.ti.snooker.category_id)

            inplay_sports = self.site.home.desktop_modules.inplay_live_stream_module.menu_carousel.items_as_ordered_dict
            snooker = inplay_sports.get(self.sport_name)
            self.assertTrue(snooker, msg=f'"{self.sport_name}" is not found among "{inplay_sports.keys()}"')
            snooker.click()
            ...
            if self.league_name_in_play_live_stream_homepage in leagues.keys():
                ...
                if self.event_name in list(events.keys()):
                    self.ob_config.change_score(event_id=self.event_id, team1_name=self.team1, team2_name=self.team2,
                                                home_score='4', away_score=self.new_away_score)
                    self.wait_for_score_update_from_inplay_ms(score='4', team='home', event_id=self.event_id)
                    self.verify_event_score_template(events=events,
    ...
    def test_005_go_to_snooker_landing_page_in_play_tab_module(self):
        """
        ...
        delimiter = '42/32,' if self.is_mobile else '42'
        if self.is_mobile:
            resp = get_in_play_module_from_ws(delimiter=delimiter)
            self.softAssert(self.assertTrue, resp, msg='In Play module is not configured for Snooker Landing Page')
            self.softAssert(self.assertTrue, self.site.sports_page.tab_content.has_inplay_module(),
                            msg='In Play module is not enabled for Snooker landing page')
        ...
        self.assertTrue(inplay_section, msg=f'"{vec.siteserve.IN_PLAY_TAB}" section is not found')
        self.assertTrue(inplay_section, msg='No one event section found in snooker sport')
    ...
    def test_006_for_desktop_sports_landing_page__in_play_widget_and_live_stream_widget(self):
        """
        DESCRIPTION: **For Desktop:**
