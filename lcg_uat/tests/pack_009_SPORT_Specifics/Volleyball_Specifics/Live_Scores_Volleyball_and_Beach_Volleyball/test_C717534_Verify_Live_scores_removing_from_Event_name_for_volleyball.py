...
@pytest.mark.login
@vtest
class Test_C717534_Verify_Live_scores_removing_from_Event_name_for_volleyball(BaseFeaturedTest, BaseBetSlipTest,
    ...
    sport_name = vec.siteserve.VOLLEYBALL_ACCORDION
    bet_type = vec.bet_history.MY_BETS_SINGLE_STAKE_TITLE

    def test_000_preconditions(self):
        """
        DESCRIPTION: Go to OB and create Volleyball live event with name:
        ...
        type_id = self.ob_config.backend.ti.volleyball.volleyball_austria.avl.type_id
    ...
    def test_002_live_stream_page(self):
        ...
        self.assertTrue(league, msg=f'"{self.league_name}" league not found among "{leagues.keys()}"')
        events = league.items_as_ordered_dict
        self.assertTrue(events, msg=f'No events found for "{self.league_name}"')
        actual_event = events.get(self.event_name)
    ...
    def test_006_bet_history_page(self):
        ...
        EXPECTED: '(2) 12-5 (0)' should be replaced on 'v'
        """
        self.result_event(selection_ids=list(self.selection_ids.values()), market_id=self.marketID, event_id=self.eventID)
