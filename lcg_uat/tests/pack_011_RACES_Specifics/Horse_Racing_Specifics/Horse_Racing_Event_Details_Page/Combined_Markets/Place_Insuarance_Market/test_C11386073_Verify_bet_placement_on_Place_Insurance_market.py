...
@pytest.mark.login
@vtest
class Test_C11386073_Verify_bet_placement_on_Place_Insurance_market(BaseRacing, BaseBetSlipTest):
    """
    TR_ID: C11386073
    ...
    def test_003_for_mobileclick_on_a_bet_button_of_one_of_the_selections(self):
        ...
        if self.is_mobile:
            ...
            self.assertRegexpMatches(self.site.quick_bet_panel.selection.content.odds_value, self.fractional_pattern,
                                     msg=f'Odds "{self.site.quick_bet_panel.selection.content.odds_value}" '
                                         f'does not match fractional pattern')
    ...
    def test_007_open_betslip_enter_stake_and_press_bet_now(self):
        ...
        self.assertRegexpMatches(single.odds, self.fractional_pattern,
                                 msg=f'Odds "{single.odds}" '
                                     f'does not match fractional pattern')
    ...
    def test_008_change_price_format_to_decimal_in_my_account__settings_and_repeat_steps_2_7(self):
        ...
        self.test_002_navigate_to_edp_and_open_place_insurance_market_tab()
        ...
        if self.is_mobile:
            ...
            self.assertIn(self.tab_name, event_markets_list.keys(),
                          msg=f'"{self.tab_name}" market was not found in the '
                              f'list of tabs "{event_markets_list.keys()}"')
            event_market = event_markets_list[self.tab_name]
            ...
            self.assertTrue(self.site.wait_for_quick_bet_panel(), msg='Quick Bet is not shown')
            self.assertRegexpMatches(self.site.quick_bet_panel.selection.content.odds_value, self.decimal_pattern,
                                     msg=f'Odds "{self.site.quick_bet_panel.selection.content.odds_value}" '
                                         f'does not match decimal pattern')

        self.test_004_for_mobileenter_stake_and_place_bet()
        self.test_005_click_on_a_bet_button_of_one_of_the_selections()
        self.test_006_for_mobileclick_on_add_to_betslip_button()
        ...
        self.assertRegexpMatches(single.odds, self.decimal_pattern,
                                 msg=f'Odds "{single.odds}" '
                                     f'does not match decimal pattern')
