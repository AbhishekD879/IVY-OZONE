...
@pytest.mark.login
@vtest
class Test_C2291616_Verify_UK_Tote_Quadpot_Bet_Placement(BaseUKTote, BaseBetSlipTest):
    """
    TR_ID: C2291616
    ...
    def test_003_enter_some_stake_amount_into_stake_per_line_input_field(self):
        ...
        self.assertEqual(actual_amount, self.bet_amount,
                         msg=f'Actual input field value: "{actual_amount}", expected: "{self.bet_amount}"')
    ...
    def test_006_verify_bet_details_for_quadpot_tote_bet(self):
        ...
        EXPECTED:    448 Lines
        """
        self.assertEqual(self.stake.outcome_name, f'{vec.uk_tote.UQDP} {vec.uk_tote.TOTEPOOL.title()}',
                         msg=f'Actual outcome name "{self.stake.outcome_name}", '
                             f'expected \'{f"{vec.uk_tote.UQDP} {vec.uk_tote.TOTEPOOL.title()}"}\'')
        self.assertEqual(self.stake.market_name, self.pool_type,
                         msg=f'Actual bet type "{self.stake.market_name}", expected "{self.pool_type}"')
    ...
    def test_008_verify_stake_field(self):
        ...
        self.assertEqual(total_stake, expected_total_stake,
                         msg=f'Actual total stake value "{total_stake}" is not equal expected "{expected_total_stake}"')
    ...
    def test_012_repeat_step_2_5_tap_on_remove_button(self):
        ...
        self.test_005_open_betslip_and_verify_the_quadpot_tote_bet()
