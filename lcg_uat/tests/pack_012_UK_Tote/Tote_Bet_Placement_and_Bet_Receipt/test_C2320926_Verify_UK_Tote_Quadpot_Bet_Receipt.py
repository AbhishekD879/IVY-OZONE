...
class Test_C2320926_Verify_UK_Tote_Quadpot_Bet_Receipt(BaseUKTote, BaseBetSlipTest):
    """
    TR_ID: C2320926
    NAME: Verify UK Tote Quadpot Bet Receipt
    DESCRIPTION: This test case verifies Quadpot Bet Receipt, which appears in betslip
    DESCRIPTION: after the user has placed an Quadpot Tote bet
    ...
    PRECONDITIONS: * **User should have placed an Quadpot Tote bet**
    ...
    def test_000_preconditions(self):
        """
        DESCRIPTION: Get the event with quadpot totepool and place bet
        """
        ...
        place_bet_results = self.place_multiple_legs_bet(event_id=self.eventID,
                                                         tab_name=vec.uk_tote.UK_TOTE_TABS.quadpot,
    ...
    def test_001_verify_quadpot_tote_bet_receipt_in_the_betslip(self):
        """
        DESCRIPTION: Verify Quadpot Tote bet receipt in the betslip
        ...
        self.assertEqual(self.single.event_market_name, f'{self.expected_combinations} Line at £{self.expected_total_stake} per line',
                         ...
                         f'Actual: {self.single.event_market_name}. Expected: "{self.expected_combinations} Lines"')

        for index, (leg_item_title, leg_item) in enumerate(list(self.single.multiple_legs.items_as_ordered_dict.items())):
    ...
    def test_004_click_on_reuse_selection_button(self):
        ...
        EXPECTED: * "Quadpot Totepool" bet appears in the BetSlip again
        ...
        self.assertEqual(stake.outcome_name, f'{vec.uk_tote.UQDP} {vec.uk_tote.TOTEPOOL.title()}',
                         msg=f'Outcome name is not the same as expected. '
                             f'Actual: {stake.outcome_name}. Expected: \'{"{vec.uk_tote.UQDP} {vec.uk_tote.TOTEPOOL.title()}"}\'')
    ...
    def test_005_place_a_quadpot_bet_again(self):
        """
        DESCRIPTION: Place a Quadpot bet again
        EXPECTED: * Quadpot bet is placed successfully
