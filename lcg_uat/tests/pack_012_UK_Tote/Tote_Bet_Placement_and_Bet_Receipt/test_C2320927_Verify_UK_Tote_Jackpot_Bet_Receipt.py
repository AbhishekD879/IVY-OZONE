...
@pytest.mark.login
@vtest
class Test_C2320927_Verify_UK_Tote_Jackpot_Bet_Receipt(BaseUKTote, BaseBetSlipTest):
    """
    TR_ID: C2320927
    ...
    def test_000_preconditions(self):
        """
        DESCRIPTION: Get the event with jackpot totepool and place bet
        """
        ...
        place_bet_results = self.place_multiple_legs_bet(event_id=self.eventID,
                                                         tab_name=vec.uk_tote.UK_TOTE_TABS.jackpot,
    ...
    def test_001_verify_jackpot_tote_bet_receipt_in_the_betslip(self):
        """
        DESCRIPTION: Verify Jackpot Tote bet receipt in the betslip
        ...
        receipt_sections = bet_receipt.bet_receipt_sections_list.items_as_ordered_dict
        ...
        name = f'{vec.uk_tote.UJKP} {vec.uk_tote.TOTEPOOL.title()}'
        ...
        self.assertEqual(self.single.event_market_name,
                         f'{self.expected_combinations} Line at £{self.expected_total_stake} per line',
        ...
        total_est_returns_raw = bet_receipt.footer.total_est_returns_raw
        ...
        self.assertTrue(bet_receipt.footer.has_reuse_selections_button(),
                        msg='"Reuse Selection" button is not displayed')
        self.assertTrue(bet_receipt.footer.has_done_button(),
    ...
    def test_002_verify_total_stake(self):
        ...
        """
        total_stake = self.single.total_stake
        self.assertEqual(total_stake, self.expected_total_stake,
                         msg=f'Total stake is not the same as expected. '
                             f'Actual: "{total_stake}" Expected: "{self.expected_total_stake}"')
    ...
    def test_004_click_on_reuse_selection_button(self):
        ...
        self.assertEqual(stake.outcome_name, f'{vec.uk_tote.UJKP} {vec.uk_tote.TOTEPOOL.title()}',
                         msg=f'Outcome name is not the same as expected. '
                             f'Actual: "{stake.outcome_name}". Expected: \'{f"{vec.uk_tote.UJKP} {vec.uk_tote.TOTEPOOL.title()}"}\'')
    ...
    def test_005_place_a_jackpot_bet_again(self):
        """
        DESCRIPTION: Place a Jackpot bet again
        EXPECTED: * Jackpot bet is placed successfully
