...
@pytest.mark.other
@vtest
class Test_C65950041_Verify_max_payout_message_for_multiple_draws_and_multiple_weeks(Common):
    """
    TR_ID: C65950041
    NAME: Verify max payout message for multiple draws and multiple weeks
    DESCRIPTION: This testcase verifies max payout message for multiple draws and multiple weeks
    ...
    def select_random_draws(self, draw_dict):
        ...
        first_expected_day_and_time = f'{date_name} - {time_draw_name.split()[-1]}'
        second_week_expected_day_and_time = self.next_week_day_and_date(input_date=first_expected_day_and_time)
        expected_day_and_time = f'{first_expected_day_and_time}, {second_week_expected_day_and_time}'
    ...
    def verify_betslip(self, draw_name, expected_day_and_time):
        ...
        expected_est_return_value = "{:.2f}".format((
                                                            (self.bet_amount * 1) + (
                                                                self.main_winning_amount * self.bet_amount)) * 2)
        ...
        self.assertEqual(actual_single_bet_est_return_value, expected_single_bet_est_return_value,
                         msg=f'Actual total stake from FE: {actual_single_bet_est_return_value} is not the same as '
                             f'expected total stake calculations: {expected_single_bet_est_return_value}')
        ...
        expected_double_bet_est_return_value = "{:.2f}".format((
                                                                       ((self.bet_amount * 1) + (
                                                                                   self.double_ball_winning_amount * self.bet_amount)) * 10) * 2)
    ...
    def test_005_choose_multiple_draws_and_multiple_weeks(self):
        """
        DESCRIPTION: Choose multiple draws and multiple weeks
        EXPECTED: Able to select multiple draws and multiple weeks
        """
        # covered in step 4
