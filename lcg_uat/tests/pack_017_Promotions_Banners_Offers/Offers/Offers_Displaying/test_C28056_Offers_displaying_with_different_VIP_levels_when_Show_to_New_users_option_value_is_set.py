...
class Test_C28056_Offers_displaying_with_different_VIP_levels_when_Show_to_New_users_option_value_is_set(Common):
    ...
    no_vip_offer = 'offer with no vip C28056'
    vip_less_than_X_offer = 'offer with vip_less_X C28056'
    vip_X_offer = 'offer with vip_X C28056'
    vip_greater_than_X_offer = 'offer with vip_greater_X C28056'
    ...
    def get_right_column_items(self):
        ...
        if not right_column_items.get(module):
            self.__class__.offer_names = []
        else:
    ...
    def test_000_preconditions(self):
        ...
        no_vip_offer_module = next((module for module in all_cms_offer_modules if module.get('name') == f'{self.no_vip_offer}'), None)
        ...
        self.cms_config.add_offer(offer_module_id=no_vip_offer_module_id, showOfferTo='new')

        vip_less_than_X_offer_module = next((module for module in all_cms_offer_modules if module.get('name') == f'{self.vip_less_than_X_offer}'), None)
        ...
        self.cms_config.add_offer(offer_module_id=vip_less_than_X_offer_module_id, vipLevelsInput=vip_level, showOfferTo='new')

        vip_X_offer_module = next((module for module in all_cms_offer_modules if module.get('name') == f'{self.vip_X_offer}'), None)
        ...
        self.cms_config.add_offer(offer_module_id=vip_X_offer_module_id, vipLevelsInput=vip_level, showOfferTo='new')

        vip_greater_than_X_offer_module = next((module for module in all_cms_offer_modules if module.get('name') == f'{self.vip_greater_than_X_offer}'), None)
        ...
        self.cms_config.add_offer(offer_module_id=vip_greater_than_X_offer_module_id, vipLevelsInput=vip_level, showOfferTo='new')
    ...
    def test_003_select_offer_with_include_vip_levels__x_and_verify_offer_displaying(self):
        ...
        self.assertNotIn(self.vip_X_offer.upper(), self.offer_names,
                         msg=f'Offer: "{self.vip_X_offer.upper()}" option value is displayed in "{self.offer_names}"')
    ...
    def test_005_select_offer_with_empty_include_vip_levels_option_and_verify_its_displaying(self):
        ...
        self.assertNotIn(self.no_vip_offer.upper(), self.offer_names,
                         msg=f'Offer: "{self.no_vip_offer.upper()}" option value is displayed in "{self.offer_names}"')
