...
@pytest.mark.medium
@vtest
class Test_C884423_Beaten_by_a_Length_Promotional_Overlay_on_Race_event(BaseRacing, BasePromotionTest):
    """
    TR_ID: C884423
    NAME: Verify 'Beaten by a Length' Promotional Overlay on Race_event
    ...
    promo_name = 'Beaten by a Length'
    event_level_flag, market_level_flag = 'EVFLAG_BBL', 'MKTFLAG_BBAL'
    ...
    def test_000_preconditions(self):
        ...
        vec.dialogs.DIALOG_MANAGER_BEATEN_BY_A_LENGTH = vec.dialogs.DIALOG_MANAGER_BEATEN_BY_A_LENGTH.format(dialog_name)

        event = self.ob_config.add_UK_racing_event(beaten_by_a_length=True, number_of_runners=2)
    ...
    def test_001_navigate_to_any_page_with_promo_signposting_and_tap_on_promo_icon(self):
        ...
        event = self.get_racing_event(section_name=self.uk_and_ire_type_name,
                                      meeting_name=self.horseracing_autotest_uk_name_pattern.upper(),
                                      event_off_time=self.event_off_time)
        event.scroll_to()
        self.assertTrue(event.promotion_icons.has_beaten_by_a_length(),
                        msg=f'"{self.promo_name}" promotion icon is not shown')

        event.promotion_icons.beaten_by_a_length.click()

        self.__class__.dialog = self.site.wait_for_dialog(dialog_name=vec.dialogs.DIALOG_MANAGER_BEATEN_BY_A_LENGTH, timeout=2)
    ...
    def test_006_verify_main_content(self):
        ...
        """
        actual_promo_description = self.promotion_overlay.details.description
        expected_promo_description = re.sub('\n', '', cleanhtml(self.promo_details['description'], clean_buttons=True))
    ...
    def test_007_verify_short_description(self):
        ...
        expected_promo_short_description = cleanhtml(self.promo_details['shortDescription']).strip().upper()
    ...
    def test_008_verify_terms_and_conditions(self):
        ...
        actual_promo_terms = cleanhtml(promo_terms_section.content).lstrip()
        promo_terms = html.unescape(cleanhtml(self.promo_details['htmlMarkup']))
        expected_promo_terms = promo_terms.replace('\n', '').lstrip()
    ...
    def test_009_verify_cta_button(self):
        ...
        result = wait_for_result(lambda: self.device.get_current_url() == expected_url,
                                 name='Page URL to change', timeout=0.5)
