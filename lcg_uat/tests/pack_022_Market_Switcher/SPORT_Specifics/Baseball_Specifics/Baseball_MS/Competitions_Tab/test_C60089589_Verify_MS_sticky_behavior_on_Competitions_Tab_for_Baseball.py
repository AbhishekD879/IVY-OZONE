...
@vtest
class Test_C60089589_Verify_MS_sticky_behavior_on_Competitions_Tab_for_Baseball(BaseSportTest):
    """
    TR_ID: C60089589
    NAME: Verify MS sticky behavior on Competitions Tab for Baseball
    DESCRIPTION: This test case verifies 'Market Selector' sticky behavior on Baseball Competition page
    PRECONDITIONS: CMS Configurations:
    PRECONDITIONS: a) Market switcher toggle should be configured in CMS global level and individual sport level in config tab in System Configuration
    PRECONDITIONS: b) The markets which are mentioned under Note, those markets “Market template name” which is in OB should be mentioned in “Primary markets” section in pipe symbol separated by comma(** No space between two market template) in Sports configuration for Particular sport.
    PRECONDITIONS: eg: |Total Runs (Over/Under)|,|Money Line|
    PRECONDITIONS: c) Dispsort name should be mentioned for the above created markets in Dispsort field.
    PRECONDITIONS: eg: HH,HL,WH
    PRECONDITIONS: 1. Load the app
    PRECONDITIONS: 2. Go to the Baseball Competition page -> 'Matches' tab
    ...
    def verify_sticky_behavior(self, market):
        is_sticky = self.baseball_tab_content.is_market_selector_sticky
        self.assertTrue(is_sticky, msg='"Market Selector" is not sticky in Baseball')
        drop_down_list = self.baseball_tab_content.dropdown_market_selector
        drop_down_list.items_as_ordered_dict.get(market).click()
        selected_market = self.baseball_tab_content.dropdown_market_selector.value
    ...
    def test_000_pre_conditions(self):
        ...
        self.cms_config.verify_and_update_market_switcher_status(sport_name='baseball', status=True)
        ...
        self.site.baseball.tabs_menu.click_button(self.expected_sport_tabs.competitions)
        ...
        has_market_selector = self.baseball_tab_content.has_dropdown_market_selector()
    ...
    def test_001_verify_that_market_selector_is_sticky_when_scrolling_the_page_down(self):
        ...
        self.__class__.markets = list(
            self.baseball_tab_content.dropdown_market_selector.items_as_ordered_dict.keys())
