...
@vtest
class Test_C65802897_Verify_MS_on_Competitions_Tab_Total180_Handicap_for_Darts(BaseBetSlipTest):
    """
    TR_ID: C65802897
    NAME: Verify MS on Competitions Tab Total180 and Handicap for Darts
    ...
    def test_000_precondition(self):
        ...
        cms_markets = competations_tab_data.get('marketsNames', None)

        self.__class__.expected_market_selector_options = [market.get('title').strip().upper() for market in
                                                           cms_markets]
        ...
        self.assertEqual(self.site.darts.tabs_menu.current.upper(), self.expected_tab_name.upper(),
    ...
    def test_003_select_match_result_in_the_market_selector_dropdown_list(self):
        ...
        for market_name in markets_dropdown_list[2:]:
            try:
            ...
            sleep(2)
            # Total 180s
            if market_name.upper() == vec.siteserve.EXPECTED_MARKETS_NAMES.total_180s_over_under.upper():
                self.verify_fixture_header_and_qty_of_bet_buttons(bet_button_qty=2, header1=vec.sb.OVER.upper(), header3=vec.sb.UNDER.upper())
            # Handicap
            elif market_name.upper() == vec.siteserve.EXPECTED_MARKETS_NAMES.handicap_2_way.upper():
                self.verify_fixture_header_and_qty_of_bet_buttons(bet_button_qty=2, header1='1', header3='2')
            elif market_name.upper() == vec.siteserve.EXPECTED_MARKETS_NAMES.handicap.upper():
                self.verify_fixture_header_and_qty_of_bet_buttons(bet_button_qty=2, header1='1', header3='2')
            self.place_bet_and_verify()
            options = self.site.competition_league.tab_content.dropdown_market_selector
    def test_004_verify_bet_placement_for_single_multiple_and_quick_bet_for_the_below_markets_handicap_total_180s(self):
        """
