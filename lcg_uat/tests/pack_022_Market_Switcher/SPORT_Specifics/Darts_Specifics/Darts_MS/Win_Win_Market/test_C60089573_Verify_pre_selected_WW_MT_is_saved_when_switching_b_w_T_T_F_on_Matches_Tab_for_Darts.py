...
@vtest
class Test_C60089573_Verify_pre_selected_WW_MT_is_saved_when_switching_b_w_T_T_F_on_Matches_Tab_for_Darts(BaseSportTest, BaseDataLayerTest):
    """
    TR_ID: C60089573
    NAME: Verify pre selected ‘WW’ MT is saved when switching b/w T/T/F on Matches Tab for Darts
    DESCRIPTION: This test case verifies that previously selected ‘WW Market’ Template is saved when user is switching between Today/Tomorrow/Future on Darts Landing Page
    ...
    def verify_market_event_fixture(self, eventID=None, section_name=None, expected_tab=None):
        ...
        market_selector_default_value = self.market_selector_default_value.upper() \
            if self.brand == 'ladbrokes' else self.market_selector_default_value
        ...
        self.assertTrue(event, msg=f'Event for the "{self.market_selector_default_value}" market not shown')
    ...
    def test_000_preconditions(self):
        ...
        event_params = self.ob_config.add_autotest_darts_event_for_WW_type()
        self.__class__.eventID = event_params.event_id
        self.__class__.section_name = 'DARTS - DARTS AUTOTEST WW'

        # Tomorrow Tab Event creation
        tomorrow = self.get_date_time_formatted_string(days=1)
        event_params_tomorrow = self.ob_config.add_autotest_darts_event_for_WW_type(start_time=tomorrow)
        ...
        event_params_future = self.ob_config.add_autotest_darts_event_for_WW_type(start_time=future)
    ...
    def test_002_verify_text_of_the_labels_for_match_results(self):
        """
    ...
    def test_007_verify_ga_tracking_for_the_match_betting(self):
        """
        DESCRIPTION: Verify GA Tracking for the 'Match Betting'
        ...
        EXPECTED: eventLabel: "Match Betting"
