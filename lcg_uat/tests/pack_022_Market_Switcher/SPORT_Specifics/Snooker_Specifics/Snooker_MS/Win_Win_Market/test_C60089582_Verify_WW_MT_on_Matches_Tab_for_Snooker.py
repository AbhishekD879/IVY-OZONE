...
@pytest.mark.sports
@vtest
class Test_C60089582_Verify_WW_MT_on_Matches_Tab_for_Snooker(BaseDataLayerTest):
    """
    TR_ID: C60089582
    NAME: Verify 'WW’ MT on Matches Tab for Snooker
    DESCRIPTION: This test case verifies displaying of ‘WW market’ Template is displaying by default for Snooker Landing Page on Matches Tab under Market Selector Dropdown
    ...
    def test_000_preconditions(self):
        ...
        event_params = self.ob_config.add_snooker_event_to_snooker_all_snooker()
    ...
    def test_005_verify_ga_tracking_for_the_match_result(self):
        ...
        EXPECTED: })
        """
        options = self.site.snooker.tab_content.dropdown_market_selector.items_as_ordered_dict
        if not self.dropdown.is_expanded():
            self.dropdown.expand()
        options.get(vec.siteserve.EXPECTED_MARKET_SECTIONS.match_result.title()).click()
