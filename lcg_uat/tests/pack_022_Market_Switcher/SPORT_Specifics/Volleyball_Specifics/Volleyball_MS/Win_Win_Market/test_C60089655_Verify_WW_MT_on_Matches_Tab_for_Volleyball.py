...
@pytest.mark.high
@vtest
class Test_C60089655_Verify_WW_MT_on_Matches_Tab_for_Volleyball(BaseSportTest, BaseDataLayerTest):
    """
    TR_ID: C60089655
    NAME: Verify 'WW’ MT on Matches Tab for Volleyball
    DESCRIPTION: This test case verifies displaying of ‘WW market’ Template is displaying while changing the markets in the Market Switcher dropdown in Matches Tab
    ...
    def test_001_verify_displaying_of_the_market_selector(self):
        ...
        """
        self.__class__.volleyball_tab_content = self.site.sports_page.tab_content
    ...
    def test_002_select_match_result_in_the_market_selector_dropdown_list(self):
        ...
        """
        austria_avl_section = self.site.sports_page.tab_content.accordions_list.items_as_ordered_dict.get(self.section_name)
        self.assertTrue(austria_avl_section, msg=f'Section "{self.section_name}" not found')
        event = self.get_event_from_league(event_id=self.eventID, section_name=self.section_name)
        events = austria_avl_section.items_as_ordered_dict.values()
        self.assertTrue(event, msg='Event for the "Match Result" market not shown')
        default_fixture_value = list(austria_avl_section.fixture_header.items_as_ordered_dict.keys())
        ...
        name, odds_from_page = list(austria_avl_section.items_as_ordered_dict.items())[0]
    ...
    def test_003_verify_displaying_of_preplay__events(self):
        """
        DESCRIPTION: Verify displaying of Preplay  events
        EXPECTED: Preplay  events are shown
        EXPECTED: Note: If there are no events Market Selector dropdown should not display
        """
        leagues = list(self.site.sports_page.tab_content.accordions_list.items_as_ordered_dict.values())
        if len(leagues) > 0:
            ...
            self.assertFalse(self.volleyball_tab_content.has_dropdown_market_selector(timeout=20),
                             msg='"Market Selector" drop-down is  displayed on volleyball landing page')
