...
@vtest
class Test_C60094816_Verify_display_of_HIDE_INFO_SHOW_INFO_label(BaseRacing):
    """
    TR_ID: C60094816
    ...
    def test_005_verify__hide_info_label_is_changed_to_show_info_label_on_clicking(self):
        ...
        racing_details.show_info_link.scroll_to()
        racing_details.show_info_link.click()
        wait_for_result(
            lambda: self.site.racing_event_details.show_info_link_text == vec.racing.SHOW_INFO_TEXT.upper(),
            name='Wait for Show info link to display',
            timeout=10)
        for outcome_name, outcome in list(self.outcomes.items()):
            outcome.scroll_to()
            if 'Unnamed' not in outcome_name and not outcome.is_non_runner:
                self.assertFalse(outcome.has_expanded_summary(), msg=f'The outcome "{outcome_name}" is not expanded')
