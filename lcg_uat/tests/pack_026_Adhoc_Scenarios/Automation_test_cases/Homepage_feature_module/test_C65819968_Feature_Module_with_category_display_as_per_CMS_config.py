...
@pytest.mark.other
@vtest
class Test_C65819968_Feature_Module_with_category_display_as_per_CMS_config(Common):
    """
    TR_ID: C65819968
    NAME: Feature Module with category display as per CMS config
    DESCRIPTION: Feature Module with category display as per CMS config
    ...
    def test_000_preconditions(self):
        ...
        if tests.settings.backend_env != 'prod':
            ...
            self.assertTrue(event, msg=f'No events for category Tennis ')
            category_id = event.ss_response['event']['categoryId']
        else:
            category_id = self.ob_config.tennis_config.category_id
            event = self.get_active_events_for_category(category_id=category_id)[0]
            self.assertTrue(event, msg=f'No events for category {category_id}:Tennis ')
            category_id = event['event']['categoryId']

        Featured_module = self.cms_config.add_featured_tab_module(select_event_by='Category',
                                                                  id=category_id,
    ...
    def test_010_under_events_loading_module__choose_select_events_by_as_category_from_the_drop_down(self):
        """
        DESCRIPTION: Under events loading module , choose select events by as "category from the drop down"
        EXPECTED: Enter category id of the sport
        EXPECTED: Ex : Foot ball - 16
        """
        pass
    ...
    def test_013_click_on_reload_option_under_loaded_from_open_bet(self):
        ...
        EXPECTED: Events under the configured category should be displayed
        """
        pass
    ...
    def test_014_click_on_apply_under_loaded_from_openbet(self):
        ...
        EXPECTED: Also ensure events are there in OB under the said category id and time range
        """
        pass
    ...
    def test_016_click_on_create_module(self):
        ...
        EXPECTED: Note
        EXPECTED: ![](index.php?/attachments/get/3cb7d68b-e0ad-4c28-b798-4e015818d55a)
        """
        pass
    ...
    def test_017_launch_front_end_application___lads_or_coral(self):
        ...
        EXPECTED: Example :
        EXPECTED: ![](index.php?/attachments/get/7566eb6e-4ef6-4e14-86ea-404c4c6b5e2b)
        """
        ...
        self.assertTrue(section.is_expanded(),
                        msg=f'Section "{self.module_featured_type_name}" feature module is not expanded')
        ...
        for actual_featured_module_event in actual_featured_module_events:
            self.assertIn(actual_featured_module_event, self.featured_module_events,
                          msg=f'{actual_featured_module_event} is not in expected list {self.featured_module_events}')
