...
@pytest.mark.sports
@vtest
class Test_C66007974_Verify_loading_of_upcoming_events_in_Upcoming_tab_of_In_play_module_for_American_Football(
    BaseBetSlipTest):
    """
    TR_ID: C66007974
    NAME: Verify loading of upcoming events in Upcoming tab of In-play module for American Football
    DESCRIPTION: This test case needs to verify loading of upcoming events in upcoming tab of In-play module for American Football
    ...
    def mobile_validations(self):
        name = self.get_sport_title(category_id=self.ob_config.american_football_config.category_id)
        self.site.home.menu_carousel.click_item(name)
        wait_for_haul(10)
        expected_tab_name = 'MATCHES'
        all_sub_tabs = self.cms_config.get_sport_tabs(sport_id=self.ob_config.backend.ti.american_football.category_id)
        is_matches_tab = next((tab_name.get('enabled') for tab_name in all_sub_tabs if
                               tab_name.get('displayName').upper() == expected_tab_name), None)
        if not is_matches_tab:
            raise SiteServeException('MATCHES tab is not enabled in cms')
        ...
        if current_tab.upper() != expected_tab_name.upper():
            ...
            if not self.site.sports_page.tab_content.has_inplay_module():
                raise SiteServeException("In-Play Events Are Unavailable")

        # verifying in play module presence status
        in_play_module_status = self.site.sports_page.tab_content.has_inplay_module()
        self.assertEqual(self.is_in_play_events_available, bool(in_play_module_status), msg=
                         'In-Play Module is not available')
        ...
        self.assertTrue(in_play_module.has_see_all_link(), msg='SEE ALL link is not displayed')
        ...
        self.assertTrue(active_sport, msg=f'Navigated to {active_sport} After clicking on "SEE ALL" link from '
                                          f'AMERICAN FOOTBALL IN-PLAY Module')
        ...
        self.assertLess(live_now_location, upcoming_location,
                        msg='LIVE NOW section is not at top of the UPCOMING section')
        ...
        self.assertEqual(live_events_count_from_ws, live_events_count_from_fe, msg=
        ...
        self.assertEqual(upcoming_events_count_from_fe, upcoming_events_count_from_ws, msg=
        ...
        for accordion_name, accordion in accordions.items():
            self.assertFalse(accordion.is_expanded(), msg=f'"{accordion_name}" is not in collapsed mode')
            accordion.expand()
            self.assertTrue(accordion.is_expanded(), msg=f'"{accordion_name}" is not expanded after clicking on it')
            accordion.collapse()
            self.assertFalse(accordion.is_expanded(), msg=f'"{accordion_name}" is not collapsed after clicking on it')
        ...
        self.assertTrue(self.site.sports_page.header_line.page_title.text.upper(), msg=
        f'After Clicking on Back Button from In-Play not navigated to AMERICAN FOOTBALL page')
        ...
        if self.brand == "ladbrokes":
            ...
            self.assertEqual(event_details_page_name, first_accordion_name.upper(), msg=
    ...
    def desktop_validations(self):
        ...
        for tab_name, tab in self.site.sports_page.tabs_menu.items_as_ordered_dict.items():
            ...
            tab.click()
            wait_for_haul(2)
            expected_breadcrumb = f'HOME > AMERICAN FOOTBALL > {breadcrumbs_for_tab.get(tab_name).upper()}'
        ...
        for tab_name in sub_tabs:
            ...
            for accordion_name, accordion in accordions.items():
                if accordion.is_expanded():
                    accordion.collapse()
                    self.assertFalse(accordion.is_expanded(), msg=
                                     f'Accordion: "{accordion_name}" is not collapsed after clicking on it')
                    accordion.expand()
                    self.assertTrue(accordion.is_expanded(), msg=
            ...
            for event_name, event in events.items():
                ...
                if self.brand == "ladbrokes":
                    self.assertLess(more_link_location, odds_location, msg=f'More link is not displayed above the odds')
                    break
            if event.template.has_markets():
                event.template.more_markets_link.click()
                self.site.wait_content_state('EVENTDETAILS')
                event_name = self.site.sport_event_details.header_line.page_title.sport_title.upper()
    ...
    def test_000_pre_condition(self):
        ...
        PRECONDITIONS:Note: In mobile when no events are available American Football sport is not displayed in A-Z sports menu and on clicking American Football from Sports ribbon user is navigated back to the sports homepage.
        """
        in_play_events = self.get_active_events_for_category(category_id="1", in_play_event=True,
    ...
    def test_004_verify_by_clicking_in_play_tab(self):
        """
        DESCRIPTION: Verify by clicking In Play tab
        EXPECTED: DESKTOP
        EXPECTED: User should be navigate to live now tab with count display  by default if live events present .
        EXPECTED: User should be navigate to upcoming  tab with count display  by default if no  live events present.
        """
        # This step is covered in above  steps
    ...
    def test_012_verify_bet_placement_for_single_multiplecomplex(self):
        """
        ...
        if self.device_type == "mobile":
            ...
            self.site.sports_page.tab_content.grouping_buttons.items_as_ordered_dict.get("UPCOMING").click()
            sections = self.site.sports_page.tab_content.accordions_list.n_items_as_ordered_dict()
        if not sections:
            raise SiteServeException('There is no active events to place bet')
