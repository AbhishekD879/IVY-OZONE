...
@pytest.mark.sports
@vtest
class Test_C66007946_Verify_loading_of_upcoming_events_in_upcoming_tab_of_in_play_module_for_Basketball(
    BaseBetSlipTest):
    """
    TR_ID: C66007946
    NAME: Verify loading of upcoming events in upcoming tab of in-play module for Basketball
    DESCRIPTION: This test case needs to  verify loading of upcoming events in upcoming tab of In-play module for Basketball
    ...
    def mobile_validations(self):
        name = 'Basketball' if self.brand != 'bma' else 'BASKETBALL'
        ...
        if not in_play_module_status:
            ...
            in_play_module_status = self.site.sports_page.tab_content.has_inplay_module()
        self.assertEqual(self.is_in_play_events_available, bool(in_play_module_status),
        ...
        self.assertEqual(active_sport, "BASKETBALL",
                         f'Navigated to {active_sport} After clicking on "SEE ALL" link from BASKETBALL IN-PLAY Module')
        ...
        self.assertEqual(self.site.sports_page.header_line.page_title.text.upper(),
                         "BASKETBALL",
                         f'After Clicking on Back Button from In-Play not navigated to BASKETBALL page')
    ...
    def desktop_validations(self):
        self.site.sport_menu.click_item('Basketball')
        wait_for_haul(10)
        ...
        for tab_name, tab in self.site.sports_page.tabs_menu.items_as_ordered_dict.items():
            ...
            tab.click()
            wait_for_haul(2)
            expected_breadcrumb = f'HOME > BASKETBALL > {breadcrumbs_for_tab.get(tab_name).upper()}'
    ...
    def get_events_details(self, type_of_events='LIVE NOW'):
        ...
        if sections:
            types = get_inplay_sports_by_section(type=types_of_events.get(type_of_events))[0].get('eventsByTypeName')
    ...
    def test_000_pre_condition(self):
        ...
        PRECONDITIONS:Note: In mobile when no events are available Basketball sport is not displayed in A-Z sports menu and on clicking Basketball  from Sports ribbon user is navigated back to the sports homepage.
        """
        in_play_events = self.get_active_events_for_category(category_id="6", in_play_event=True,
