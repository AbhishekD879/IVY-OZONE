...
@pytest.mark.sports
@vtest
class Test_C66007947_Verify_Basketball_In_play_widget_display_in_SLP_and_verify_carousel_behaviour(BaseBetSlipTest):
    """
    TR_ID: C66007947
    NAME: Verify Basketball In-play widget display in SLP and verify carousel behaviour
    DESCRIPTION: This test case needs to  verify Basketball In-play widget display in SLP and verify carousel behaviour
    ...
    expected_tab = vec.sb.TABS_NAME_MATCHES.lower()
    home_breadcrumb = vec.sb.HOME_FOOTER_ITEM.upper()
    sport_name = vec.sb.BASKETBALL.upper()
    ...
    def validate_scores_updation(self, scores, odds, count=0):
        ...
        for widget_name, widget in self.widgets.items():
            ...
            if count == 5:
                raise VoltronException(f'prices not updated in in-play widget of basketball under match tab"')
            else:
                wait_for_haul(5)
    ...
    def test_000_preconditions(self):
        ...
        PRECONDITIONS: Sports Menu-> Sports Category ->Basketball -> Matches tab -> Enable/Disable.
        ...
        """
        self.get_active_events_for_category(category_id=self.ob_config.basketball_config.category_id, in_play_event=True)
        self.__class__.in_play_status = self.get_initial_data_system_configuration().get('DesktopWidgetsToggle').get('inPlay')
        if not self.in_play_status:
            if tests.settings.cms_env != 'prd0':
                ...
                self.__class__in_play_status = self.get_initial_data_system_configuration().get('DesktopWidgetsToggle').get('inPlay')
            else:
    ...
    def test_003_verify_basketball_landing_page(self):
        ...
        self.assertEqual(current_tab.lower(), self.expected_tab,
                         msg=f"Basketball landing page not same {current_tab} as expected {self.expected_tab}")
    ...
    def test_005_verify_score_updates(self):
        ...
        EXPECTED: Scores updates needs to be happened
        """
        scores = {}
        odds = {}
        for widget_name, widget in self.widgets.items():
    ...
    def test_006_verify_various_signposting(self):
        ...
        self.assertTrue(cashout, msg=f'Widget cashout {cashout} is not displayed')
    ...
    def test_008_verify_breadcrumbs(self):
        ...
        event_name = self.event_name.upper().replace(' V ', " VS ") if self.brand == "bma" else self.event_name.upper()
        ...
        self.assertTrue(
            ...
            msg=f'{event_name}" hyperlink from breadcrumbs is not highlighted according to the selected page')
    ...
    def test_009_verify_bet_placements_for_single_multiplecomplex_by_adding_selections_from_the_in_play_widget(self):
        ...
        sections = self.site.sports_page.in_play_widget.items_as_ordered_dict.get('In-Play LIVE Basketball')
