...
@pytest.mark.other
@vtest
class Test_C65949642_Verify_featured_coupons_display_andnavigation_to_coupons(Common):
    """
    TR_ID: C65949642
    NAME: Verify featured coupons display and
navigation to coupons
    DESCRIPTION: This test case verify featured
    ...
    def test_003_find_the_featured_coupons_displayedbelow_the_featured_coupons_incoupons_tabnote_todays_coupons_lads(self):
        """
        DESCRIPTION: Find the featured coupons displayed
        DESCRIPTION: below the featured coupons in
        DESCRIPTION: coupons tab
        DESCRIPTION: Note: Todays Coupons (Lads)
        EXPECTED: Featured coupons displayed below the
    ...
    def test_004_click_on_any_of_the_featured_couponand_navigate_to_coupons_page(self):
        """
        DESCRIPTION: Click on any of the featured coupon
        DESCRIPTION: and navigate to coupons page
        EXPECTED: Navigated to coupons detail page
        """
        featured_coupons = next((featured_coupon_dict for featured_coupon_type, featured_coupon_dict in self.all_coupons.items() if featured_coupon_type != vec.coupons.POPULAR_COUPONS.upper()),None)
        self.assertTrue(featured_coupons, msg=f'Featured_coupons leagues are not found in FE')
        random_fc_league_name, random_fc_league = next(iter(featured_coupons.items()))
        random_fc_league.click()

    def test_005_verify_cdp_displayed_with_respectivecoupon_leagues_events_in_expanded_mode(self):
        """
        DESCRIPTION: Verify CDP displayed with respective
        DESCRIPTION: coupon leagues events in expanded mode
        EXPECTED: CDP displayed with respective leagues
    ...
    def test_007_click_on_show_stats_and_verify_thedisplay_of_he_respective_data_andverify_the_hide_stats_which_undisplay_the_stats_data(self):
        """
        DESCRIPTION: Click on show stats and verify the
        DESCRIPTION: display of he respective data and
