...
@pytest.mark.grid
@vtest
class Test_C64893144_Verify_if_the_In_Shop_user_is_able_to_click_on_back_button_on_the_Breadcrumb_in_Shop_bet_tracker_page_and_navigate_back_to_the_Grid_home_page(Common):
    """
    TR_ID: C64893144
    NAME: Verify if the In-Shop user is able to click on (<) back button on the Breadcrumb in "Shop bet tracker" page and navigate back to the Grid home page.
    DESCRIPTION:
    ...
    def test_001_1_1launch_ladbrokes_sports_web_application2click_on_login_button_and_enter_valid_user_details3click_on_grid_tab_from_main_header4click_on_shop_bet_tracker_from_the_grid_hub5click_on__back_buttonexpected_result1sports_application_should_be_launched_successfully2user_should_be_logged_in3user_should_be_able_to_open_grid_tab_from_main_header4user_should_be_navigated_to_shop_bet_tracker_page5user_should_come_back_to_the_grid_home_page(self):
        ...
        self.assertEqual(actual_title, vec.retail.BET_TRACKER.title(), msg=f'Actual Title: "{actual_title}" is not Expected as: "{vec.retail.BET_TRACKER.title()}"')
        if self.device_type == 'mobile':
