...
@pytest.mark.in_play
@vtest
class Test_C44870212_In_play_sports_bet_placement(BaseCashOutTest):
    """
    TR_ID: C44870212
    ...
    def test_003_the_users_account_balance_is_sufficient_to_cover_a_bet_stake(self):
        ...
        self.assertGreaterEqual(self.balance, self.bet_amount,
                                msg=f'{self.balance} not greater than or equal to {self.bet_amount}')

    def test_004_go_to_football_in_play_event_make_a_selection_and_add_to_betslip(self, number_of_events=1):
        """
        ...
        if tests.settings.backend_env == 'prod':
            ...
            events = self.create_several_autotest_premier_league_football_events(number_of_events=number_of_events, is_live=True)
            self.assertEquals(len(events), number_of_events, msg=f'Expected number of events: {number_of_events} '
                                                                 f'actual number of events: {len(events)}')
        all_selections = []
        for event in events:
    ...
    def test_005_verify_selection_details_in_betslip(self, bet_type=None):
        ...
        for i in range(self.betslip_counter):
            ...
            self.assertIn(selection_name, self.all_selection_details['selection_names'][i],
                          msg=f'Expected selection {self.all_selection_details["selection_names"][i]} actual selection in betslip {selection_name}')
            self.assertTrue(self.stake.amount_form.is_displayed(), msg='stake box is not displayed')
            self.assertIn(self.stake.event_name, self.all_selection_details['event_name'][i],
                          msg=f'Expected event {self.all_selection_details["event_name"][i]} actual event in betslip {self.stake.event_name}')
    ...
    def test_009_repeat_steps_4_to_8_for_different_in_play_sports_and_multiple__complex_bet(self):
        ...
        for bet_type in range(len(bet_types)):
            ...
            self.test_004_go_to_football_in_play_event_make_a_selection_and_add_to_betslip(
                number_of_events=list(bet_types.values())[bet_type])
            self.test_005_verify_selection_details_in_betslip(bet_type=list(bet_types.keys())[bet_type])
            self.test_006_verify_bet_receipt_displaying_after_clickingtapping_the_place_bet_button_and_verify_bet_is_placed_successfully(
                bet_type=list(bet_types.keys())[bet_type])
