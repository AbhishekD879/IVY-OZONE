...
@pytest.mark.betslip
@vtest
class Test_C59898443_Single_Horse_Racing_Bet_Countered_by_Price_to_SP(BaseBetSlipTest):
    """
    TR_ID: C59898443
    ...
    def test_002_counter_by_price_to_sp_in_ob_ti_tool(self, event_id=None):
        """
        DESCRIPTION: Counter by Price to SP in OB TI tool
        EXPECTED: Counter offer with the SP price highlighted and potential returns shown as N/A to the customer
        """
        if not event_id:
    ...
    def test_005_if_customer_declines_the_offer(self):
        ...
        self.test_002_counter_by_price_to_sp_in_ob_ti_tool(event_id=self.event_ids[1])
        ...
        self.site.open_my_bets_open_bets()
        self.verify_bet_in_open_bets(event_name=self.event_names[1], bet_type=vec.bet_history.MY_BETS_SINGLE_STAKE_TITLE,
