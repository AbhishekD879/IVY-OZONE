...
@pytest.mark.betslip
@vtest
class Test_C59898446_Single_Tennis_Bet_Countered_by_Price(BaseBetSlipTest, BaseUserAccountTest):
    """
    TR_ID: C59898446
    ...
    def test_000_preconditions(self):
        ...
        """
        for i in range(2):
            event_params = self.ob_config.add_tennis_event_to_autotest_trophy(lp_prices=self.prices, max_bet=self.max_bet)
    ...
    def test_001_add_tennis_selection_to_quick_betbetsliptrigger_overask_try_to_place_bet_with_higher_stake_than_a_max_allowed_stake(self, selection_id=None):
        """
        DESCRIPTION: Add Tennis selection to Quick bet/Betslip
    ...
    def test_002_counter_by_price_in_ob_ti_tool(self, event_id=None):
        """
        DESCRIPTION: Counter by price in OB TI tool
        EXPECTED: Counter offer with the new price highlighted and updated potential returns shown to the customer in betslip
        """
        if not event_id:
    ...
    def test_005_if_customer_declines_the_offer(self):
        ...
        """
        self.test_001_add_tennis_selection_to_quick_betbetsliptrigger_overask_try_to_place_bet_with_higher_stake_than_a_max_allowed_stake(selection_id=self.selection_ids[1])
        self.test_002_counter_by_price_in_ob_ti_tool(event_id=self.event_ids[1])
        ...
        self.site.open_my_bets_open_bets()
        self.verify_bet_in_open_bets(event_name=self.event_names[1], bet_in_open_bets=False,
                                     bet_type=vec.bet_history.MY_BETS_SINGLE_STAKE_TITLE)
        self.navigate_to_page('bet-history')
        self.site.bet_history.tabs_menu.items_as_ordered_dict.get(vec.bet_history.OPEN_BETS_TAB_NAME).click()
        self.verify_bet_in_open_bets(event_name=self.event_names[1], bet_in_open_bets=False,
