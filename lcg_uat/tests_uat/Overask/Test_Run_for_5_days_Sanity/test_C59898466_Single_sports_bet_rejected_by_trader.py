...
@pytest.mark.betslip
@vtest
class Test_C59898466_Single_sports_bet_rejected_by_trader(BaseBetSlipTest):
    """
    TR_ID: C59898466
    NAME: Single sports bet rejected by trader
    ...
    def test_000_preconditions(self):
        ...
        """
        event = self.ob_config.add_autotest_premier_league_football_event(lp_prices=self.prices[0],
                                                                          max_bet=self.max_bet, max_mult_bet=self.max_mult_bet)
        self.__class__.event_name = event.ss_response['event']['name']
        self.__class__.eventID, self.__class__.selection_ids = event.event_id, event.selection_ids
