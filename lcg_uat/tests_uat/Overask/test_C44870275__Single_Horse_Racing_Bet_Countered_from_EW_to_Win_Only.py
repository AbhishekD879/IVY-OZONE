...
@pytest.mark.other
@vtest
class Test_C44870275__Single_Horse_Racing_Bet_Countered_from_EW_to_Win_Only(BaseRacing, BaseBetSlipTest):
    """
    TR_ID: C44870275
    ...
    def test_002_in_the_ti_change_the_type_of_the_bet_from_each_way_to_win_only(self):
        ...
        self.bet_intercept.change_bet_to_each_way_or_win(account_id=account_id, bet_id=bet_id,
                                                         betslip_id=betslip_id, bet_amount=self.sugested_max_bet, leg_type='W')
    ...
    def test_003_check_that_the_counter_offer_shows_that1_your_stake_is_highlighted2_you_see_the_win_only_signposting3_the_potential_returns_are_correct(self):
        ...
        self.assertEqual(sections.overask_trader_offer.stake_content.win_only_sign_post, 'Win Only',
                         msg=f'Actual Text: "{sections.overask_trader_offer.stake_content.win_only_sign_post}" is not same as'
                             'Expected Text: "Win Only" ')
        self.__class__.expected_return = self.get_betslip_content().total_estimate_returns
        self.verify_estimated_returns(est_returns=self.expected_return, bet_amount=self.sugested_max_bet, odds=self.new_price)
    ...
    def test_004_place_the_bet_and_check_that_the_bet_receipt_shows_the_correct_stake_and_potential_returns_and_does_not_show_any_each_way_terms_and_places_eg_12_1_2_3(self):
        ...
        self.assertEqual(actual_stake, str(self.sugested_max_bet),
                         msg=f'Actual stake: "{actual_stake}" is not same as '
                             f'Expected stake:"{str(self.sugested_max_bet)}"')
