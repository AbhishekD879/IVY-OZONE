...
@pytest.mark.other
@vtest
class Test_C44870286__Double_and_Singles_on_Football_Countered_by_Stake_for_Double_and_a_Price_for_one_of_the_Single_and_user_removes_the_counter_single_and_places_the_bet(BaseBetSlipTest):
    """
    TR_ID: C44870286
    ...
    def test_002_in_the_ti_change_the_prices_for_the_single(self):
        ...
        self.assertTrue(stakes.is_displayed(),
                        msg=f'original odds "{stakes.name}" is not crossed out and new odds "{new_odds}" are not displayed')
    ...
    def test_004_place_the_bets(self, single=True):
        ...
        if single:
            ...
            self.assertEqual(actual_price, self.new_price,
                             msg=f'Actual price "{actual_price}" is not same as New price "{self.new_price}" for single bet')
        else:
            ...
            self.assertEqual(actual_stake, suggested_stake,
                             msg=f'Actual stake "{actual_stake}" is not same as Suggested stake "{suggested_stake}" for the double bet')
    ...
    def test_005_check_the_bets_in_my_bets_open_bets_make_sure_that_the_bets_have_the_correct_prices_and_stake_and_correct_potential_returns(self, single=True):
        ...
        if single:
            ...
            self.assertEqual(float(actual_stake), float(self.bet_amount),
                             msg=f'Actual stake: "{float(actual_stake)}" is not same as Expected stake:"{float(self.bet_amount)}"')
            ...
            self.assertEqual(actual_est_returns, expected_returns,
                             msg=f'Actual Returns: "{actual_est_returns}" is not same as Expected Returns:"{expected_returns}"')
        else:
            ...
            for i in range(len(selections)):
                ...
                self.assertEqual(actual_odd, expected_odd,
                                 msg=f'Actual price "{actual_odd}" is not same as Expected price "{expected_odd}"')
            self.assertEqual(float(actual_stake), float(self.suggested_max_bet),
                             msg=f'Actual stake: "{float(actual_stake)}" is not same as Expected stake:"{float(self.suggested_max_bet)}"')
    ...
    def test_006_in_the_ti_change_the_stake_of_the_double_and_click_on_submit(self):
        ...
        overask_trader_message = wait_for_result(lambda: self.get_betslip_content().overask_trader_section.trader_message, timeout=25)
        cms_overask_trader_message = self.get_overask_trader_offer()
        self.assertEqual(overask_trader_message, cms_overask_trader_message,
                         msg=f'Actual overask message: "{overask_trader_message}" is not equal: "{cms_overask_trader_message}" from CMS')
        overask_expires_message = wait_for_result(lambda: self.get_betslip_content().overask_trader_section.expires_message, timeout=10)
    ...
    def test_007_now_repeat_steps_1_5_for_double(self):
        ...
        """
        # step1 and step 2 are covered in step 6
