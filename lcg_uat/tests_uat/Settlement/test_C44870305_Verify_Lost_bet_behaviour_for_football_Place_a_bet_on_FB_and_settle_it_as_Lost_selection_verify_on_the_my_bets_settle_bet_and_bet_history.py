...
@pytest.mark.betslip
@vtest
class Test_C44870305_Verify_Lost_bet_behaviour_for_football_Place_a_bet_on_FB_and_settle_it_as_Lost_selection_verify_on_the_my_bets_settle_bet_and_bet_history(BaseBetSlipTest):
    """
    TR_ID: C44870305
    NAME: Verify 'Lost' bet behaviour for football. (Place a bet on FB and settle it as 'Lost' selection, verify on the my bets settle bet and bet history)
    ...
    def test_003_lost_the_bet_in_open_bet_and_check_the_status_in_my_bets_and_bet_history(self):
        ...
        """
        self.ob_config.update_selection_result(event_id=self.event_params.event_id, market_id=self.market_id,
