...
class WindowClientConfig(mixins.LoggingMixin):
    ...
    @property
    def messages_title(self) -> str:
        """
        Method to get Gambling controls Menu title
        """
        messages = self.get_gvc_config_item(key_title='name', value_title='myinbox')
        messages_title = messages.get('text')
        if not messages_title:
            raise VoltronException('Cannot get Gambling controls Menu title')
        return self.__convert_right_menu_name_for_brand(messages_title)
