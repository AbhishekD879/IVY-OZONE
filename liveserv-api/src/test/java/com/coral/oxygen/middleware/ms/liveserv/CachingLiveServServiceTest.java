...
public class CachingLiveServServiceTest {
  ...
  public void setUp() throws ServiceException {
    Mockito.reset(call, messageHandler, eventIdResolver);
    Mockito.when(eventIdResolver.resolveEventId(Mockito.anyString())).thenReturn(5L);
    service =
        new LiveServeServiceFactory()
            .createCachingLiveServeService(call, messageHandler, eventIdResolver, 100, 100);
  }
  ...
  @Test
  public void tesUnsubscribeAftertDuplicateSubscription() {
    String channel = "sEVENT0102030405";
    ...
    // verification
    ArgumentCaptor<? extends EventMessageEnvelope> captor =
        ArgumentCaptor.forClass(Unsubscribed.class);
    Mockito.verify(messageHandler, Mockito.times(2)).handle(captor.capture());
    List<? extends EventMessageEnvelope> captors = captor.getAllValues();
    captors.forEach(
        message -> {
          Assert.assertEquals(channel, message.getChannel());
          Assert.assertEquals(5L, message.getEventId());
  ...
  @Test
  public void tesUnsubscribeWithoutSubscription() {
    String channel = "sEVENT0102030405";
