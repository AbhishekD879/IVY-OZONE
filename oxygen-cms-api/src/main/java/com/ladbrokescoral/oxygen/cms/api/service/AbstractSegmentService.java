...
@Slf4j
public abstract class AbstractSegmentService<T extends AbstractSegmentEntity>
    extends SortableService<T> implements SegmentedService<T> {
  ...
  protected AbstractSegmentService(
  ...
  public void dragAndDropOrder(OrderDto newOrder) {
    if (!StringUtils.hasText(newOrder.getSegmentName())
        || SegmentConstants.UNIVERSAL.equalsIgnoreCase(newOrder.getSegmentName())) {
  ...
  public void segmentedDragAndDropOrder(OrderDto newOrder) {
  ...
  /** find all the records by segmentName in segment references */
  public List<T> getAllRecordsBySegmentName(
      String brand, String segmentName, Optional<String> deviceType) {
    // FIX ME:deviceType Always empty need to validated in future
    return segmentedRepository.findAllByBrandAndSegmentName(brand, Arrays.asList(segmentName));
  }
  ...
  /** find all universal records by brand and sort by universal sortOrder */
  public List<T> getUniversal(String brand, Optional<String> deviceType) {
    // FIX ME:deviceType Always empty need to validated in future
    return segmentedRepository.findUniversalRecordsByBrand(
