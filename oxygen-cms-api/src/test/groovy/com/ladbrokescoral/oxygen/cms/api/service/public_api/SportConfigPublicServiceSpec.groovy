...
class SportConfigPublicServiceSpec extends Specification {
  ...
  def "should return valid sport configurations"() {
    ...
    def expected = TestUtil.deserializeWithJackson("service/public_api/sport-config/10.json", SportPageConfigDto.class)

    when:
    SportPageConfigDto actual = service.getSportConfig(brand, cricketSportId)
  ...
  def "should return valid Outright sport configurations"() {
    ...
    def expected = TestUtil.deserializeWithJackson("service/public_api/sport-config/12.json", SportPageConfigDto.class)

    when:
    SportPageConfigDto actual = service.getSportConfig(brand, cyclingSportId)
  ...
  def "should return valid Football sport configurations"() {
    ...
    def expected = TestUtil.deserializeWithJackson("service/public_api/sport-config/16.json", SportPageConfigDto.class)

    when:
    SportPageConfigDto actual = service.getSportConfig(brand, footballSportId)
  ...
  def "should filter out configs for invalid categories"() {
    given:
    def brand = CORAL_BRAND
    categoryRepository.findByBrandAndCategoryId(brand, 1000) >> Collections.emptyList()
    categoryRepository.findByBrandAndCategoryId(brand, 10) >> Collections.singletonList(cricketCategory())
    List<SportTab> cricketTabs = new TabsBuilder(brand, 10)
        .addTab("Matches")
        .addTab("Competitions")
        .addTab("Outrights")
        .addTab("Coupons")
        .build()
    tabRepository.findAllByBrandAndSportId(brand, 10) >> cricketTabs

    when:
    def actualConfigs = service.getSportsConfigs(CORAL_BRAND, Arrays.asList(10, 1000))

    then:
    actualConfigs.size() == 1
    actualConfigs.get(0).getSportConfig().getName() == "cricket"
  }
  ...
  def "should sport tabs sorted by sort order with hidden on top"() {
    ...
    when:
    SportPageConfigDto actualConfig = service.getSportConfig(brand, cricketSportId)

    then:
    // order: competitions, coupons, matches, outrights
    actualConfig.sportConfig.getName() == "cricket"
