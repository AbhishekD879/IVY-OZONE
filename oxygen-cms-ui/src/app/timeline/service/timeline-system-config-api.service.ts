...
@Injectable()
export class TimelineSystemConfigApiService {
  constructor(
  ...
  getOneByBrand(showLoader: boolean = true) {
    ...
    const popupData = this.apiClientService.timelineConfigService().getOneByBrand();
  ...
  create(timelineConfig: any) {
    this.globalLoaderService.showLoader();
    const getData = this.apiClientService.timelineConfigService().saveConfig(timelineConfig);
    return this.wrappedObservable(getData);
  }

  update(timelineConfig: any) {
    this.globalLoaderService.showLoader();
    const getData = this.apiClientService.timelineConfigService().updateConfig(timelineConfig);
    return this.wrappedObservable(getData);
  }
