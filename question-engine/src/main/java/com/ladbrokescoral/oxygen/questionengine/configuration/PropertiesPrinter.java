...
public class PropertiesPrinter {
  ...
  public void handleContextRefresh(ContextRefreshedEvent event) {
    String properties = StreamSupport.stream(environment.getPropertySources().spliterator(), false)
        ...
        .reduce(
            ...
            (collectedProperties, propertyName) -> collectedProperties + formatProperty(propertyName)
        );
  ...
  private String formatProperty(String propertyName) {
    return String.format("%s=%s;", propertyName, StringEscapeUtils.escapeJava(environment.getProperty(propertyName)));
  }
}
