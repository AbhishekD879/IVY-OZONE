...
public class FailOverRequestPerformer implements RequestPerformer {
  ...
  private static final transient Logger LOGGER = LoggerFactory.getLogger(FailOverRequestPerformer.class);
  ...
  public FailOverRequestPerformer(RequestPerformer delegate, FailOverStrategy failOverStrategy, Runnable reloginAction) {
  ...
  public <T> T invokeSync(Call<T> call) {
    ...
    do {
      ...
      try {
        ...
        counter++;
        FailOverStrategy.FailOverAction action = failOverStrategy.onError(convertEception(e), counter);
  ...
  private void relogin() {
    ...
    do {
      ...
      try {
        ...
        counter++;
        FailOverStrategy.FailOverAction action = failOverStrategy.onReloginError(convertEception(e), counter);
        if (FailOverStrategy.FailOverAction.RELOGIN_AND_RETRY.equals(action) //
                || FailOverStrategy.FailOverAction.RETRY.equals(action)) {
          retry = true;
          try {
  ...
  private class FailOverCallback<T> implements DataCallback<T> {
    ...
    public void onError(Throwable throwable) {
      try {
        counter++;
        FailOverStrategy.FailOverAction action = failOverStrategy.onError(convertEception(throwable), counter);
        if (FailOverStrategy.FailOverAction.RELOGIN_AND_RETRY.equals(action)) {
          relogin();
        }
        if (FailOverStrategy.FailOverAction.RELOGIN_AND_RETRY.equals(action) //
