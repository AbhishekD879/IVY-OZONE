...
public class TimeLimitRequestPerformer implements RequestPerformer {
  ...
  private static final transient Logger LOGGER =
      LoggerFactory.getLogger(TimeLimitRequestPerformer.class);
  ...
  public TimeLimitRequestPerformer(
      RequestPerformer delegate, TimeProvider timeProvider, Map<Long, Integer> limitations) {
    this.delegate = delegate;
    this.timeProvider = timeProvider;
    this.timeTrackers =
        limitations.entrySet().stream()
            .map(e -> new TimeTracker(e.getValue(), e.getKey(), timeProvider))
