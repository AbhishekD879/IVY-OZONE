...
public class FailOverRequestPerformerTest {
  ...
  @Mock private RequestPerformer delegate;

  @Mock private FailOverStrategy failOverStrategy;

  @Mock private Runnable reloginAction;

  @Mock private Call call;

  @Mock private Call callClone;
  ...
  public void setUp() {
    ...
    Mockito.doAnswer(
        ...
        .when(delegate)
        .invokeAsync(Mockito.any(), Mockito.any());
  }
  ...
  public void testRetrySync() {
    ...
    Mockito.when(failOverStrategy.onError(Mockito.any(), Mockito.anyInt()))
        .thenAnswer(
            new Answer<FailOverStrategy.FailOverAction>() {
              @Override
              public FailOverStrategy.FailOverAction answer(InvocationOnMock invocation)
                  throws Throwable {
                return ((int) invocation.getArguments()[1]) <= count
                    ? FailOverStrategy.FailOverAction.RETRY
                    : null;
              }
            });
    ...
    try {
      ...
      Mockito.verify(failOverStrategy, Mockito.times(count + 1))
          .onError(Mockito.any(), Mockito.anyInt());
      Mockito.verify(reloginAction, Mockito.never()).run();
    } catch (Exception e) {
      Assert.fail("TimeFormException should be thrown");
    }
  ...
  public void testRetryReloginSync() {
        ...
        .thenAnswer(
            new Answer<FailOverStrategy.FailOverAction>() {
              ...
              public FailOverStrategy.FailOverAction answer(InvocationOnMock invocation)
                  throws Throwable {
                return ((int) invocation.getArguments()[1]) <= count
                    ? FailOverStrategy.FailOverAction.RELOGIN_AND_RETRY
                    : null;
              }
            });
    ...
    try {
      ...
      Mockito.verify(failOverStrategy, Mockito.times(count + 1))
          .onReloginError(Mockito.any(), Mockito.anyInt());
  ...
  public void testRetryAsyncWithErrorInDelegate() {
    ...
    performer.invokeAsync(call, dataCallback);
    // verification
    Mockito.verify(dataCallback, Mockito.timeout(1000))
        .onError(Mockito.any(TimeFormException.class));
  ...
  public void testThrowCustomExceptionFromStrategySync() {
    ...
    Mockito.doThrow(customException)
        .when(failOverStrategy)
        .onError(Mockito.any(), Mockito.anyInt());
