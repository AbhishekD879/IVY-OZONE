...
public class RetryReloginFailOverStrategyTest {
  ...
  public void testRetry() {
    ...
    FailOverStrategy.FailOverAction action =
        strategy.onError(new TimeFormException(), RETRY_COUNT + 1);
  ...
  public void testReloginRetry() {
    ...
    FailOverStrategy.FailOverAction action =
        strategy.onReloginError(new TimeFormException(), RELOGIN_COUNT + 1);
    Assert.assertNull(action);
  }
    ...
    Response response =
        Response.error(
            ...
            ResponseBody.create(
                ...
                "{'odata.error':{'code':'','message':{'lang':'en-US','value':'Authorization has been denied for this request.'}}}"));
  ...
  public void testUnparsableResponse() {
    // in fact we receive 302 redirect but for test 400 is used
    Response response =
        Response.error(400, ResponseBody.create(MediaType.parse("application/json"), "EEE"));
    FailOverStrategy.FailOverAction action = strategy.onError(new TimeFormException(response), 0);
    Assert.assertEquals(FailOverStrategy.FailOverAction.RETRY, action);
  }
}
