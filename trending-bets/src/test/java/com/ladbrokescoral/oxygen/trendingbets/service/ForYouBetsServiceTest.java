...
@ActiveProfiles("TEST")
class ForYouBetsServiceTest {

  @InjectMocks private ForYouBetsService forYouBetsService;
  ...
  void testPersonalizedWithMultipleSelection() {
    ...
    personalizedBets = TestUtils.deserialize("personalizedbets.json", PersonalizedBets.class);
    when(adaService.getForYouTrendingBets(any())).thenReturn(Mono.just(personalizedBets));

    List<TrendingItem> items = personalizedBets.getPersonalizedRecords();
    ...
    Mono<PersonalizedBetsDto> sessionMono = forYouBetsService.processTrendingBets("test");
    StepVerifier.create(sessionMono)
        .assertNext(
            data -> {
              assertNotNull(data.getPositions());
            })
        .verifyComplete();
  }
  ...
  void testPersonalizedWithsameSelection() {
    ...
    when(personalizedBetsSiteServImpl.getEventToOutcomeForOutcome(items))
        .thenReturn(Mono.justOrEmpty(personalizedBets.getPersonalizedRecords()));
  ...
  @Test
  void testPersonalizedWithPersonalisedBetNull() {
